...
1. **Repository Intelligence core + Professionalization Completion may run as one sequential bounded PR for speed.** RI-1 is merged. Inside the run: first build/prove RI-2, then integrate/prove RI-3 behind the existing `workflow-map:context` / `agent:context` interfaces with tested fallback, then use RI-3 to finish Professionalization. Keep each stage internally gated and independently testable even though they may ship in one PR.
2. **Finish the entire remaining Repository & Architecture Professionalization program before Excel-native implementation.** Use RI-3 to reduce rediscovery and complete all still-valid structural, repository-hygiene, human-onboarding, evidence-policy, branding/current-name cleanup, secondary-large-module triage, and integration-aware repository-identity work that belongs to the professionalization design. Do not leave the program in an indefinite "broader work remains" state.
3. **Professionalization completion gate.** Before moving to Excel, explicitly prove the repository is organized enough for new feature work: remaining large/shared modules are either appropriately decomposed or deliberately documented as cohesive; source/test ownership is clear; stale repository-facing metadata/current branding is resolved where safe; tracked-vs-transient evidence policy is explicit; human onboarding/front-door docs are current; any repository rename/integration decision is resolved or explicitly documented as an external/manual constraint rather than an open architectural task.
4. **Excel Phase 0/readiness only after professionalization is complete, followed by Excel-Native Phase 1 and the Procurement pilot.** Preserve real bidirectional `.xlsx` round trips, validation, permissions, history, financial authority, stale-workbook conflict review, and human confirmation before apply.
...
3. **Excel-Native Operations UX — APPROVED, QUEUED AFTER PROFESSIONALIZATION.** The canonical design is `docs/superpowers/specs/2026-09-18-excel-native-operations-ux-design.md`. Complete the remaining Repository & Architecture Professionalization program first; then perform Excel Phase 0/readiness and move into the shared Excel foundation.
