...
1. **Accelerated Repository Intelligence core — RI-2 + RI-3 in one sequential bounded implementation run.** RI-1 is merged. Build the provenance-aware unified graph/query layer first, then integrate the bounded AI Context Engine behind the existing `workflow-map:context` / `agent:context` interfaces with tested fallback. Keep the implementation internally staged and independently testable even though it may ship in one PR.
2. **Excel-readiness gate / Excel Phase 0 immediately after RI-3.** Use the new bounded context to inventory authenticated operational registers, existing XLSX utilities, protected/derived fields, concurrency/version evidence, and authoritative mutation boundaries. Fix only concrete architecture blockers that genuinely prevent a safe shared Excel layer. Do not spend this gate on repository rename, cosmetic branding, generic onboarding, artifact cleanup, or arbitrary decomposition.
3. **Excel-Native Operations UX Phase 1, then the Procurement pilot, as the next product implementation once the readiness gate is green.** Preserve real bidirectional `.xlsx` round trips, validation, permissions, history, financial authority, stale-workbook conflict review, and human confirmation before apply.
4. **Remaining Repository & Architecture Professionalization work is non-blocking by default.** Continue it after the Excel foundation/pilot unless the readiness gate proves a specific item is an actual prerequisite. PR #179 remains reconciled/closed; do not revive superseded broad cleanup plans.
5. **Complete remaining Wave 4D provider/readiness work** when required Brevo/SMS credentials, devices, or safe QA prerequisites become available. Provider certification may proceed opportunistically but must not block the accelerated RI-core -> Excel path.
...
8. **Repository Intelligence RI-4 through RI-6** (structured/2D explorer, change intelligence, and agent-effectiveness hardening) remain later developer-tooling work and are not prerequisites for Excel-native implementation.
9. **Repository Intelligence RI-7 — optional 3D explorer — LAST.** Do not prioritize or start the 3D/WebGL explorer while any earlier core, approved product, provider-readiness, or release-critical work remains ahead of it unless the user explicitly changes this order.
...
3. **Excel-Native Operations UX — APPROVED AND ACCELERATED.** The canonical design is `docs/superpowers/specs/2026-09-18-excel-native-operations-ux-design.md`. After the combined RI-2/RI-3 run, perform the explicit Excel-readiness/Phase-0 gate and move directly to the shared Excel foundation when no concrete architecture blocker remains. Non-blocking repository polish does not delay Excel.
