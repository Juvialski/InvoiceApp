...
1. **Repository Intelligence core — RI-2 + RI-3 in one sequential bounded implementation run.** RI-1 is merged. Build the provenance-aware unified graph/query layer first, then integrate the bounded AI Context Engine behind the existing `workflow-map:context` / `agent:context` interfaces with tested fallback. Keep the implementation internally staged and independently testable even though it may ship in one PR.
