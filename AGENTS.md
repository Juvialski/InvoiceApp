...
2. **Excel-native implementation follows the completed professionalization boundary.** The next implementation direction is Excel Phase 0/readiness followed by the approved shared Excel foundation, subject to live repository state and the Excel design contract.
...
3. **Excel-Native Operations UX — APPROVED NEXT IMPLEMENTATION DIRECTION.** The canonical design is `docs/superpowers/specs/2026-09-18-excel-native-operations-ux-design.md`. Professionalization is complete for the current repository boundary; the next phase is Excel Phase 0/readiness, followed by the shared Excel foundation when live architecture and validation prerequisites support it.
