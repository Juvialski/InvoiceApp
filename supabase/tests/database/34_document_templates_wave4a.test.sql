...
select
  ...
  '40000000-0000-4000-8000-000000004102'::uuid as po_two_id,
  '50000000-0000-4000-8000-000000004101'::uuid as po_one_line_id,
  '50000000-0000-4000-8000-000000004102'::uuid as po_two_line_id;
grant select on wave4_ids to authenticated, service_role;
create temp table wave4_template_ids (version_one_id uuid, version_two_id uuid, client_version_id uuid, template_id uuid, snapshot_id uuid);
...
select has_function('public', 'create_document_template_version', 'template creation RPC exists');
select has_function('public', 'update_document_template_bindings', 'template binding RPC exists');
select has_function('public', 'activate_document_template_version', 'template activation RPC exists');
...
select is((select count(*) from public.document_template_versions where company_id = (select company_id from wave4_ids)), 1::bigint, 'admin can create one company-bound template version through the RPC');
...
select lives_ok($$select public.activate_document_template_version((select version_one_id from wave4_template_ids))$$, 'validated template version can be activated');
...
select (payload->>'id')::uuid, (payload->>'template_id')::uuid from created;
select lives_ok($$select public.activate_document_template_version((select client_version_id from wave4_template_ids where client_version_id is not null))$$, 'validated client invoice template can be activated');
with created as (
...
select (payload->>'id')::uuid, (payload->>'template_id')::uuid from created;
select lives_ok($$select public.activate_document_template_version((select version_two_id from wave4_template_ids where version_two_id is not null))$$, 'a second validated version can replace the active template');
