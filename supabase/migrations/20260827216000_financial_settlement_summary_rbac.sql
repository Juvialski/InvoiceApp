-- Settlement aggregate state belongs to the invoice/payroll domain, but linked
-- bank/account/reference evidence must not bypass Cash & Banking read RBAC.
-- Keep totals available to the target reader while redacting history unless the
-- caller also has cash.transactions.read.
