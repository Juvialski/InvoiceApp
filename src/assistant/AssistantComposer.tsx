  ...
  return (
    <form data-tour="assistant-composer" onSubmit={(event) => void submit(event)} className="border-t border-slate-200 bg-white p-3 sm:p-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-2 shadow-inner focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-100">
        <textarea value={draft} onChange={(event) => setDraft(event.target.value.slice(0, 8000))} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} rows={2} placeholder="Ask Invoice Operations AI..." aria-label="Ask Invoice Operations AI" className="min-h-[3.25rem] w-full resize-none border-0 bg-transparent px-1 py-1 text-xs leading-5 text-slate-800 outline-none placeholder:text-slate-400" />
        <div className="flex items-center justify-between gap-2 pt-1">
          <div>
            ...
            <button type="button" data-tour="assistant-attach" onClick={() => inputRef.current?.click()} disabled={isLoading} className="inline-flex items-center gap-1.5 rounded-xl px-2 py-1.5 text-[10px] font-bold text-slate-500 transition hover:bg-white hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-50"><Paperclip aria-hidden="true" className="h-3.5 w-3.5" /> Attach</button>
          </div>
          <button type="submit" data-tour="assistant-send" disabled={isLoading || (!draft.trim() && attachments.length === 0)} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-[10px] font-black text-white shadow-sm shadow-indigo-200 transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50">{isLoading ? <Loader2 aria-hidden="true" className="h-3.5 w-3.5 animate-spin" /> : <Send aria-hidden="true" className="h-3.5 w-3.5" />} {isLoading ? "Working…" : "Send"}</button>
        </div>
      </div>
      <p className="mt-2 text-[9px] leading-4 text-slate-400">Supported: PDF, JPG, PNG, WEBP, XLSX, CSV, TXT · 10 MB per file</p>
    </form>
  );
};
