  ...
  return (
    <article className={`flex gap-2.5 ${isUser ? "flex-row-reverse" : ""}`} aria-label={isUser ? "You" : "Invoice Operations AI"}>
      ...
      <div className={`min-w-0 max-w-[88%] ${isUser ? "items-end" : "items-start"}`}>
        <div className={`rounded-2xl px-3.5 py-3 text-xs leading-5 ${isUser ? "rounded-tr-md bg-slate-900 text-white" : isSystem ? "rounded-tl-md border border-amber-200 bg-amber-50 text-amber-950" : "rounded-tl-md border border-slate-200 bg-white text-slate-700 shadow-sm"}`}>
          <p className="whitespace-pre-wrap break-words">{message.text}</p>
          {message.warnings.length > 0 && <div className="mt-2 space-y-1.5 border-t border-amber-200/80 pt-2">{message.warnings.map((warning) => <p key={warning} className="flex items-start gap-1.5 text-[10px] leading-4 text-amber-800"><AlertTriangle aria-hidden="true" className="mt-0.5 h-3.5 w-3.5 shrink-0" />{warning}</p>)}</div>}
          {message.attachments.length > 0 && <div className="mt-2 flex flex-wrap gap-1.5 border-t border-slate-100 pt-2">{message.attachments.map((attachment) => <span key={attachment.id} className="inline-flex max-w-full items-center gap-1 rounded-lg bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-600"><FileText aria-hidden="true" className="h-3 w-3 shrink-0" /><span className="truncate">{attachment.fileName}</span></span>)}</div>}
        </div>
        {message.references.length > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{message.references.map((reference) => <span key={`${reference.type}:${reference.id || reference.label}`} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-[9px] font-bold text-slate-500">{reference.label}</span>)}</div>}
