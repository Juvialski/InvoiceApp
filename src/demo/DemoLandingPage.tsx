...
import { ArrowRight, Banknote, Bot, Building2, ClipboardList, FileCheck2, FileStack, HardHat, PlayCircle, Workflow } from "lucide-react";
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-5 py-6 sm:px-8 lg:px-12 lg:py-10">
        <header className="flex items-center justify-between gap-4 border-b border-white/10 pb-5">
          <div className="flex min-w-0 items-center gap-3">
            <span aria-hidden="true" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-200"><Workflow className="h-4 w-4" /></span>
            <div className="min-w-0">
              <p className="max-w-44 whitespace-normal break-words text-xs font-bold leading-4 tracking-wide text-white">{QA_SOFTWARE_SHOWCASE.softwareIdentity.productBrand || QA_SOFTWARE_SHOWCASE.softwareIdentity.neutralDescriptor}</p>
              <p className="mt-1 whitespace-normal break-words text-[10px] font-semibold uppercase leading-4 tracking-[0.12em] text-cyan-200/80">QA Software Showcase</p>
            </div>
          </div>
        ...
        <section className="grid flex-1 items-center gap-10 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16 lg:py-14">
          <div className="max-w-2xl">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-indigo-300">Synthetic engineering-company data · demo only</p>
            ...
            <p className="mt-5 max-w-xl text-sm leading-7 text-slate-300 sm:text-base">
              Explore an engineering operations workspace using synthetic project, finance, workforce, payroll, and engineering-document data. No production company records are involved.
            </p>
        ...
        <footer className="border-t border-white/10 pt-5 text-[10px] leading-5 text-slate-500">
          This demo workspace is isolated from production company records. Demo edits stay in this browser session and can be reset at any time.
        </footer>
      </div>
    </main>
  );
