...
function ProviderChoice({ provider, active, onOpenCompose }: { provider: SmsProviderStatus; active: boolean; onOpenCompose?: () => void }) {
  ...
  return <article className={`rounded-2xl border p-4 ${active ? "border-indigo-200 bg-indigo-50/40" : "border-slate-200 bg-white"}`} data-sms-provider={provider.providerId}>
    ...
    <p className="mt-3 text-xs leading-5 text-slate-600">{provider.message || (isGateway ? "Verify the private gateway phone connection." : "Verify the hosted account and Sender ID.")} <span className="sr-only">The server-side provider adapter keeps credentials private.</span></p>
...
export function SmsProviderStatusPanel({ onOpenCompose }: SmsProviderStatusPanelProps) {
  ...
  return (
    <section className="space-y-4" data-sms-provider-status={overview.status} aria-labelledby="sms-provider-status-title">
      <div className="flex flex-col justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-start sm:p-5"><div className="flex min-w-0 items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-700"><Smartphone className="h-5 w-5" /></div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Email / SMS</p><h2 id="sms-provider-status-title" className="mt-1 text-lg font-black text-slate-950">SMS setup</h2><p className="mt-1 max-w-2xl text-xs font-semibold leading-5 text-slate-700">{overview.providerLabel || "Company SIM Gateway / PhilSMS"} — {statusLabel(overview.status)}</p>{overview.status === "NOT_CONFIGURED" && <p className="mt-1 text-[10px] font-black uppercase tracking-wide text-slate-500">SMS · Not configured</p>}</div></div><button type="button" onClick={() => void load()} disabled={loading || !canView || !companyAccess?.activeCompanyId} className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`} />Verify</button></div>
      ...
      {canView && <details className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><summary className="cursor-pointer list-none text-sm font-black text-slate-900">Setup / Help</summary><div className="mt-4 grid gap-4 text-xs leading-5 text-slate-600 lg:grid-cols-2"><div><h3 className="font-black text-slate-900">Connect Company SIM</h3><ol className="mt-2 list-decimal space-y-1 pl-5"><li>Install the maintained Android SMS Gateway app.</li><li>Insert the company SIM and grant SMS permission.</li><li>In the Android app, open Settings → Cloud Server and enter the private server URL ending in <code>/api/mobile/v1</code> plus the private token configured for that gateway.</li><li>Activate the Cloud Server connection. After the device registers successfully, keep the generated API username and password for the {presentWorkspaceCopy("HydroQualiSense")} server integration.</li><li>Configure {presentWorkspaceCopy("HydroQualiSense")} with the private gateway server origin (or its <code>/api/3rdparty/v1</code> base), generated username/password, and any approved SIM/device selector, then Verify here before a controlled test SMS.</li></ol><p className="mt-3 text-[10px] text-slate-500">The phone may remain minimized or locked during normal use; Android/OEM battery management, internet connectivity, cellular signal, and SIM load/plan can still interrupt service.</p></div><div><h3 className="font-black text-slate-900">Configure PhilSMS</h3><ol className="mt-2 list-decimal space-y-1 pl-5"><li>Create or use the company PhilSMS account and obtain its server-side API token.</li><li>Register and obtain approval for the company Sender ID where required.</li><li>Configure the token and Sender ID on the deployment server, then verify the account balance endpoint here.</li><li>Use SMS credits for controlled Philippine transactional messages; pricing and route availability remain provider-side facts.</li></ol><p className="mt-3 text-[10px] text-slate-500">{canManage ? "An administrator must change provider environment configuration in the deployment secret store; this screen never displays or stores the token." : "Only a deployment administrator should change provider environment configuration; this screen never displays or stores the token."}</p><a className="mt-3 inline-flex items-center gap-1 font-black text-indigo-700 hover:underline" href="https://docs.sms-gate.app/getting-started/private-server/" target="_blank" rel="noreferrer">Android private-server guide <ExternalLink className="h-3 w-3" /></a></div></div></details>}
