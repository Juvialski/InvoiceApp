...
import { ArrowRight, Landmark, LockKeyhole, RotateCcw, ShieldCheck, WalletCards } from "lucide-react";
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm" aria-label={`${targetType.toLowerCase()} settlement`}>
    ...
    {summary && <>
      ...
      <div className="mt-4 space-y-2">
        {summary.historyRedacted && summary.reconciledCashPaid > 0.005 ? <p className="flex items-start gap-1.5 rounded-lg border border-slate-200 bg-slate-50 p-3 text-[10px] text-slate-600"><LockKeyhole className="mt-0.5 h-3 w-3 shrink-0" />Confirmed settlement exists, but linked account, transaction date, reference, and reversal controls require Cash & Banking transaction access.</p> : active.length ? active.map((item) => <div key={item.id} className="rounded-lg border border-slate-100 bg-slate-50 p-3">
