...
export function EmailComposePanel({
  ...
  return (
    <section className="space-y-4" data-email-compose="true" aria-labelledby="email-compose-title">
      <div className="flex flex-col justify-between gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4 sm:flex-row sm:items-start sm:p-5">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-xs leading-5 text-indigo-950">Draft an email, choose an eligible document, then review before sending.</p>
          </div>
        </div>
      ...
      <section className={`rounded-2xl border p-4 sm:p-5 ${connectionStatus === "HEALTHY" ? "border-emerald-200 bg-emerald-50/50" : connectionStatus === "RECONNECT_REQUIRED" ? "border-amber-200 bg-amber-50/60" : connectionStatus === "UNAVAILABLE" ? "border-rose-200 bg-rose-50/60" : "border-slate-200 bg-white"}`} aria-label="Gmail connection for compose">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={() => setReviewOpen((value) => !value)} className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50"><Paperclip className="h-3.5 w-3.5" />{reviewOpen ? "Hide review" : "Preview / Review"}</button><button type="button" onClick={() => void send()} disabled={busy || !canUseGmail || !reviewOpen} className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-black text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-45"><Mail className="h-3.5 w-3.5" />{busy ? "Sending…" : "Confirm & Send"}</button></div>
        </div>
