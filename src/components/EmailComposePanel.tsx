...
import { AlertCircle, CheckCircle2, FileText, Loader2, Mail, MessageSquareText, Paperclip, RefreshCw, Sparkles } from "lucide-react";
...
import { DocumentSendError, sendEmailMessage } from "../lib/documentEmail.ts";
import { newDocumentDeliveryAttemptKey } from "../lib/documentDelivery.ts";
import type { EmailProviderStatus } from "../lib/emailMessaging.ts";
import { loadEmailProviderStatus } from "../lib/emailMessaging.ts";
...
const INITIAL_STATUS: EmailProviderStatus = { status: "CONNECTION_PROBLEM", message: "Checking Brevo provider status…" };
...
export function EmailComposePanel({
  ...
  const initialKey = initialDocumentType && initialDocumentId ? `${initialDocumentType}:${initialDocumentId}` : "";
  ...
  const [providerStatus, setProviderStatus] = useState<EmailProviderStatus>(INITIAL_STATUS);
  ...
  const selectedDocument = useMemo(() => eligibleDocuments.find((entry) => documentKey(entry) === selectedKey), [eligibleDocuments, selectedKey]);
  const canUseBrevo = canSend && providerStatus.status === "READY" && !statusLoading && !historyLoading && !historyBlocked;
  ...
  useEffect(() => {
    ...
    setSubject((current) => current.trim() ? current : selectedDocument.documentType === "PURCHASE_ORDER" ? `Purchase Order ${selectedDocument.title}` : `Client Invoice ${selectedDocument.title}`);
    setMessage((current) => current.trim() ? current : selectedDocument.documentType === "PURCHASE_ORDER" ? "Please find the attached purchase order for your review and confirmation." : "Please find the attached client invoice for your review.");
  }, [selectedDocument]);

  useEffect(() => {
    let cancelled = false;
    if (!canSend || !companyAccess?.activeCompanyId) {
      setProviderStatus({ status: "CONNECTION_PROBLEM", message: "Email provider status is restricted until company access is resolved." });
      setStatusLoading(false);
      return () => { cancelled = true; };
    }
    setStatusLoading(true);
    void loadEmailProviderStatus().then((status) => { if (!cancelled) setProviderStatus(status); }).finally(() => { if (!cancelled) setStatusLoading(false); });
    return () => { cancelled = true; };
  ...
  const refreshStatus = async () => {
    if (!canSend) return;
    setStatusLoading(true);
    try { setProviderStatus(await loadEmailProviderStatus()); }
  ...
  const send = async () => {
    ...
    if (providerStatus.status !== "READY") { setError("Verify the Brevo sender setup before sending."); return; }
    ...
    if (!reviewOpen) { setError("Review the message before confirming the send."); return; }
    setBusy(true);
    try {
      ...
      const sent = await sendEmailMessage({
        to: recipients.map((email) => ({ email, ...(selectedDocument?.counterpartyEmail?.toLowerCase() === email.toLowerCase() && selectedDocument.counterparty ? { name: selectedDocument.counterparty } : {}) })),
      ...
      setResult(sent.status === "ACCEPTED" ? (selectedDocument ? `Brevo accepted the message with ${selectedDocument.title} attached; delivery is not confirmed.` : "Brevo accepted the message; delivery is not confirmed.") : "Email delivery history was recorded.");
      setReviewOpen(false);
      ...
      if (nextError instanceof DocumentSendError && nextError.reconciliationRequired) {
        ...
        setReviewOpen(false);
        setError("Email acceptance could not be confirmed safely. Check Sent / Delivery History before retrying.");
      } else {
  ...
  const statusTone = providerStatus.status === "READY" ? "border-emerald-200 bg-emerald-50/50" : providerStatus.status === "CONNECTION_PROBLEM" ? "border-rose-200 bg-rose-50/50" : "border-amber-200 bg-amber-50/60";
  const statusLabel = providerStatus.status === "READY" ? "Ready" : providerStatus.status === "SENDER_SETUP_REQUIRED" ? "Sender setup required" : providerStatus.status === "NOT_CONFIGURED" ? "Not configured" : "Connection problem";
  ...
  return (
    <section className="space-y-4" data-email-compose="true" aria-labelledby="email-compose-title">
      <div className="flex flex-col justify-between gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4 sm:flex-row sm:items-start sm:p-5">
        <div className="flex min-w-0 items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-indigo-700 shadow-sm"><MessageSquareText className="h-5 w-5" /></div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-700">Email / SMS · Compose</p><h2 id="email-compose-title" className="mt-1 text-lg font-black text-slate-950">New email</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-indigo-950">Draft an email, choose an eligible document, then review before sending through Brevo.</p></div></div>
