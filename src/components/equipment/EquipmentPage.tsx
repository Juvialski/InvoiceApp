...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return <div className="fixed inset-0 z-[90] flex items-end justify-center bg-slate-950/45 p-0 sm:items-center sm:p-4"><section role="dialog" aria-modal="true" aria-label={title} className="hqs-surface-raised hqs-border max-h-[94vh] w-full overflow-y-auto rounded-t-3xl border p-4 shadow-2xl sm:max-w-2xl sm:rounded-3xl sm:p-6"><div className="hqs-border flex items-start justify-between gap-4 border-b pb-4"><div><p className="hqs-accent-text text-[10px] font-black uppercase tracking-[0.16em]">Company Equipment</p><h2 className="hqs-primary-text mt-1 text-xl font-black">{title}</h2></div><button type="button" onClick={onClose} className="hqs-control hqs-focus-ring rounded-xl p-2" aria-label="Close dialog"><X className="h-5 w-5" /></button></div><div className="pt-4">{children}</div></section></div>;
}
...
export const EquipmentPage: React.FC<EquipmentPageProps> = ({ equipment, assignments, projects, legacyEquipment = [], siteLogsData, canRead, canManage, onOpenProject, onSave, onAssign, onTransfer, onReturn, onSetLifecycle }) => {
  ...
  const equipmentIds = useMemo(() => equipment.map((item) => item.id), [equipment]);
  const { byEntityId: mediaByEquipmentId } = useEntityMediaThumbnails("EQUIPMENT", equipmentIds, canRead);
  ...
  const save = async (input: EquipmentSaveInput) => { if (!onSave) throw new Error("Equipment management is unavailable."); return onSave(input); };
    ...
    <section className="space-y-5" data-domain="equipment-registry">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {filtered.length ? filtered.map((item) => {
          ...
          return <div key={item.id} className="grid min-w-0 gap-3 border-b border-slate-100 px-4 py-4 last:border-b-0 lg:grid-cols-[minmax(160px,1.3fr)_100px_110px_minmax(130px,1fr)_100px_minmax(150px,1fr)] lg:items-center">
            <div className="flex min-w-0 items-center gap-2"><EntityMediaThumbnail media={mediaByEquipmentId[item.id]} label="Equipment" alt={`${item.assetReference || item.equipmentName} image`} className="h-10 w-12 aspect-[4/3]" fallback={<Truck className="h-4 w-4" aria-hidden="true" />} /><div className="min-w-0"><button type="button" onClick={() => setSelected(item)} className="break-words text-left text-sm font-black text-orange-700 hover:underline">{item.assetReference || "No asset code"} · {item.equipmentName}</button><p className="mt-1 text-[10px] text-slate-500">{item.equipmentType || "Type not recorded"}{item.providerName ? ` · ${item.providerName}` : ""}</p></div></div>
      ...
      {selected && <Modal title={`${selected.assetReference || "Equipment"} · ${selected.equipmentName}`} onClose={() => setSelected(null)}>
        <div className="space-y-4">
          <EntityMediaControl entityType="EQUIPMENT" entityId={selected.id} label="Equipment" canManage={canManage} />
      ...
      {(action?.type === "ADD" || action?.type === "EDIT") && <CanonicalEquipmentWorksheetModal equipment={equipment} projects={projects} canManage={canManage} initialEquipmentId={action.item?.id} createNew={action.type === "ADD"} onClose={() => setAction(null)} onSave={save} />}
