...
export const InvoiceDirectory: React.FC<InvoiceDirectoryProps> = ({ invoices, onSelectInvoice, onDeleteInvoice, onAddNew, projects = [], projectAllocations = [] }) => {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-3" aria-label="Invoice filters">
      <div className="flex flex-col gap-2 xl:flex-row xl:items-center">
        <label className="flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5"><Search aria-hidden="true" className="h-4 w-4 shrink-0 text-slate-400" /><span className="sr-only">Search invoices</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search invoice, vendor, TIN, PO, amount…" className="w-full bg-transparent text-xs text-slate-800 outline-none placeholder:text-slate-400" /></label>
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
          <select aria-label="Review status" value={reviewFilter} onChange={(event) => setReviewFilter(event.target.value)} className="field-input min-w-[9.5rem]"><option value="ALL">All review states</option><option value="NEEDS_REVIEW">Needs review</option><option value="VERIFIED">Verified</option></select>
          <select aria-label="Payment status" value={paymentFilter} onChange={(event) => setPaymentFilter(event.target.value)} className="field-input min-w-[9.5rem]"><option value="ALL">All payment states</option><option value="UNPAID">Unpaid</option><option value="PARTIALLY_PAID">Partially paid</option><option value="PAID">Paid</option><option value="OVERDUE">Overdue</option></select>
          <select aria-label="Project" value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)} className="field-input min-w-[9.5rem]"><option value="ALL">All projects</option><option value="UNALLOCATED">Unallocated</option>{projects.filter((project) => project.status !== "ARCHIVED").map((project) => <option key={project.id} value={project.id}>{project.projectCode} — {project.projectName}</option>)}</select>
          <button type="button" onClick={() => setMoreFiltersOpen((open) => !open)} aria-expanded={moreFiltersOpen} className={`inline-flex items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-bold ${moreFiltersOpen ? "border-indigo-200 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}><Filter className="h-3.5 w-3.5" /> More filters <ChevronDown className={`h-3.5 w-3.5 transition ${moreFiltersOpen ? "rotate-180" : ""}`} /></button>
        </div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3"><p className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-500"><Files className="h-3.5 w-3.5" /> Showing <span className="text-slate-900">{filtered.length}</span> of {invoices.length}</p>{activeFilterCount > 0 && <button type="button" onClick={resetFilters} className="inline-flex items-center gap-1.5 text-[10px] font-bold text-indigo-700 hover:text-indigo-900"><RotateCcw className="h-3 w-3" /> Reset filters <span className="rounded-full bg-indigo-50 px-1.5 py-0.5">{activeFilterCount}</span></button>}</div>
    </section>
