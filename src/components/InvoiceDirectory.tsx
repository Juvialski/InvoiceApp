...
export const InvoiceDirectory: React.FC<InvoiceDirectoryProps> = ({ invoices, onSelectInvoice, onOpenCorrection, onAddNew, projects = [], projectAllocations = [] }) => {
  ...
  }), [invoices, query, reviewFilter, paymentFilter, currencyFilter, taxFilter, typeFilter, sourceFilter, duplicateFilter, lifecycleFilter, projectFilter, dateFrom, dateTo, allocationsByInvoice, projectById]);
  ...
  const counts = {
    ...
    overdue: invoices.filter((invoice) => invoice.status === "OVERDUE" && invoice.lifecycleStatus !== "VOID" && !invoice.archivedAt).length,
  };
  const activeFilterCount = [query.trim(), reviewFilter !== "ALL", paymentFilter !== "ALL", currencyFilter !== "ALL", taxFilter !== "ALL", typeFilter !== "ALL", sourceFilter !== "ALL", duplicateFilter !== "ALL", lifecycleFilter !== "ALL", projectFilter !== "ALL", dateFrom, dateTo].filter(Boolean).length;
  ...
  return <div className="space-y-5">
    <PageHeader eyebrow="Supplier evidence" title="Supplier source documents" description="Search, allocate, review, and correct preserved supplier documents without creating a second cost record." actions={<button type="button" onClick={onAddNew} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3.5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-indigo-700"><Plus className="h-3.5 w-3.5" /> Upload supplier invoice</button>} />

    <div className="flex flex-wrap items-center gap-1 border-b border-slate-200" aria-label="Invoice summary filters">
      {[['ALL', `All ${counts.all}`], ['NEEDS_REVIEW', `Needs review ${counts.review}`], ['VERIFIED', `Verified ${counts.verified}`], ['OVERDUE', `Overdue ${counts.overdue}`]].map(([value, label]) => { const selected = (value === "OVERDUE" && paymentFilter === "OVERDUE") || (value !== "OVERDUE" && reviewFilter === value && paymentFilter === "ALL"); return <button type="button" key={value} aria-pressed={selected} onClick={() => { if (value === "OVERDUE") { setPaymentFilter("OVERDUE"); setReviewFilter("ALL"); } else { setReviewFilter(value); setPaymentFilter("ALL"); } }} className={`border-b-2 px-3 py-2.5 text-xs font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 ${selected ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-800"}`}>{label}</button>; })}
    </div>
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4" aria-label="Invoice filters">
      <div className="grid gap-3 lg:grid-cols-[minmax(16rem,1.15fr)_minmax(0,1.85fr)] lg:items-start">
        ...
        <div className="grid grid-cols-2 gap-2 2xl:grid-cols-4">
          ...
          <select aria-label="Invoice lifecycle" value={lifecycleFilter} onChange={(event) => setLifecycleFilter(event.target.value)} className="field-input"><option value="ALL">All lifecycle states</option><option value="ACTIVE">Active visibility</option><option value="ARCHIVED">Archived visibility</option><option value="VOID">Voided</option></select>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-3"><p className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-500" role="status" aria-live="polite"><Files className="h-3.5 w-3.5" aria-hidden="true" /> Showing <span className="text-slate-900">{invoiceResultLabel}</span></p>{activeFilterCount > 0 && <button type="button" onClick={resetFilters} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-sm font-bold text-indigo-700 transition hover:bg-indigo-50 hover:text-indigo-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"><RotateCcw className="h-3 w-3" aria-hidden="true" /> Reset filters <span className="rounded-full bg-indigo-50 px-1.5 py-0.5">{activeFilterCount}</span></button>}</div>
    </section>
