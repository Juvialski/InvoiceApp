...
export const ReviewQueue: React.FC<ReviewQueueProps> = ({ invoices, onOpenInvoice, onStartReview, readOnly = false }) => {
  ...
  return <div className="space-y-5">
    <PageHeader eyebrow="Human verification" title="Review queue" description={description} actions={<><span className="inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1.5 text-xs font-bold text-amber-900" role="status" aria-live="polite"><AlertTriangle className="h-3.5 w-3.5" aria-hidden="true" /> {queue.length} awaiting action</span>{!readOnly && onStartReview && <button type="button" onClick={() => onStartReview(queue)} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3.5 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"><FileSearch className="h-3.5 w-3.5" aria-hidden="true" /> Start review</button>}</>} />
