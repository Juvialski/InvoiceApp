    ...
    <section className="space-y-4" data-communication-history="true" aria-labelledby="communication-history-title">
      ...
      {entries.length > 0 && <div className="space-y-3">{entries.map((entry) => {
        ...
        return <article key={entry.id} className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm sm:p-4" data-delivery-attempt={entry.id} data-delivery-status={entry.status}>
          ...
          <div className="mt-3 grid gap-2 text-xs sm:grid-cols-[7rem_minmax(0,1fr)]"><span className="font-bold text-slate-500">Recipient</span><span className="break-words font-semibold text-slate-900">{entry.recipients.join(", ") || "Not recorded"}</span><span className="font-bold text-slate-500">{isSms ? "Message" : "Subject"}</span><span className="break-words text-slate-800">{entry.subject || (isSms ? "SMS" : "No subject")}</span><span className="font-bold text-slate-500">Provider</span><span className="break-words text-slate-700">{entry.providerId || (isSms ? "SMS provider" : "Gmail")}{entry.providerMessageId ? ` · ${entry.providerMessageId}` : ""}{entry.providerStatus ? ` · ${entry.providerStatus}` : ""}</span><span className="font-bold text-slate-500">Attachment</span><span className="break-words text-slate-700">{entry.attachmentName || "No attachment"}{entry.attachmentSource === "NONE" ? "" : ` · ${entry.attachmentSource.replaceAll("_", " ")}`}</span><span className="font-bold text-slate-500">Record</span><span className="break-words text-slate-700">{hasDocument ? `${entry.documentType === "PURCHASE_ORDER" ? "Purchase Order" : "Client Invoice"} · ${entry.documentId}` : isSms ? "Transactional SMS" : "Ordinary email"}</span></div>
