...
import { reconcileSmsDelivery } from "../lib/messaging.ts";
...
function statusClass(status: DocumentDeliveryHistoryEntry["status"]) {
  if (status === "SENT" || status === "DELIVERED") return "border-emerald-200 bg-emerald-50 text-emerald-800";
  if (status === "FAILED") return "border-rose-200 bg-rose-50 text-rose-800";
  return "border-amber-200 bg-amber-50 text-amber-900";
}

function statusLabel(status: DocumentDeliveryHistoryEntry["status"]) {
  if (status === "ACCEPTED") return "Accepted by provider";
  if (status === "SENT") return "Sent";
  if (status === "DELIVERED") return "Delivered";
  if (status === "FAILED") return "Failed";
  if (status === "UNKNOWN") return "Needs reconciliation";
  return status;
}
  ...
  return (
    <section className="space-y-4" data-communication-history="true" aria-labelledby="communication-history-title">
      ...
      {entries.length > 0 && <div className="space-y-3">{entries.map((entry) => {
        ...
        const providerLabel = entry.providerId === "BREVO" ? "Brevo" : entry.providerId || (isSms ? "SMS provider" : "Gmail (historical)");
        return <article key={entry.id} className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm sm:p-4" data-delivery-attempt={entry.id} data-delivery-status={entry.status}>
          <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 flex-wrap items-center gap-2"><span className={`rounded-full border px-2 py-0.5 text-[10px] font-black ${statusClass(entry.status)}`}>{statusLabel(entry.status)}</span><span className="inline-flex items-center gap-1 text-xs font-black text-slate-800">{isSms ? <Smartphone className="h-3.5 w-3.5" /> : <Mail className="h-3.5 w-3.5" />}{isSms ? "SMS" : "Email"}</span><span className="text-[10px] text-slate-500">{entry.senderLabel}</span></div><time dateTime={entry.sentAt} className="text-[10px] text-slate-500">{timeLabel(entry.sentAt)}</time></div>
          <div className="mt-3 grid gap-2 text-xs sm:grid-cols-[7rem_minmax(0,1fr)]"><span className="font-bold text-slate-500">Recipient</span><span className="break-words font-semibold text-slate-900">{entry.recipients.join(", ") || "Not recorded"}</span><span className="font-bold text-slate-500">{isSms ? "Message" : "Subject"}</span><span className="break-words text-slate-800">{entry.subject || (isSms ? "SMS" : "No subject")}</span><span className="font-bold text-slate-500">Provider</span><span className="break-words text-slate-700">{providerLabel}{entry.providerMessageId ? ` · ${entry.providerMessageId}` : ""}{entry.providerStatus ? ` · ${entry.providerStatus}` : ""}</span><span className="font-bold text-slate-500">Attachment</span><span className="break-words text-slate-700">{entry.attachmentName || "No attachment"}{entry.attachmentSource === "NONE" ? "" : ` · ${entry.attachmentSource.replaceAll("_", " ")}`}</span><span className="font-bold text-slate-500">Record</span><span className="break-words text-slate-700">{hasDocument ? `${entry.documentType === "PURCHASE_ORDER" ? "Purchase Order" : "Client Invoice"} · ${entry.documentId}` : isSms ? "Transactional SMS" : "Ordinary email"}</span></div>
          <div className={`mt-3 flex items-start gap-2 rounded-lg border p-2.5 text-[10px] leading-4 ${entry.reconciliationRequired ? "border-amber-200 bg-amber-50 text-amber-900" : "border-slate-100 bg-slate-50 text-slate-600"}`}>{entry.reconciliationRequired ? <ShieldAlert className="mt-0.5 h-3.5 w-3.5 shrink-0" /> : entry.status === "SENT" || entry.status === "DELIVERED" ? <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-600" /> : <Clock3 className="mt-0.5 h-3.5 w-3.5 shrink-0" />}<span>{entry.safeMessage}{entry.attemptCount > 1 ? ` Attempt ${entry.attemptCount}.` : ""}</span></div>
          <div className="mt-3 flex flex-wrap justify-end gap-2">{isSms && entry.providerMessageId && entry.status !== "DELIVERED" && <button type="button" onClick={async () => { setReconcilingId(entry.id); setError(""); try { await reconcileSmsDelivery(entry.id); await refresh(); } catch (nextError) { setError(nextError instanceof Error ? nextError.message : "SMS status could not be reconciled safely."); } finally { setReconcilingId(""); } }} disabled={reconcilingId === entry.id} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-black text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw className={`h-3 w-3 ${reconcilingId === entry.id ? "animate-spin" : ""}`} />{entry.reconciliationRequired ? "Reconcile status" : "Check status"}</button>}{hasDocument && onOpenDocument && <button type="button" onClick={() => onOpenDocument(entry)} className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-black text-slate-700 hover:bg-slate-50">Open owning document</button>}{entry.resendAllowed && !entry.reconciliationRequired && onCompose && <button type="button" onClick={() => onCompose(entry)} className="inline-flex items-center gap-1 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-[10px] font-black text-indigo-700 hover:bg-indigo-100"><RotateCcw className="h-3 w-3" />Prepare new attempt</button>}{entry.reconciliationRequired && !isSms && <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-800"><AlertTriangle className="h-3 w-3" />Retry locked pending reconciliation</span>}</div>
        </article>;
      })}</div>}
    </section>
  );
