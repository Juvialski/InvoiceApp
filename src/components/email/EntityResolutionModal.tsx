...
export const EntityResolutionModal: React.FC<EntityResolutionModalProps> = ({
  ...
  const actionBadge = (action: string) => {
    switch (action) {
      case "LINK_EXISTING":
        return { label: "Link to Existing Record", bg: "bg-emerald-50 text-emerald-700 border-emerald-200" };
      case "ENRICH_EXISTING":
        return { label: "Enrich Existing Record", bg: "bg-blue-50 text-blue-700 border-blue-200" };
      case "CREATE_NEW":
        return { label: "Propose New Record", bg: "bg-purple-50 text-purple-700 border-purple-200" };
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 id="entity-resolution-modal-title" className="text-base font-black text-slate-900">
                  {isVendor ? "Vendor / Supplier Resolution" : "Financial Account Resolution"}
                </h2>
              ...
              <p className="text-xs text-slate-500">
                Advisory entity matching for &quot;{candidate.subject || candidate.sender || "Candidate"}&quot;
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-slate-800">
          ...
          {resolution.conflicts.length > 0 && (
            <div className="p-4 rounded-2xl bg-rose-50/80 border border-rose-200 text-rose-950 space-y-2">
              <div className="flex items-center gap-2 text-xs font-black text-rose-800 uppercase tracking-wide">
                ...
                <span>Authoritative Identity Conflicts Detected</span>
              </div>
                  ...
                  <li key={idx} className="flex flex-col gap-0.5 pl-6">
                    ...
                    {conflict.existingValue && conflict.candidateValue && (
                      <span className="text-[11px] text-rose-700">
                        Existing: <code className="bg-rose-100 px-1 py-0.5 rounded text-rose-900">{conflict.existingValue}</code> vs Extracted: <code className="bg-rose-100 px-1 py-0.5 rounded text-rose-900">{conflict.candidateValue}</code>
                      </span>
                    )}
                  </li>
                ))}
              ...
              <p className="text-[11px] text-rose-700 italic pl-6">
                Extraction never silently replaces authoritative company records. Explicit selection is required.
              </p>
            </div>
          )}
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
            <h3 className="text-xs font-black text-slate-700 uppercase tracking-wide flex items-center gap-1.5">
              ...
              <span>Resolution Evidence & Reasons</span>
            </h3>
          ...
          {groupMembers.length > 1 && (
            <div className="p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 space-y-2">
              <div className="flex items-center gap-2 text-xs font-black text-indigo-900 uppercase">
                ...
                <span>Same-Batch Candidate Group ({groupMembers.length} emails)</span>
              </div>
              <p className="text-xs text-indigo-800 leading-relaxed">
                The following candidates in this scan batch share compatible tax ID, sender domain, or entity evidence. They share this single entity proposal rather than creating duplicate records independently.
              </p>
          ...
          {resolution.proposedEnrichments.length > 0 && (
            <div className="p-4 rounded-2xl bg-blue-50/50 border border-blue-200 space-y-2.5">
              <div className="flex items-center gap-2 text-xs font-black text-blue-900 uppercase">
                ...
                <span>Safe Enrichment Proposals</span>
              </div>
              <p className="text-xs text-blue-800">
                New non-conflicting details found in this document. Select which fields you would like to enrich on the existing record upon confirmation:
              </p>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <span className="font-semibold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded">
                        {enrichment.proposedValue}
                      </span>
                      {enrichment.currentValue && (
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          Current value: {enrichment.currentValue}
                        </p>
                      )}
                    </div>
                  </label>
          ...
          {/* Target Entity Selector / Reviewer Override */}
          <div className="space-y-2 pt-2 border-t border-slate-100">
            <label className="block text-xs font-black text-slate-800 uppercase">
              Target {isVendor ? "Vendor" : "Financial Account"} Record
            </label>
            {isVendor ? (
              <select
              ...
              >
                <option value="">-- Propose New Vendor: {resolution.matchedEntityName || "New Vendor"} --</option>
                {vendors.map((v) => (
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-semibold text-slate-800 bg-white focus:outline-hidden focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              >
                <option value="">-- Propose New Account: {resolution.matchedEntityName || "New Account"} --</option>
            ...
            <p className="text-[10px] text-slate-400">
              You can accept the proposed match or explicitly choose a different entity before importing.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50">
          <p className="text-[11px] text-slate-500">
            Advisory resolution only. Master-data changes remain human-confirmed.
          </p>
          <div className="flex items-center gap-2">
            <button
              type="button"
