...
import { convertFinancialAmountWithFallback, displayFinancialAmountInPhp, normalizeFinancialCurrency } from "../../utils/financialCurrency.ts";
...
interface ExpensesPageProps {
  ...
  /** Stable detail selection; this is intentionally separate from correction mode. */
...
function supplierInvoiceDisplayAmount(invoice: InvoiceData, financialFxSnapshots: readonly FinancialFxSnapshot[]) {
  const amount = Number(invoice.grandTotal);
  return Number.isFinite(amount)
    ? displayFinancialAmountInPhp(amount, invoice.currency, "SUPPLIER_INVOICE", invoice.id, financialFxSnapshots)
    : { baseLabel: "Amount unresolved", requiresFx: true };
}
...
export const ExpensesPage: React.FC<ExpensesPageProps> = ({
  ...
  const selectedExpense = selectedExpenseId ? expenses.find((expense) => expense.id === selectedExpenseId) : undefined;
  const selectedExpenseLoading = Boolean(selectedExpenseId && (!expensesLoaded || workspaceDataPending));
  const selectedExpenseMissing = Boolean(selectedExpenseId && expensesLoaded && !workspaceDataPending && !selectedExpense);
  ...
  const confirmedBaseCost = useMemo(() => expenses
    ...
    .reduce((sum, expense) => {
      const linkedInvoice = expense.supplierInvoiceId ? invoiceMap.get(expense.supplierInvoiceId) : undefined;
      const converted = convertFinancialAmountWithFallback(expense.amount, expense.currency, baseCurrency, "EXPENSE", expense.id, financialFxSnapshots, linkedInvoice ? { sourceCurrency: linkedInvoice.currency, sourceType: "SUPPLIER_INVOICE", sourceId: linkedInvoice.id } : undefined);
  ...
  const expenseDetail = selectedExpenseId ? selectedExpenseLoading
    ? <section className="rounded-2xl border border-slate-200 bg-white p-5 text-xs text-slate-500" aria-label="Expense detail" role="status">Resolving the requested Expense…</section>
  ...
  return <div className="space-y-5">
    ...
    {supplierDocuments.length > 0 && <section className="space-y-3" aria-label="Supplier document work">
      ...
      {supplierDocuments.some((row) => row.state === "LINKED") && <SupplierDocumentSection title="Linked supplier source evidence" rows={supplierDocuments.filter((row) => row.state === "LINKED")} projects={projects} financialFxSnapshots={financialFxSnapshots} />}
    </section>}
...
function expenseAmountForPhp(expense: Expense, invoice: InvoiceData | undefined, snapshots: readonly FinancialFxSnapshot[]) {
  const direct = displayFinancialAmountInPhp(expense.amount, expense.currency, "EXPENSE", expense.id, snapshots);
  if (!direct.requiresFx || !invoice) return direct;
  return displayFinancialAmountInPhp(expense.amount, invoice.currency, "SUPPLIER_INVOICE", invoice.id, snapshots);
}

function expenseRouteHref(path: string, id: string) {
  return id.startsWith("demo-") && !path.startsWith("/demo/") ? `/demo/app${path}` : path;
}

interface ExpenseRegisterCardProps {
  expense: Expense;
  project?: Project;
  invoice?: InvoiceData;
  purchaseOrder?: PurchaseOrder;
...
function ExpenseRegisterCard({
  ...
  invoicePath,
  purchaseOrderPath,
}: ExpenseRegisterCardProps) {
  const needsFx = phpAmount.requiresFx;
  const navigateSource = (event: React.MouseEvent<HTMLAnchorElement>, path: string) => {
    if (!onNavigatePath) return;
    event.preventDefault();
    onNavigatePath(path);
  };
  return <article className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm" data-expense-register-card={expense.id}>
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0">
        <h3 className="break-words text-sm font-black text-slate-950">{expense.description}</h3>
        <p className="mt-1 text-[10px] text-slate-500">{expense.expenseDate} · {expense.category}</p>
      </div>
      <div className="flex shrink-0 flex-wrap justify-end gap-1">
        <StatusBadge tone={expenseTone(expense.status)} icon={expense.status === "APPROVED" || expense.status === "PAID" ? CheckCircle2 : expense.status === "VOID" ? Ban : undefined}>{expense.status}</StatusBadge>
        {expense.archivedAt && <StatusBadge tone="neutral" icon={Archive}>Archived</StatusBadge>}
        {needsFx && <StatusBadge tone="warning">FX required</StatusBadge>}
      </div>
    </div>
...
function ExpenseDetailPanel({ expense, invoice, purchaseOrder, project, returnPath, canRecordPayments, canReversePayments, financialFxSnapshots, onNavigatePath }: ExpenseDetailPanelProps) {
  ...
  const supplierInvoicePath = invoice ? appPathForInvoice(invoice.id, appPathForExpense(expense.id)) : undefined;
  const purchaseOrderPath = purchaseOrder ? appPathForPurchaseOrder(purchaseOrder.id, appPathForExpense(expense.id)) : undefined;
  const phpAmount = expenseAmountForPhp(expense, invoice, financialFxSnapshots);
  return <section className="space-y-3 rounded-2xl border border-indigo-100 bg-white p-4 shadow-sm sm:p-5" aria-label="Expense detail" data-testid="expense-detail-panel">
    <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <p className="text-[10px] font-black uppercase tracking-[0.15em] text-indigo-600">Authoritative Expense</p>
        <h2 className="mt-1 break-words text-lg font-black text-slate-950">{expense.description}</h2>
        <p className="mt-1 break-words text-xs text-slate-500">{expense.category} · {expense.expenseDate} · {expense.referenceNumber || "No reference"}</p>
      </div>
    ...
    <div className="grid min-w-0 gap-2 sm:grid-cols-2 lg:grid-cols-3">
      <DetailFact label="Payee" value={expense.payee || "Not recorded"} />
      <DetailFact label="PHP amount" value={phpAmount.baseLabel} />
      <DetailFact label="Source amount" value={phpAmount.sourceLabel || `${expense.currency} source`} />
      <DetailFact label="Project" value={project ? `${project.projectCode} · ${project.projectName}` : "Unallocated"} />
      <DetailFact label="Payment method" value={expense.paymentMethod || "Not recorded"} />
      <DetailFact label="Source" value={invoice ? `Supplier invoice ${invoice.invoiceNumber || invoice.id}` : "Direct expense"} />
      <DetailFact label="History" value="Preserved through the Expense lifecycle" />
    </div>
    {supplierInvoicePath ? <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
      <p className="min-w-0 flex-1 break-words text-[10px] text-slate-600">This Expense originated from supplier invoice evidence. The invoice remains in its owning workflow.</p>
      <a href={expenseRouteHref(supplierInvoicePath, invoice!.id)} onClick={(event) => navigate(event, supplierInvoicePath)} className="inline-flex min-h-10 shrink-0 items-center justify-center rounded-lg border border-indigo-200 bg-white px-3 py-2 text-[10px] font-black text-indigo-800 hover:bg-indigo-50">View Supplier Invoice</a>
    </div> : expense.supplierInvoiceId ? <p role="alert" className="break-words rounded-xl border border-amber-200 bg-amber-50 p-3 text-[10px] font-semibold text-amber-900">The source supplier invoice is not available in this workspace. The Expense remains the authoritative payable record.</p> : null}
    {purchaseOrderPath && <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
      <p className="min-w-0 flex-1 break-words text-[10px] text-slate-600">This Expense also carries an authoritative purchase-order source reference.</p>
      <a href={expenseRouteHref(purchaseOrderPath, purchaseOrder!.id)} onClick={(event) => navigate(event, purchaseOrderPath)} className="inline-flex min-h-10 shrink-0 items-center justify-center rounded-lg border border-indigo-200 bg-white px-3 py-2 text-[10px] font-black text-indigo-800 hover:bg-indigo-50">Open Purchase Order</a>
    </div>}
    <FinancialSettlementCard
      ...
      financialFxSnapshots={financialFxSnapshots}
      fxSourceType={invoice ? "SUPPLIER_INVOICE" : undefined}
...
function DetailFact({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 rounded-xl border border-slate-100 bg-slate-50 p-3"><p className="text-[10px] font-semibold text-slate-500">{label}</p><p className="mt-1 break-words text-xs font-black text-slate-800">{value}</p></div>;
}
