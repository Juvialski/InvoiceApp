...
export const ExpensesPage: React.FC<ExpensesPageProps> = ({
  ...
  const expenseDialogCloseButtonRef = useRef<HTMLButtonElement>(null);
  const expenseDialogRef = useDialogFocus({
    open: canManage && modal,
    onClose: closeExpenseEditor,
    initialFocusRef: expenseDialogCloseButtonRef,
  });
...
function ExpenseDetailPanel({ expense, invoice, purchaseOrder, project, returnPath, canEdit, onEditDraft, canRecordPayments, canReversePayments, financialFxSnapshots, onNavigatePath }: ExpenseDetailPanelProps) {
  ...
  return <section className="space-y-3 rounded-2xl border border-indigo-100 bg-white p-4 shadow-sm sm:p-5" aria-label="Expense detail" data-testid="expense-detail-panel">
    <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <p className="text-[10px] font-black uppercase tracking-[0.15em] text-indigo-600">Expense record</p>
