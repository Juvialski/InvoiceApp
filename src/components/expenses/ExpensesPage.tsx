...
import { appPathForCashTarget, appPathForExpense, appPathForInvoice, appPathForTab } from "../../utils/appRouting.ts";
...
export const ExpensesPage: React.FC<ExpensesPageProps> = ({
  ...
  const expenseDetail = selectedExpenseId ? selectedExpenseLoading
    ...
    : selectedExpenseMissing
      ...
      : selectedExpense
        ? <ExpenseDetailPanel expense={selectedExpense} invoice={selectedExpense.supplierInvoiceId ? invoiceMap.get(selectedExpense.supplierInvoiceId) : undefined} project={selectedExpense.projectId ? projects.find((project) => project.id === selectedExpense.projectId) : undefined} returnPath={expenseReturnPath} canRecordPayments={canRecordPayments} canReversePayments={canReversePayments} financialFxSnapshots={financialFxSnapshots} onNavigatePath={onNavigatePath} />
        : null
    : null;
...
function ExpenseDetailPanel({ expense, invoice, project, returnPath, canRecordPayments, canReversePayments, financialFxSnapshots, onNavigatePath }: ExpenseDetailPanelProps) {
  ...
  return <section className="space-y-3 rounded-2xl border border-indigo-100 bg-white p-4 shadow-sm sm:p-5" aria-label="Expense detail" data-testid="expense-detail-panel">
    ...
    <FinancialSettlementCard
      ...
      recordPaymentPath={appPathForCashTarget("EXPENSE", expense.id)}
