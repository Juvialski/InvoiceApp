...
export function CompanyDocumentProfileSettings() {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="company-document-profile-title" aria-busy={busy}>
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-50 text-sky-700"><FileSignature className="h-5 w-5" /></div><div><p id="company-document-profile-title" className="text-sm font-black text-slate-950">Company document profile</p><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">Complete the deployment company&apos;s approved legal identity before issuing documents or posting supplier payables. Product branding is not used as a client legal default.</p></div></div>{!canManage && <span className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-[10px] font-bold text-slate-500"><LockKeyhole className="h-3 w-3" />Read-only</span>}</div>
    {!draft.legalName.trim() && <p role="status" className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">Document identity is incomplete. Buyer validation and issued-document workflows remain blocked until an authorized administrator completes the legal name.</p>}
