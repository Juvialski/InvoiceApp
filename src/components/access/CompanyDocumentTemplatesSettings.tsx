...
export function CompanyDocumentTemplatesSettings({ demoMode = false }: CompanyDocumentTemplatesSettingsProps) {
  ...
  const upload = async (file: File) => {
    ...
    try {
      ...
      setSelectedVersion(version); setBindings([]); setAnalysis(null); setMappingSelections({}); setNotice({ tone: "info", text: "The original DOCX was preserved as a draft. Review the detected mappings, then prepare a new tagged draft inside HydroQualiSense." });
      await refresh();
