...
function statusText(config: CompanyAiConfigMetadata) {
  ...
  if (config.status === "INVALID") return "Initial credential invalid; replace it to complete deployment bootstrap";
...
export function DeploymentAiBootstrapSettings() {
  ...
  const needsBootstrapCredential = !config?.credentialConfigured || config.status === "INVALID";
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="deployment-ai-bootstrap-title" aria-busy={busy}>
    ...
    {!needsBootstrapCredential && <p className="mt-4 text-[10px] leading-4 text-slate-500">The one-time bootstrap refuses replacement after configuration. Credential rotation, disablement, and removal remain platform maintenance operations.</p>}
  </section>;
}
