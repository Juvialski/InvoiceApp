...
export function NoCompanyAccess({ onSignOut, children }: { onSignOut?: () => void | Promise<void>; children?: ReactNode }) {
  return <main className="flex min-h-[70vh] items-center justify-center px-4 py-10"><section role="alert" className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_60px_-32px_rgba(15,23,42,0.42)] sm:p-8"><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-700"><Building2 className="h-5 w-5" /></div><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-indigo-600">Deployment access</p><h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950">Company access unavailable</h1><p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">This email has not been authorized for this {BRAND.productName} deployment, or its existing access is inactive. Contact your company administrator.</p></div></div><div className="mt-6 flex items-start gap-2.5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-950"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-amber-700" /><span>{BRAND.productName} only opens company data after authenticated membership resolution. It does not select or switch between unrelated client companies inside one deployment.</span></div>{children}{onSignOut && <button type="button" onClick={() => void onSignOut()} className="mt-6 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50">Sign out</button>}</section></main>;
}
