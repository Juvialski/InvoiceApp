    ...
    <div className="border-t border-indigo-100 bg-indigo-50/40 px-3 py-4 sm:px-4" data-permission-editor={member.id}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-[10px] leading-5 text-slate-600">Role defaults remain the baseline. Custom grants add access and custom denies remove a role default. Reserved administration permissions stay role-controlled.</p>
        </div>
        <span className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[10px] font-bold text-slate-600">{effectiveCount} effective permission{effectiveCount === 1 ? "" : "s"}</span>
      </div>

      {member.status !== "ACTIVE" && <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] leading-5 text-amber-900">This membership is {member.status.toLowerCase()}; all effective access remains denied until it is active.</div>}

      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        {groups.map(([group, entries]) => (
          <section key={group} className="rounded-xl border border-slate-200 bg-white p-3" aria-labelledby={`permission-group-${member.id}-${group}`}>
            <div className="mb-2 flex items-center justify-between gap-2">
              <h4 id={`permission-group-${member.id}-${group}`} className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">{permissionGroupDisplayName(group)}</h4>
              <span className="text-[9px] font-semibold text-slate-400">{entries.length} option{entries.length === 1 ? "" : "s"}</span>
            </div>
            <div className="space-y-2">
              {entries.map((entry) => {
                ...
                return (
                  <div key={entry.permissionKey} className={`rounded-lg border px-2.5 py-2 ${isSensitivePermission(entry.permissionKey) ? "border-amber-200 bg-amber-50/45" : "border-slate-100 bg-slate-50/50"}`}>
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0">
                        <p className="text-[11px] font-bold text-slate-800">{permissionDisplayName(entry.permissionKey)}</p>
                        <p className="truncate text-[9px] text-slate-400" title={entry.permissionKey}>{entry.permissionKey}</p>
                        {entry.description && <p className="mt-1 text-[9px] leading-4 text-slate-500">{entry.description}</p>}
                      </div>
                      <div className="flex shrink-0 items-center gap-2">
                        <span className={`text-[9px] font-black ${effective ? "text-emerald-700" : "text-slate-400"}`}>{effective ? "Effective" : "No access"}</span>
                        {entry.memberAssignable ? <select aria-label={`${permissionDisplayName(entry.permissionKey)} override`} value={effect || ""} onChange={(event) => onChange(entry.permissionKey, (event.target.value || undefined) as PermissionOverrideEffect | undefined)} disabled={saving} className="min-h-9 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[10px] font-bold text-slate-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:cursor-not-allowed disabled:opacity-60">
                          <option value="">Role default</option>
                          <option value="GRANT">Custom grant</option>
                          <option value="DENY">Custom deny</option>
                        </select> : <span className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[9px] font-bold text-slate-400">Role-controlled</span>}
                      </div>
                    </div>
                  </div>
                );
...
export function DeploymentAccessManagement() {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5" aria-labelledby="company-access-title">
    ...
    <div className="mt-5 overflow-hidden rounded-xl border border-slate-200">
      <div className="border-b border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-slate-500">Members and effective permissions</div>
      {loading ? <div className="flex items-center gap-2 px-3 py-5 text-xs text-slate-500"><Loader2 className="h-4 w-4 animate-spin" />Loading access records…</div> : members.length === 0 ? <p className="px-3 py-5 text-xs text-slate-500">No member records were returned.</p> : <div className="divide-y divide-slate-100">{members.map((member) => {
        ...
        return <div key={member.id || member.userId || member.email}>
          <div className="grid gap-3 px-3 py-3 sm:grid-cols-[minmax(0,1fr)_10rem_auto] sm:items-start">
            <div className="min-w-0"><p className="truncate text-xs font-bold text-slate-900">{member.displayName || member.email || "Member"}{isSelf && <span className="ml-1.5 rounded bg-indigo-50 px-1.5 py-0.5 text-[9px] font-black text-indigo-700">You</span>}</p><p className="truncate text-[10px] text-slate-500">{member.email || member.userId || ""} · {member.status}</p><p className="mt-1 text-[10px] leading-4 text-slate-500"><span className="font-bold text-slate-600">Effective:</span> {effective.length ? effective.slice(0, 4).map(permissionDisplayName).join(", ") : "No active access"}{effective.length > 4 ? ` +${effective.length - 4} more` : ""}</p></div>
            <div className="min-w-0">{canManage && member.id && !isSelf ? <select value={member.roleKey || "VIEWER"} disabled={Boolean(busy)} onChange={(event) => void updateMember(member, { roleKey: event.target.value })} className="w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-[10px] font-semibold">{ASSIGNABLE_ROLES.map((role) => <option key={role} value={role}>{roleDisplayName(role)}</option>)}</select> : <span className="text-xs font-semibold text-slate-700">{roleDisplayName(member.roleKey)}</span>}</div>
            <div className="flex flex-wrap gap-1.5 sm:justify-end">{canManage && member.id && !isSelf ? <><button type="button" disabled={Boolean(busy)} onClick={() => toggleEditor(member)} aria-expanded={editorOpen} className="inline-flex items-center gap-1 rounded-lg border border-indigo-200 px-2 py-1.5 text-[10px] font-bold text-indigo-800 hover:bg-indigo-50 disabled:opacity-50">{editorOpen ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}Permissions</button>{member.status === "ACTIVE" ? <button type="button" disabled={Boolean(busy)} onClick={() => void updateMember(member, { status: "SUSPENDED" })} className="rounded-lg border border-amber-200 px-2 py-1.5 text-[10px] font-bold text-amber-800">Suspend</button> : <button type="button" disabled={Boolean(busy)} onClick={() => void updateMember(member, { status: "ACTIVE" })} className="rounded-lg border border-emerald-200 px-2 py-1.5 text-[10px] font-bold text-emerald-800">Reactivate</button>}<button type="button" disabled={Boolean(busy)} onClick={() => void updateMember(member, { status: "REVOKED" })} className="rounded-lg border border-rose-200 px-2 py-1.5 text-[10px] font-bold text-rose-800">Revoke</button></> : <span className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-[10px] font-semibold text-slate-500"><ShieldCheck className="h-3 w-3" />Own access protected</span>}</div>
          </div>
    ...
    {pendingInvitations.length > 0 && <div className="mt-4 rounded-xl border border-slate-200 p-3"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-slate-500"><MailCheck className="h-3.5 w-3.5" />Pending invitations</div><div className="mt-2 space-y-2">{pendingInvitations.map((invitation) => <div key={invitation.id || `${invitation.email}:${invitation.roleKey}`} className="flex flex-col gap-2 rounded-lg border border-slate-100 bg-slate-50/60 p-2.5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate text-xs font-semibold text-slate-800">{invitation.email}</p><p className="mt-1 text-[10px] text-slate-500">{roleDisplayName(invitation.roleKey)}{invitation.expiresAt ? ` · expires ${displayTime(invitation.expiresAt)}` : ""}</p></div><div className="flex flex-wrap items-center gap-1.5"><span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-black ${deliveryTone(invitation)}`}>{invitation.deliveryStatus === "FAILED" ? <MailWarning className="h-3 w-3" /> : <MailCheck className="h-3 w-3" />}{deliveryLabel(invitation)}</span>{canManage && invitation.id && <><button type="button" disabled={Boolean(busy)} onClick={() => void resendInvitation(invitation)} className="rounded-lg border border-indigo-200 bg-white px-2 py-1.5 text-[10px] font-bold text-indigo-800 disabled:opacity-50">{busy === `resend:${invitation.id}` ? "Sending…" : "Resend"}</button><button type="button" disabled={Boolean(busy)} onClick={() => void revokeInvitation(invitation)} className="rounded-lg border border-rose-200 bg-white px-2 py-1.5 text-[10px] font-bold text-rose-800 disabled:opacity-50">{busy === `revoke:${invitation.id}` ? "Revoking…" : "Revoke"}</button></>}</div></div>)}</div></div>}

    {invitations.filter((item) => item.status !== "PENDING").length > 0 && <div className="mt-4 rounded-xl border border-slate-200 p-3"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-slate-500"><ShieldCheck className="h-3.5 w-3.5" />Invitation history</div><div className="mt-2 space-y-2">{invitations.filter((item) => item.status !== "PENDING").slice(0, 12).map((invitation) => <div key={invitation.id || `${invitation.email}:${invitation.createdAt}`} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-100 px-2.5 py-2 text-xs"><span className="min-w-0 truncate font-semibold text-slate-700">{invitation.email || "Invitation"}</span><div className="flex flex-wrap items-center gap-1.5"><span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-black ${deliveryTone(invitation)}`}>{deliveryLabel(invitation)}</span>{canManage && invitation.id && invitation.status !== "ACCEPTED" && <button type="button" disabled={Boolean(busy)} onClick={() => void resendInvitation(invitation)} className="rounded-lg border border-indigo-200 bg-white px-2 py-1.5 text-[10px] font-bold text-indigo-800 disabled:opacity-50">{busy === `resend:${invitation.id}` ? "Sending…" : "Resend"}</button>}</div></div>)}</div></div>}
  </section>;
}
