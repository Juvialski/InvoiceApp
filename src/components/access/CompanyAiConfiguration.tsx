...
export function CompanyAiConfiguration({ config, loading = false, onSaveKey, onTest, onDisable, onEnable, onRemove }: CompanyAiConfigurationProps) {
  ...
  return <div role="tabpanel" aria-label="AI Configuration">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><h3 className="text-sm font-black text-slate-950">AI Configuration</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">Configure the company’s Gemini credential for invoice extraction, email classification, InvoiceApp Assistant, and document processing.</p></div>
    ...
    {loading ? <div className="mt-5 flex items-center gap-2 rounded-xl border border-dashed border-slate-200 px-3 py-5 text-xs font-semibold text-slate-500"><Loader2 className="h-4 w-4 animate-spin text-indigo-600" />Loading AI configuration…</div> : <>
      ...
      <div className="mt-4 rounded-xl border border-slate-100 bg-white p-3 text-[11px] leading-5 text-slate-600"><p className="font-bold text-slate-800">Used for</p><p className="mt-1">Invoice extraction · email classification · InvoiceApp Assistant · AI document processing</p><p className="mt-2">Last tested: <span className="font-semibold">{config?.lastTestedAt ? new Date(config.lastTestedAt).toLocaleString() : "Not tested"}</span> · {testStatusLabel(config?.lastTestStatus)}</p></div>
