...
import type { CashSettlementTargetContext } from "../utils/appRouting.ts";
...
export const CashBankingPage: React.FC<CashBankingPageProps> = ({
  ...
  return <CashModalBusyContext.Provider value={busy !== null}><div className="space-y-5">
    ...
    {targetContext?.requested && <section className={`rounded-xl border p-3 ${targetContext.invalid || !targetCandidate || (targetCandidate && !isFinancialReconciliationCandidateLifecycleEligible(targetCandidate)) ? "border-amber-200 bg-amber-50" : "border-indigo-200 bg-indigo-50"}`} aria-label="Cash payment target" role={targetContext.invalid || !targetCandidate ? "alert" : undefined} data-testid="cash-page-target-context"><p className="break-words text-xs font-black text-slate-950">Payment target: {targetCandidate?.label || `${targetContext.targetType || "Settlement"} ${targetContext.targetId || ""}`}</p>{targetContextMessage && <p className="mt-1 break-words text-[10px] leading-4 text-slate-700">{targetContextMessage}</p>}</section>}
