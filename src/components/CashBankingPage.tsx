...
import {
  ...
  isFinancialReconciliationCandidateLifecycleEligible,
...
import { appPathForCashTransaction, type CashSettlementTargetContext } from "../utils/appRouting.ts";
...
export interface CashBankingPageProps {
  ...
  targetContext?: CashSettlementTargetContext;
  ...
  primarySettlementWorkspace?: React.ReactNode;
}

type Modal = "account" | "balance" | "transaction" | "transaction-edit" | "import" | null;

const CashModalBusyContext = React.createContext(false);
...
export const CashBankingPage: React.FC<CashBankingPageProps> = ({
  ...
  const routeTransaction = selectedTransactionId ? data.transactions.find((transaction) => transaction.id === selectedTransactionId) : undefined;
  const routeAccount = routeTransaction ? data.accounts.find((account) => account.id === routeTransaction.accountId) : undefined;
  const selectedAccount = routeAccount || data.accounts.find((account) => account.id === selectedAccountId && account.active && account.currency.toUpperCase() === currency.toUpperCase()) || position.accounts[0]?.account || data.accounts.find((account) => account.active);
  const targetCandidate = targetContext?.targetType && targetContext.targetId
    ? reconciliationCandidates.find((candidate) => candidate.targetType === targetContext.targetType && candidate.targetId === targetContext.targetId)
    : undefined;
  useEffect(() => {
    if (!selectedTransactionId || !routeTransaction) return;
    setSelectedAccountId(routeTransaction.accountId);
    if (!controlledCurrency) setLocalCurrency(routeTransaction.currency.toUpperCase());
    const timer = typeof window === "undefined" ? undefined : window.setTimeout(() => document.getElementById(`cash-transaction-${routeTransaction.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 0);
    return () => { if (timer !== undefined) window.clearTimeout(timer); };
  }, [controlledCurrency, routeTransaction, selectedTransactionId]);
  ...
  const reviewableTransactions = useMemo(() => eligibleSettlementReviewTransactions(accountTransactions, data.matches), [accountTransactions, data.matches]);
  const unresolved = reviewableTransactions.filter((transaction) => transaction.reconciliationStatus !== "MATCHED");
  ...
  const targetContextMessage = !targetContext?.requested
    ...
    : targetContext.invalid
      ? "The payment target link is incomplete or unsupported. Choose a target from the current Cash & Banking workspace."
      : workspaceDataPending
        ? "Resolving the exact target and its current settlement eligibility…"
        : !targetCandidate
          ? "The requested target is unavailable in this company workspace. No fallback payment target was selected."
          : !isFinancialReconciliationCandidateLifecycleEligible(targetCandidate)
            ? `This target is ${targetCandidate.lifecycleStatus || "not in an eligible lifecycle"}; settlement validation remains authoritative.`
            : targetOutstanding !== undefined && targetOutstanding <= 0.005
              ? "This target is already fully settled from confirmed evidence."
              : "The exact target is carried into reconciliation below. No cash evidence is created automatically.";
  const reviewTransactionPath = (transactionId: string) => appPathForCashTransaction(transactionId, targetContext?.targetType, targetContext?.targetId, targetContext?.returnTo);

  return <CashModalBusyContext.Provider value={busy !== null}><div className="space-y-5">
    <PageHeader eyebrow="Financial operations" title="Cash & Banking" description="Review cash records and confirm reconciliations; balances are not a live bank connection." actions={<div className="flex flex-wrap gap-2">{onOpenDashboard && <button type="button" onClick={onOpenDashboard} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-black text-slate-700 hover:bg-slate-50">Executive Dashboard</button>}{canImport && data.accounts.length > 0 && <button type="button" onClick={() => { setImportError(""); setStatementDocument(null); setPreview(null); setModal("import"); }} className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-[10px] font-black text-indigo-800 hover:bg-indigo-100"><Upload className="h-3.5 w-3.5" /> Import statement</button>}{canManageAccounts && <button type="button" onClick={() => openAccountForm()} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-black text-white shadow-sm hover:bg-indigo-700"><Plus className="h-3.5 w-3.5" /> Add account</button>}</div>} />
    {targetContext?.requested && <section className={`rounded-xl border p-3 ${targetContext.invalid || !targetCandidate || (targetCandidate && !isFinancialReconciliationCandidateLifecycleEligible(targetCandidate)) ? "border-amber-200 bg-amber-50" : "border-indigo-200 bg-indigo-50"}`} aria-label="Cash source context" role={targetContext.invalid || !targetCandidate ? "alert" : undefined} data-testid="cash-page-target-context"><p className="break-words text-xs font-black text-slate-950">Source context: {targetCandidate?.label || `${targetContext.targetType || "Settlement"} ${targetContext.targetId || ""}`}</p>{targetContextMessage && <p className="mt-1 break-words text-[10px] leading-4 text-slate-700">{targetContextMessage}</p>}</section>}
    ...
    <section className="flex flex-col justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-end" aria-label="Cash controls"><div><SectionHeader title="Position controls" description="Selected-currency totals; no implicit FX." icon={WalletCards} /></div><label className="min-w-[10rem] space-y-1"><span className="field-label">Currency</span><select className="field-input" value={currency} onChange={(event) => setCurrency(event.target.value)}>{(currencies.length ? currencies : [currency]).map((item) => <option key={item} value={item}>{item}</option>)}</select></label></section>
    {!position.hasAccounts ? (workspaceDataPending ? <div className="p-8 text-center text-xs font-semibold text-slate-500">Loading cash accounts…</div> : <EmptyState
      title={targetContext?.requested ? "Payment account required" : data.accounts.length ? "No active cash accounts" : "No cash accounts yet"}
      description={targetContext?.requested
        ? canManageAccounts
          ? "This payment flow is ready to continue, but the company has no active financial account. Add a bank, GCash / e-wallet, or permitted cash-on-hand account; the selected payment target will remain in context."
          : "This payment flow is paused because the company has no active financial account. Ask a user with Cash account management permission to add one; no payment or paid state will be created without settlement evidence."
        : "Add a bank or GCash account to see your company’s cash position. The first release uses manual balances and statement imports; it does not pretend to be a live bank connection."}
      icon={Landmark}
      action={canManageAccounts && <button type="button" onClick={() => openAccountForm()} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-black text-white hover:bg-indigo-700"><Plus className="h-3.5 w-3.5" /> {targetContext?.requested ? "Add payment account" : "Add account"}</button>}
    />) : <>
  ...
  </div></CashModalBusyContext.Provider>;
};
...
function ModalShell({ title, children, onClose, wide = false }: { title: string; children: React.ReactNode; onClose: () => void; wide?: boolean }) {
  const modalBusy = React.useContext(CashModalBusyContext);
  const closeButtonRef = useRef<HTMLButtonElement>(null);
  const close = () => { if (!modalBusy) onClose(); };
  const dialogRef = useDialogFocus({ open: true, onClose: close, initialFocusRef: closeButtonRef });
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/45 p-2 sm:items-center sm:p-6" role="presentation"><section ref={dialogRef} role="dialog" aria-modal="true" aria-label={title} className={`max-h-[min(92vh,54rem)] min-h-0 w-full overflow-y-auto overscroll-contain rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl sm:p-6 ${wide ? "max-w-5xl" : "max-w-2xl"}`}><div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-4"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-600">Cash &amp; Banking</p><h2 className="mt-1 break-words text-lg font-black text-slate-950">{title}</h2></div><button ref={closeButtonRef} type="button" disabled={modalBusy} aria-disabled={modalBusy} onClick={close} className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-50" aria-label={modalBusy ? "Close dialog unavailable while the operation is in progress" : "Close dialog"}><X className="h-4 w-4" /></button></div><div className="pt-4">{children}</div></section></div>;
}
