...
export const CashBankingPage: React.FC<CashBankingPageProps> = ({
  ...
  const saveAccount = async () => {
    ...
    try {
      ...
      setSelectedAccountId(savedAccount?.id || account.id);
      setModal(null);
  ...
  const buildPreview = () => {
    if (!document || !importAccountId) {
      setImportError("Choose an account and statement file first.");
      return;
    }
    const account = data.accounts.find((item) => item.id === importAccountId);
    if (!account) return;
    setPreview(buildStatementPreview(document, mapping, account.id, account.currency, data.transactions, data.importBatches.filter((batch) => batch.accountId === account.id).map((batch) => batch.fileFingerprint)));
    setImportError("");
  };
  ...
  return <div className="space-y-5">
    <PageHeader eyebrow="Financial operations" title="Cash & Banking" description="Company-scoped liquidity, statement-ledger imports, and confirmation-based reconciliation. Manual and statement balances are never presented as a live bank connection." actions={<div className="flex flex-wrap gap-2">{onOpenDashboard && <button type="button" onClick={onOpenDashboard} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-black text-slate-700 hover:bg-slate-50">Executive Dashboard</button>}{canImport && data.accounts.length > 0 && <button type="button" onClick={() => { setImportError(""); setDocument(null); setPreview(null); setModal("import"); }} className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-[10px] font-black text-indigo-800 hover:bg-indigo-100"><Upload className="h-3.5 w-3.5" /> Import statement</button>}{canManageAccounts && <button type="button" onClick={openAccountForm} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-black text-white shadow-sm hover:bg-indigo-700"><Plus className="h-3.5 w-3.5" /> Add account</button>}</div>} />
