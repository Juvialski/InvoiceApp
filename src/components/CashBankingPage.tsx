...
import { ActionButton, EmptyState, MetricCard, Notice, PageActionBar, PageHeader, SectionHeader, StatusBadge } from "./ui/OperationsUI.tsx";
...
export const CashBankingPage: React.FC<CashBankingPageProps> = ({
  ...
  return <CashModalBusyContext.Provider value={busy !== null}><div className="space-y-5">
    <PageHeader eyebrow="Financial operations" title="Cash & Banking" description="Review cash records and confirm reconciliations; balances are not a live bank connection." actions={<PageActionBar>{onOpenDashboard && <ActionButton size="sm" variant="ghost" label="Executive Dashboard" onClick={onOpenDashboard} />}{canImport && data.accounts.length > 0 && <ActionButton size="sm" variant="secondary" icon={<Upload aria-hidden="true" className="h-3.5 w-3.5" />} label="Import statement" onClick={() => { setImportError(""); setStatementDocument(null); setPreview(null); setModal("import"); }} />}{canManageAccounts && <ActionButton size="sm" variant="primary" icon={<Plus aria-hidden="true" className="h-3.5 w-3.5" />} label="Add account" onClick={() => openAccountForm()} />}</PageActionBar>} />
    ...
    {!position.hasAccounts ? (workspaceDataPending ? <div className="p-8 text-center text-xs font-semibold text-slate-500">Loading cash accounts…</div> : <EmptyState
      ...
      icon={Landmark}
      action={canManageAccounts && <ActionButton size="sm" variant="primary" icon={<Plus aria-hidden="true" className="h-3.5 w-3.5" />} label={targetContext?.requested ? "Add payment account" : "Add account"} onClick={() => openAccountForm()} />}
    />) : <>
