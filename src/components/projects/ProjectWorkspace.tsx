...
export const ProjectWorkspace: React.FC<ProjectWorkspaceProps> = ({ project, summary, dashboard, invoices, invoiceAllocations, expenses, workers = [], assignments = [], payrollAllocations = [], payrollPeriods = [], initialTab = "overview", onTabChange, onBack, onOpenInvoice, onUploadInvoice, onEditProject, onArchiveProject, onAddExpense, onOpenPayroll, onSaveInvoiceAllocations }) => {
  ...
  return <div className="space-y-5">
    ...
    {tab !== "overview" && <PageHeader eyebrow={project.projectCode} title={project.projectName} description="Project workspace sections keep supplier, labor, and expense records in one operational context." actions={<button type="button" onClick={onBack} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">← Projects</button>} />}
    <nav className="flex gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1" aria-label="Project workspace sections">{tabs.map(([id, label, Icon]) => <button key={id} type="button" onClick={() => selectTab(id)} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-bold whitespace-nowrap ${tab === id ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}><Icon className="h-3.5 w-3.5" />{label}</button>)}</nav>
