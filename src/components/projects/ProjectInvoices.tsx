...
export const ProjectInvoices: React.FC<ProjectInvoicesProps> = ({ project, invoices, allocations, onOpenInvoice, onUploadInvoice, onSaveAllocations }) => {
  ...
  return <>
    ...
    {editingInvoice && <AllocationEditor project={project} invoice={editingInvoice} allocations={allocations.filter((allocation) => allocation.invoiceId === editingInvoice.id)} onSave={(nextAllocations) => onSaveAllocations(editingInvoice, nextAllocations)} onClose={() => setEditingInvoice(null)} />}
  </>;
};
