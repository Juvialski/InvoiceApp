...
const AllocationEditor: React.FC<{
  ...
  onClose: () => void;
}> = ({ project, invoice, allocations, costCodes = [], onSave, onClose }) => {
...
export const ProjectInvoices: React.FC<ProjectInvoicesProps> = ({ project, invoices, allocations, costCodes = [], onOpenInvoice, onUploadInvoice, onSaveAllocations, settlementProjections, today }) => {
  ...
  const linkedRows = useMemo<InvoiceRow[]>(() => invoices.map((invoice) => {
    ...
    return { invoice, paymentState, linked, currentAmount, allocatedAmount, remainingAmount, state };
  }).filter((row): row is InvoiceRow => Boolean(row)), [invoices, allocations, project.id, settlementProjections, today]);
  ...
  const filteredRows = useMemo(() => linkedRows.filter(({ invoice, paymentState, state }) => {
    const q = normalizeProjectText(query);
    const haystack = normalizeProjectText([invoice.invoiceNumber, invoice.projectReference, invoice.invoiceDate, partyName(invoice), paymentState, invoice.reviewStatus].filter(Boolean).join(" "));
  ...
  }), [linkedRows, query, paymentFilter, allocationFilter, currencyFilter, project.currency, settlementProjections, today]);
