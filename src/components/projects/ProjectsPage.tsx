...
import { Archive, BriefcaseBusiness, ChevronRight, Filter, Pencil, Plus, Search, X } from "lucide-react";
...
export const ProjectsPage: React.FC<ProjectsPageProps> = ({
  ...
  useEffect(() => {
    if (initialEditingProject) setEditing(initialEditingProject);
  }, [initialEditingProject]);
  ...
  const filtered = useMemo(
    () =>
      projects.filter(
        (project) =>
          projectSearchMatches(project, query) &&
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        title="Projects"
        description="Projects are the cost context for supplier invoices, labor, and direct expenses."
        actions={
          <Button
            variant="primary"
            label="New project"
      ...
      <Card className="p-3" elevation="low" aria-label="Project filters">
        <div className="flex flex-col gap-2 md:flex-row">
          <label className="flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
            <Search aria-hidden="true" className="h-4 w-4 text-slate-400" />
            <span className="sr-only">Search projects</span>
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search code, project, client, location, manager…"
          ...
          <label className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2.5">
            ...
            >
              ...
              {["PLANNING", "ACTIVE", "ON_HOLD", "COMPLETED", "CANCELLED", "ARCHIVED"].map((value) => (
      ...
      {filtered.length ? (
        <Card className="overflow-hidden p-0" elevation="low" aria-label="Projects table">
          <div className="ops-scrollbar overflow-auto">
            <table className="ops-table min-w-[900px] w-full text-left text-xs">
                ...
                {filtered.map((project) => {
                  const summary =
                    ...
                    ({
                      ...
                      totalActualCost: 0,
                      remainingBudget: project.projectBudget,
                      budgetUsedPercent: 0,
                    ...
                    <tr key={project.id}>
                      <td className="px-4 py-3">
                        ...
                        >
                          ...
                          <strong className="mt-0.5 block text-xs text-slate-900">{project.projectName}</strong>
                        </button>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right font-sans font-bold tabular-nums">
                        {money(summary.budget, project.currency)}
                      </td>
                      <td className="px-4 py-3 text-right font-sans font-bold tabular-nums">
                        {money(summary.totalActualCost, project.currency)}
                      </td>
                      <td
                        className={`px-4 py-3 text-right font-sans font-bold tabular-nums ${
                          summary.remainingBudget < 0 ? "text-rose-700" : "text-emerald-700"
                        }`}
                      >
                        {money(summary.remainingBudget, project.currency)}
                      </td>
                      <td className="px-4 py-3">
        ...
        <EmptyState
          ...
          description="Create a project to connect supplier invoices, payroll, and direct costs."
          action={
            <Button
              variant="primary"
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 id="project-form-title" className="mt-1 text-lg font-black text-slate-950">
                  {projects.some((project) => project.id === editing.id) ? "Edit project" : "New project"}
                </h2>
                <p className="mt-1 text-xs text-slate-500">Codes are unique within the workspace and remain searchable.</p>
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {(
                [
                  ...
                  ["currency", "Currency"],
                ] as Array<[keyof Project, string]>
              ).map(([key, label]) => (
                <label key={String(key)} className="space-y-1">
                  <span className="field-label">{label}</span>
                  <input
                    required={key === "projectCode" || key === "projectName"}
              ...
              <label className="space-y-1">
                ...
                <input
                  ...
                  onChange={(event) => setEditing({ ...editing, projectBudget: Number(event.target.value) })}
                  className="field-input"
                />
              </label>
              <label className="space-y-1">
              ...
              <label className="space-y-1 sm:col-span-2">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setEditing({ ...editing, description: event.target.value, notes: event.target.value })
                  }
                  rows={3}
