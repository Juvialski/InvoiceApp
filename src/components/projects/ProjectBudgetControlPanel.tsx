    ...
    <div className="hidden md:block" aria-label="Project cost codes table">
      <OperationsGrid
        ...
        columns={[
          ...
          { key: "committed", header: "Committed Cost", align: "right" as const, sortValue: (row) => row.committedCost ?? -Infinity, protected: true, cellClassName: "font-black tabular-nums text-purple-700", value: (row) => row.hasForeignAmounts ? <span className="font-semibold italic text-slate-400">Partial</span> : money(row.committedCost || 0, currency) },
