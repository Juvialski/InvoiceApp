...
function RegisterModal({
  ...
  const compatibleInventoryItems = inventoryItems.filter((item) => item.status === "ACTIVE" && item.stockUnit.trim().toLowerCase() === materialValue.unit.trim().toLowerCase());
          ...
          <div className="mt-5 space-y-4">
            <section className={sectionClass}>
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <label className="block sm:col-span-2"><span className={labelClass}>Canonical warehouse item (optional)</span><select className={inputClass} value={materialValue.inventoryItemId || ""} onChange={(event) => setMaterialValue((value) => ({ ...value, inventoryItemId: event.target.value || null }))}><option value="">No warehouse item linked</option>{compatibleInventoryItems.map((item) => <option key={item.id} value={item.id}>{item.itemName} · {item.stockUnit}{item.itemCode ? ` · ${item.itemCode}` : ""}</option>)}</select><span className="mt-1 block text-[10px] leading-4 text-slate-500">Only an exact stock-unit match can be linked. Planned quantity remains separate from warehouse on-hand.</span>{materialValue.inventoryItemId && !compatibleInventoryItems.some((item) => item.id === materialValue.inventoryItemId) && <span className="mt-1 block text-[10px] leading-4 text-amber-700">The current warehouse item is unavailable or uses a different unit; resolve the link before changing it.</span>}</label>
  ...
  return (
    <section className="space-y-4" data-phase3b="materials-equipment">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-start sm:justify-between sm:p-5">
        ...
        {(canManage || onOpenWarehouse) && <div className="flex shrink-0 flex-wrap gap-2">{canManage && <button type="button" onClick={() => setModal({ kind: view === "materials" ? "material" : "equipment" })} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-black text-white"><Plus className="h-3.5 w-3.5" />Add {view === "materials" ? "material" : "equipment"}</button>}{onOpenSiteLogs && canManage && <button type="button" onClick={onOpenSiteLogs} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700"><ClipboardList className="h-3.5 w-3.5" />Daily Site Logs</button>}{onOpenWarehouse && <button type="button" onClick={onOpenWarehouse} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-black text-indigo-800"><WarehouseIcon className="h-3.5 w-3.5" />Warehouse Inventory</button>}</div>}
      </div>
