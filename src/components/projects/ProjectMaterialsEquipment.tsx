...
export const ProjectMaterialsEquipment: React.FC<ProjectMaterialsEquipmentProps> = ({
  ...
  const canonicalEquipmentIds = useMemo(() => canonicalEquipment.map((item) => item.id), [canonicalEquipment]);
  const { byEntityId: mediaByEquipmentId } = useEntityMediaThumbnails("EQUIPMENT", canonicalEquipmentIds, canReadEquipment);
  ...
  return (
    <section className="space-y-4" data-phase3b="materials-equipment">
      ...
      {view === "equipment" && <section className="rounded-2xl border border-orange-100 bg-orange-50/60 p-4"><div className="flex items-start gap-2"><Cog className="mt-0.5 h-4 w-4 shrink-0 text-orange-700" /><div><p className="text-xs font-black text-orange-950">Formal Equipment assigned here</p><p className="mt-1 text-[10px] leading-4 text-orange-900">These rows reference the company Equipment Registry. This Project is the current operational context, not the owner of the canonical asset.</p></div></div>{formalEquipment.length ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{formalEquipment.map((item) => <div key={item.id} className="flex min-w-0 items-center gap-3 rounded-xl border border-orange-100 bg-white p-3"><EntityMediaThumbnail media={mediaByEquipmentId[item.id]} label="Equipment" alt="Canonical Equipment asset image" className="h-12 w-16 aspect-[4/3]" fallback={<Cog className="h-4 w-4" aria-hidden="true" />} /><div className="min-w-0"><p className="text-xs font-black text-slate-900">{item.assetReference || "No asset code"} · {item.equipmentName}</p><p className="mt-1 text-[10px] text-slate-600">{item.equipmentType || "Type not recorded"} · {item.equipmentSource} · Assigned {item.currentAssignmentStart || "date unavailable"}</p><p className="mt-1 text-[10px] font-semibold text-orange-800">Canonical Registry identity · assignment history preserved</p></div></div>)}</div> : <p className="mt-3 text-[10px] text-orange-900">No canonical Equipment is formally assigned to this Project. Existing project-register rows below remain preserved as historical/read-through data.</p>}</section>}
