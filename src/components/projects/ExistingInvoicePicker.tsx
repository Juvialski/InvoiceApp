...
import { supplierInvoicePaymentStateFor, type SupplierInvoicePaymentDisplayState, type SupplierInvoiceSettlementProjection } from "../../lib/supplierInvoiceSettlement.ts";
...
function AllocationCard({
  ...
  selectableCodes: ProjectCostCode[];
  onToggle: () => void;
  onCostCodeChange: (value: string) => void;
  onAmountChange: (value: number) => void;
}) {
  const reviewLabel = row.invoice.reviewStatus === "VERIFIED" ? "Verified" : "Needs review";
  const paymentLabel = row.paymentState.replaceAll("_", " ").toLowerCase().replace(/(^|\s)\S/g, (letter) => letter.toUpperCase());
  return (
    <article className={selected ? "rounded-xl border border-indigo-300 bg-indigo-50/60 p-4 shadow-sm" : "rounded-xl border border-slate-200 bg-white p-4 shadow-sm"} data-project-allocation-row={row.invoice.id}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-[0.12em] text-indigo-600">{project.projectCode} allocation</p>
          <h3 className="mt-1 break-words text-sm font-black text-slate-950">{row.invoice.invoiceNumber || "Supplier invoice"}</h3>
          <p className="mt-1 break-words text-xs text-slate-600">{vendorName(row.invoice)} · {row.invoice.invoiceDate || "Date not recorded"}</p>
        </div>
        <button type="button" onClick={onToggle} disabled={row.remainingAmount <= 0.009} className={selected ? "inline-flex min-h-10 shrink-0 items-center rounded-lg bg-indigo-600 px-3 py-2 text-xs font-black text-white" : "inline-flex min-h-10 shrink-0 items-center rounded-lg border border-indigo-200 bg-white px-3 py-2 text-xs font-black text-indigo-700 disabled:opacity-45"}>
          {selected ? "Selected" : "Select"}
        </button>
      </div>
      <div className="mt-4 grid grid-cols-3 gap-2" aria-label="Allocation amounts">
        <div className="min-w-0 rounded-lg border border-slate-200 bg-white p-2.5"><p className="text-[10px] font-bold text-slate-500">Invoice total</p><p className="mt-1 break-words text-sm font-black tabular-nums text-slate-900">{money(row.invoice.grandTotal, row.invoice.currency || project.currency)}</p></div>
        <div className="min-w-0 rounded-lg border border-slate-200 bg-white p-2.5"><p className="text-[10px] font-bold text-slate-500">Allocated</p><p className="mt-1 break-words text-sm font-black tabular-nums text-slate-700">{money(row.allocatedAmount, row.invoice.currency || project.currency)}</p></div>
        <div className="min-w-0 rounded-lg border border-emerald-200 bg-emerald-50 p-2.5"><p className="text-[10px] font-bold text-emerald-800">Remaining</p><p className="mt-1 break-words text-sm font-black tabular-nums text-emerald-900">{money(row.remainingAmount, row.invoice.currency || project.currency)}</p></div>
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Project</p><p className="mt-1 break-words text-xs font-black text-slate-900">{project.projectCode} — {project.projectName}</p><p className="mt-1 text-[10px] text-slate-500">Allocation destination</p></div>
        <label className="block rounded-lg border border-slate-200 bg-white p-3"><span className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Cost code</span><select aria-label={"Cost code for " + (row.invoice.invoiceNumber || "invoice")} disabled={!selected} value={selectedCostCode || ""} onChange={(event) => onCostCodeChange(event.target.value)} className="field-input mt-1.5"><option value="">Not assigned</option>{selectableCodes.map((cc) => <option key={cc.id} value={cc.id}>{formatCostCodeOptionLabel(cc)}</option>)}</select></label>
      </div>
      <label className="mt-3 block rounded-lg border border-indigo-200 bg-white p-3"><span className="text-[10px] font-bold uppercase tracking-wide text-indigo-700">Amount to assign ({row.invoice.currency || project.currency})</span><input type="number" min="0" max={row.remainingAmount} step="0.01" disabled={!selected} value={selected ? amount : ""} onChange={(event) => onAmountChange(Number(event.target.value))} placeholder={selected ? "Enter an amount" : "Select this invoice first"} className="field-input mt-1.5 text-right tabular-nums disabled:bg-slate-50" aria-label={"Amount to assign for " + (row.invoice.invoiceNumber || "invoice")} /></label>
      <p className="mt-3 text-[10px] text-slate-500">{reviewLabel} · {paymentLabel}{row.currentProjectAmount > 0 ? " · Already on this project: " + money(row.currentProjectAmount, row.invoice.currency || project.currency) : ""}</p>
    </article>
  );
}

export const ExistingInvoicePicker: React.FC<ExistingInvoicePickerProps> = ({ project, invoices, allocations, costCodes = [], settlementProjections, today, onAssign, onClose }) => {
  ...
  const rows = useMemo<PickerRow[]>(() => invoices.map((invoice) => {
    ...
    return { invoice, allocations: invoiceAllocations, currentProjectAllocations, allocatedAmount, currentProjectAmount, remainingAmount: remainingInvoiceAllocatableAmount(invoice.grandTotal, invoiceAllocations), paymentState: supplierInvoicePaymentStateFor(invoice, settlementProjections?.get(invoice.id), today) };
  }), [invoices, allocations, project.id, settlementProjections, today]);
