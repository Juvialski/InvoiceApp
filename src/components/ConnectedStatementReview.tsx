...
export const ConnectedStatementReview: React.FC<ConnectedStatementReviewProps> = ({ data, canImport = false, onCommitImport }) => {
  ...
  const commit = async () => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "The statement import could not be committed.");
  ...
  return <section className="rounded-2xl border border-sky-200 bg-sky-50/50 p-4 shadow-sm sm:p-5" aria-label="Connected mailbox statement review">
    <div className="flex items-start justify-between gap-3">
      ...
      <button type="button" disabled={busy} onClick={dismiss} aria-label="Dismiss connected statement review" className="rounded-lg p-1.5 text-slate-500 hover:bg-white disabled:opacity-50"><X className="h-4 w-4" /></button>
    </div>
    ...
    {document && activeAccounts.length > 0 && <div className="mt-4 space-y-4">
      <div className="grid gap-3 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
        <label className="space-y-1"><span className="field-label">Cash account</span><select className="field-input" value={accountId} onChange={(event) => { setAccountId(event.target.value); setPreview(null); }}>{activeAccounts.map((account) => <option key={account.id} value={account.id}>{account.displayName} · {account.currency}</option>)}</select></label>
