...
export interface SupplierInvoiceReviewProps {
  ...
  /** Supplied by the workspace so readiness uses the current company profile. */
...
export const SupplierInvoiceReview: React.FC<SupplierInvoiceReviewProps> = ({ invoice, readOnly = false, onUpdateInvoice, onVerify: verifyHandler, verifyLabel = "Verify & Create Expense", onReopen, onRevertToAI, onFocusField, vendors = [], projects = [], projectAllocations = [], buyerProfile, allowReopen = true }) => {
  ...
  const mismatch = supplierInvoiceBuyerMismatch(invoice, profile || DEFAULT_COMPANY_DOCUMENT_PROFILE);
  const readiness = useMemo(() => getSupplierInvoiceExpenseReadiness(invoice, {
    allocations: projectAllocations,
    projects,
    buyerProfile: buyerProfile === undefined ? profile : buyerProfile,
  }), [buyerProfile, invoice, projectAllocations, profile, projects]);
  const onVerify = mismatch || !readiness.complete ? undefined : verifyHandler;
  ...
  const attentionItems = useMemo(() => [
    ...
    ...readiness.issues.filter((issue) => issue.code !== "BUYER_MISMATCH").map((issue) => ({ id: `readiness-${issue.code}`, text: issue.message, field: issue.field })),
  ...
  ], [confidenceWarning, conflicts, duplicate, issues, invoice.confidenceScore, mismatch, readiness.issues]);

  useEffect(() => {
    if (buyerProfile !== undefined) {
  ...
  const chooseVendor = (vendorId: string) => {
    ...
    onUpdateInvoice({
      ...invoice,
      vendor: vendor
        ? { ...invoice.vendor, vendorId: vendor.id, name: vendor.name, taxId: vendor.taxId || invoice.vendor.taxId, email: vendor.email || invoice.vendor.email, phone: vendor.phone || invoice.vendor.phone, address: vendor.address || invoice.vendor.address }
        : { ...invoice.vendor, vendorId: undefined },
    ...
    <section className="space-y-3" data-testid="supplier-invoice-review" aria-label="Supplier invoice review">
      <div className={`rounded-2xl border p-4 shadow-sm ${attentionItems.length ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap justify-end gap-2">{!readOnly && onRevertToAI && <button type="button" onClick={onRevertToAI} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700"><Undo2 className="h-3.5 w-3.5" />Revert to original</button>}{!readOnly && <button type="button" onClick={() => setEditMode((value) => !value)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700"><Edit3 className="h-3.5 w-3.5" />{editMode ? "Review mode" : "Edit details"}</button>}{invoice.reviewStatus === "VERIFIED" && onReopen && allowReopen && <button type="button" onClick={() => void onReopen()} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-300 bg-white px-3 py-2 text-xs font-bold text-emerald-800"><Undo2 className="h-3.5 w-3.5" />Reopen for review</button>}{invoice.reviewStatus === "VERIFIED" && onReopen && !allowReopen && <span className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-[10px] font-black text-indigo-800">Linked Expense is authoritative; use Expense correction</span>}{invoice.reviewStatus === "VERIFIED" ? <span className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-300 bg-white px-3 py-2 text-[10px] font-black text-emerald-800"><ShieldCheck className="h-3.5 w-3.5" />Verified {formatDateTime(invoice.verifiedAt)}</span> : onVerify && <button type="button" onClick={onVerify} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-700 px-3 py-2 text-xs font-black text-white hover:bg-emerald-800"><ShieldCheck className="h-3.5 w-3.5" />{verifyLabel}</button>}</div>
        </div>
        {attentionItems.length > 0 && <div className="mt-3 grid gap-2 sm:grid-cols-2">{attentionItems.slice(0, 6).map((item) => <button type="button" key={item.id} onClick={() => { setEditMode(true); onFocusField?.(item.field); }} className="rounded-xl border border-amber-200 bg-white/80 p-3 text-left hover:bg-white"><span className="block text-[10px] font-black text-amber-900">{item.text}</span><span className="mt-1 block text-[9px] font-bold text-amber-700">{readOnly ? "Reopen to resolve" : "Review field"}</span></button>)}</div>}
      </div>
      ...
      {editMode && !readOnly ? (
        <div className="space-y-3 rounded-2xl border border-indigo-200 bg-white p-4 shadow-sm" data-testid="supplier-invoice-edit-details">
          ...
          <div className="grid gap-3 rounded-xl border border-indigo-100 bg-indigo-50/50 p-3 sm:grid-cols-2" data-testid="supplier-invoice-expense-facts"><div className="sm:col-span-2"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-indigo-700">Expense posting facts</p><p className="mt-1 text-[10px] text-indigo-900">These human-confirmed values create the authoritative Expense; source line items remain preserved evidence.</p></div><label className="space-y-1"><span className="field-label">Expense category</span><input value={invoice.category || ""} onChange={(event) => update("category", event.target.value)} className="field-input" /></label><label className="space-y-1"><span className="field-label">Expense description</span><input value={invoice.description || ""} onChange={(event) => update("description", event.target.value)} className="field-input" /></label></div><div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1"><span className="field-label">Amount paid</span><input type="number" step="0.01" value={invoice.amountPaid ?? ""} onChange={(event) => update("amountPaid", event.target.value === "" ? undefined : Number(event.target.value))} className="field-input" /></label><label className="space-y-1"><span className="field-label">Balance due</span><input type="number" step="0.01" value={invoice.balanceDue ?? ""} onChange={(event) => update("balanceDue", event.target.value === "" ? undefined : Number(event.target.value))} className="field-input" /></label><label className="space-y-1 sm:col-span-2"><span className="field-label">Notes and terms</span><textarea value={invoice.notes || ""} onChange={(event) => update("notes", event.target.value)} rows={3} className="field-input resize-y" /></label></div>
