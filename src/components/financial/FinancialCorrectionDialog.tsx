...
function actionLabel(action: FinancialCorrectionAction, entityLabel: string) {
  if (action === "DELETE_UNUSED") return entityLabel === "invoice" ? "Delete unused invoice" : `Delete unused ${entityLabel}`;
  if (action === "VOID") return `Void ${entityLabel}`;
  if (action === "ARCHIVE") return entityLabel === "invoice" ? "Archive / hide invoice" : `Archive ${entityLabel}`;
...
export const FinancialCorrectionDialog: React.FC<FinancialCorrectionDialogProps> = ({ entityLabel, recordLabel, preview, loading, error, reason, onReasonChange, onApply, onClose }) => {
  ...
  const actionCard = (action: FinancialCorrectionAction, enabled: boolean, tone: "danger" | "warning" | "success", description: string, Icon: React.ElementType) => {
    ...
    return <div className={`rounded-xl border p-3 ${tones[tone]}`} key={action}>
      ...
      <button type="button" disabled={loading || !canApply(action)} onClick={() => applyAction(action)} className={`mt-3 rounded-lg px-3 py-2 text-[10px] font-black text-white disabled:cursor-not-allowed disabled:opacity-40 ${buttons[tone]}`}>{loading ? "Working…" : actionLabel(action, entityLabel)}</button>
    </div>;
  };

  const protectedDependencyCount = preview?.protectedDependencyCount ?? preview?.totalDependencyCount ?? 0;
  const disposableDependencyCount = preview?.disposableDependencyCount ?? 0;
  const blockingDependencies = preview?.blockingDependencies || preview?.dependencies || {};
  const disposableDependencies = preview?.disposableDependencies || {};
  ...
  return <div ref={dialogRef} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="financial-correction-title" aria-busy={loading}>
    <section className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
      ...
      {preview && <div className="mt-5 space-y-4">
        <div className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3 text-xs text-indigo-950"><p className="font-black">{preview.source === "database" ? "Authoritative database preflight" : "Local workspace preflight"}</p><p className="mt-1">Current state: <strong>{preview.lifecycleStatus.replaceAll("_", " ")}</strong>{preview.reviewStatus ? ` · review ${preview.reviewStatus.replaceAll("_", " ")}` : ""}{preview.paymentStatus ? ` · payment ${preview.paymentStatus.replaceAll("_", " ")}` : ""}.</p><p className="mt-1">{protectedDependencyCount ? `${protectedDependencyCount} protected downstream or finalized record${protectedDependencyCount === 1 ? "" : "s"} preserve this ${entityLabel}.` : `No protected downstream financial or operational history was found.`}{disposableDependencyCount ? ` ${disposableDependencyCount} ${entityLabel}-owned extraction or intake record${disposableDependencyCount === 1 ? "" : "s"} can be cleaned or retained safely.` : ""}</p>{preview.source !== "database" && <p className="mt-1 text-[10px] text-indigo-800">Permanent deletion remains unavailable without an authoritative database preflight.</p>}<div className="mt-2 flex flex-wrap gap-1.5"><span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold">{preview.canDelete ? "Delete eligible" : "Delete guarded"}</span><span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold">{preview.canVoid ? "Void available" : "Void blocked"}</span>{preview.archivedAt && <span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold">Archived visibility</span>}{preview.lifecycleStatus === "VOID" && <span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold">Financially void</span>}</div>{Object.values(blockingDependencies).some((count) => count > 0) && <><p className="mt-2 text-[10px] font-black uppercase tracking-wide text-rose-800">Protected blockers</p><ul className="grid gap-1 text-[10px] sm:grid-cols-2">{Object.entries(blockingDependencies).filter(([, count]) => count > 0).map(([key, count]) => <li key={key} className="flex justify-between gap-2"><span>{titleCase(key)}</span><strong>{count}</strong></li>)}</ul></>}{Object.values(disposableDependencies).some((count) => count > 0) && <><p className="mt-2 text-[10px] font-black uppercase tracking-wide text-indigo-800">Disposable or retained provenance</p><ul className="grid gap-1 text-[10px] sm:grid-cols-2">{Object.entries(disposableDependencies).filter(([, count]) => count > 0).map(([key, count]) => <li key={key} className="flex justify-between gap-2"><span>{titleCase(key)}</span><strong>{count}</strong></li>)}</ul></>}</div>
        ...
        {actionCard("DELETE_UNUSED", preview.canDelete, "danger", `This permanently removes the unused ${entityLabel}. It is shown only when the authoritative preflight found no protected downstream or finalized history.`, Trash2)}
        {actionCard("VOID", preview.canVoid, "warning", `This removes the ${entityLabel} from active financial use while preserving the original values and history. Confirmed settlement evidence must be corrected first.`, ShieldAlert)}
        {actionCard("ARCHIVE", preview.canArchive, "warning", `This hides the ${entityLabel} from active work without changing its financial status or cost contribution. History is preserved.`, Archive)}
