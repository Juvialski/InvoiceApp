  ...
  return (
    <span ref={containerRef} className={`relative inline-flex align-middle ${className}`}>
      <button
        ...
        className="hqs-control inline-flex min-h-7 min-w-7 items-center justify-center rounded-full p-1 text-slate-500 hover:border-indigo-300 hover:text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
      >
      ...
      {open && (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <button type="button" aria-label={`Close ${label}`} onClick={() => { setOpen(false); triggerRef.current?.focus(); }} className="hqs-control inline-flex min-h-7 min-w-7 items-center justify-center rounded-lg border-0 bg-transparent text-slate-400 hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500">
