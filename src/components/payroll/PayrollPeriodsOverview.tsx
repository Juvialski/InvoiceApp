...
const PeriodCard: React.FC<{
  ...
  onSelectPeriod?: (periodId: string) => void;
  onOpenOverview?: (period: PayrollPeriod) => void;
}> = ({ period, periods, runs, entries, importBatches, automaticDrafts, automaticDraft, frequencyLabel, today, selected, onSelectPeriod, onOpenOverview }) => {
  ...
  return <article className={`rounded-2xl border bg-white p-4 shadow-sm ${selected ? "border-indigo-400 ring-2 ring-indigo-100" : "border-slate-200"}`}>
    <button type="button" onClick={() => onSelectPeriod?.(period.id)} aria-pressed={selected} className="block w-full text-left">
      <div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="text-base font-black text-slate-950">{formatPayrollPeriodLabel(period, frequency(frequencyLabel))}</h3><p className="mt-1 text-xs text-slate-500">{frequencyLabel ? `${frequencyLabel} payroll` : "Payroll period"}</p></div><span className={`rounded-full px-2 py-1 text-[9px] font-black ${statusClass(state)}`}>{state}</span></div>
...
export const PayrollPeriodsOverview: React.FC<PayrollPeriodsOverviewProps> = ({ periods, runs, entries, importBatches, automaticDrafts, automaticDraft, selectedPeriodId, frequencyLabel, today, onSelectPeriod, onOpenOverview }) => {
