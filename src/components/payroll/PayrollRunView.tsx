...
const RunCard: React.FC<RunCardProps> = ({ run, period, entries, allocations, workers, projects, onCalculate, onUpdateRun }) => {
  ...
  return <div className="px-4 py-5 sm:px-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div className="flex min-w-0 items-start gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${run.status === "APPROVED" || run.status === "PAID" ? "bg-emerald-50 text-emerald-700" : run.status === "VOID" ? "bg-slate-100 text-slate-400" : "bg-violet-50 text-violet-700"}`}>{locked ? <LockKeyhole className="h-4 w-4" /> : run.status === "DRAFT" ? <Clock3 className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-black">{period.periodStart} – {period.periodEnd}</p><span className={`rounded-full px-2 py-1 text-[10px] font-black ${statusStyle(run.status)}`}>{run.status}</span></div><p className="mt-1 text-[10px] text-slate-500">Run {run.id.slice(0, 8)} · created {run.createdAt.slice(0, 10)}{run.approvedAt ? ` · approved ${run.approvedAt.slice(0, 10)}` : ""}{run.paidAt ? ` · paid ${run.paidAt.slice(0, 10)}` : ""}</p></div></div><div className="flex flex-wrap items-center gap-2 xl:justify-end"><button onClick={onCalculate} disabled={run.status !== "DRAFT" && run.status !== "CALCULATED"} className="inline-flex items-center gap-1.5 rounded-lg bg-violet-50 px-3 py-2 text-[10px] font-bold text-violet-800 disabled:cursor-not-allowed disabled:opacity-40"><Calculator className="h-3.5 w-3.5" /> {run.status === "CALCULATED" ? "Recalculate" : "Calculate"}</button><button onClick={() => onUpdateRun?.({ ...run, status: "APPROVED", approvedAt: new Date().toISOString() })} disabled={run.status !== "CALCULATED" || !onUpdateRun} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-[10px] font-bold text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 className="h-3.5 w-3.5" /> Approve</button><button onClick={() => onUpdateRun?.({ ...run, status: "PAID", paidAt: new Date().toISOString() })} disabled={run.status !== "APPROVED" || !onUpdateRun} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 px-3 py-2 text-[10px] font-bold text-indigo-800 disabled:cursor-not-allowed disabled:opacity-40"><WalletCards className="h-3.5 w-3.5" /> Mark paid</button></div></div>
    <div className="mt-4 grid gap-3 sm:grid-cols-3"><Summary label="Gross" value={money(total)} /><Summary label="Allocated" value={money(allocated)} /><Summary label="Unallocated" value={money(completeness.unallocatedAmount)} warning={completeness.unallocatedAmount > 0} /></div>
    ...
    {locked && <p className="mt-4 flex items-center gap-1.5 text-[10px] text-slate-500"><LockKeyhole className="h-3 w-3" /> Entry and allocation edits are disabled after approval, payment, or voiding.</p>}
  </div>;
};
