...
export const PayrollRunView: React.FC<PayrollRunViewProps> = ({ runs, periods, entries, allocations, workers, projects = [], workEntries = [], assignments = [], selectedPeriodId, onCreateRun, onSaveEntry, onUpdateRun, onCalculateRun }) => {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" data-tour="payroll-runs">
    ...
    {periodRuns.length > 0 && <div className="divide-y divide-slate-100">{periodRuns.map((run) => <RunCard key={run.id} run={run} period={period!} entries={entries.filter((entry) => entry.payrollRunId === run.id)} allocations={allocations} workers={workers} projects={projects} onCalculate={() => calculateRun(run)} onUpdateRun={onUpdateRun} canManagePayroll={canManagePayroll} canApprovePayroll={canApprovePayroll} />)}</div>}
  </section>;
};

interface RunCardProps { run: PayrollRun; period: PayrollPeriod; entries: PayrollEntry[]; allocations: PayrollProjectAllocation[]; workers: Worker[]; projects: Project[]; onCalculate: () => void; onUpdateRun?: (run: PayrollRun) => void; canManagePayroll: boolean; canApprovePayroll: boolean; }
const RunCard: React.FC<RunCardProps> = ({ run, period, entries, allocations, workers, projects, onCalculate, onUpdateRun, canManagePayroll, canApprovePayroll }) => {
  const permissions = useAppPermissions();
  ...
  return <div className="px-4 py-5 sm:px-5">
    ...
    {(run.status === "APPROVED" || run.status === "PAID") && <div className="mt-4"><FinancialSettlementCard targetType="PAYROLL" targetId={run.id} compact canReverse={canReverseSettlement} targetLabel={`Payroll Period ${period.periodStart} – ${period.periodEnd}`} /></div>}
