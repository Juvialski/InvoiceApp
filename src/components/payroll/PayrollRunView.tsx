...
export const PayrollRunView: React.FC<PayrollRunViewProps> = ({ runs, periods, entries, allocations, workers, projects = [], costCodes = [], workEntries = [], assignments = [], selectedPeriodId, onCreateRun, onSaveEntry, onUpdateRun, onCalculateRun, onNavigatePath }) => {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" data-tour="payroll-runs">
    <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-black">Payroll runs</h3><p className="mt-1 text-xs text-slate-500">Review a selected period, calculate its snapshot, approve it, then record employee net-pay disbursement through Cash &amp; Banking.</p></div><div className="flex flex-wrap gap-2">{canManagePayroll && onSaveEntry && <PayrollEntryForm runs={editableRuns} workers={workers} projects={projects} costCodes={costCodes} onSave={onSaveEntry} />}{canManagePayroll && onCreateRun && period && <button onClick={() => onCreateRun(period.id)} className="rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white"><HardHat className="mr-1 inline h-3.5 w-3.5" /> Create run</button>}</div></div>
    ...
    {periodRuns.length > 0 && <div className="divide-y divide-slate-100">{periodRuns.map((run) => <RunCard key={run.id} run={run} period={period!} entries={entries.filter((entry) => entry.payrollRunId === run.id)} allocations={allocations} workers={workers} projects={projects} onCalculate={() => calculateRun(run)} onUpdateRun={onUpdateRun} onNavigatePath={onNavigatePath} canManagePayroll={canManagePayroll} canApprovePayroll={canApprovePayroll} />)}</div>}
  </section>;
};
...
const RunCard: React.FC<RunCardProps> = ({ run, period, entries, allocations, workers, projects, onCalculate, onUpdateRun, onNavigatePath, canManagePayroll, canApprovePayroll }) => {
  ...
  return <div className="px-4 py-5 sm:px-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div className="flex min-w-0 items-start gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${run.status === "APPROVED" || run.status === "PAID" ? "bg-emerald-50 text-emerald-700" : run.status === "VOID" ? "bg-slate-100 text-slate-400" : "bg-violet-50 text-violet-700"}`}>{locked ? <LockKeyhole className="h-4 w-4" /> : run.status === "DRAFT" ? <Clock3 className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-black">{period.periodStart} – {period.periodEnd}</p><span className={`rounded-full px-2 py-1 text-[10px] font-black ${statusStyle(run.status)}`}>{run.status}</span></div><p className="mt-1 text-[10px] text-slate-500">Payroll run · created {run.createdAt.slice(0, 10)}{run.approvedAt ? ` · approved ${run.approvedAt.slice(0, 10)}` : ""}{run.paidAt ? ` · paid ${run.paidAt.slice(0, 10)}` : ""}</p></div></div><div className="flex flex-wrap items-center gap-2 xl:justify-end">{canManagePayroll && <button onClick={onCalculate} disabled={run.status !== "DRAFT" && run.status !== "CALCULATED"} className="inline-flex items-center gap-1.5 rounded-lg bg-violet-50 px-3 py-2 text-[10px] font-bold text-violet-800 disabled:cursor-not-allowed disabled:opacity-40"><Calculator className="h-3.5 w-3.5" /> {run.status === "CALCULATED" ? "Recalculate" : "Calculate"}</button>}{canApprovePayroll && <button onClick={() => onUpdateRun?.({ ...run, status: "APPROVED", approvedAt: new Date().toISOString() })} disabled={run.status !== "CALCULATED" || !onUpdateRun} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-[10px] font-bold text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 className="h-3.5 w-3.5" /> Approve</button>}</div></div>
    ...
    {(run.status === "APPROVED" || run.status === "PAID") && <div className="mt-4"><FinancialSettlementCard targetType="PAYROLL" targetId={run.id} compact canReverse={canReverseSettlement} targetLabel={`Payroll Period ${period.periodStart} – ${period.periodEnd}`} onNavigatePath={onNavigatePath} /></div>}
