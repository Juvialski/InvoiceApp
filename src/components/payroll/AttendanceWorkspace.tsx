...
export const AttendanceWorkspace: React.FC<AttendanceWorkspaceProps> = ({
  ...
  const saveRow = (snapshot: (typeof snapshots)[number]) => {
    if (!canManagePayrollSources || selectedPeriodLocked) {
      setMessage({ tone: "error", text: "This payroll period is locked; attendance records are read-only." });
      return;
    }
  ...
  const markPresent = () => {
    ...
    if (selectedPeriodLocked) { setMessage({ tone: "error", text: "This payroll period is locked; attendance records are read-only." }); return; }
  ...
  const saveLeave = (event: React.FormEvent) => {
    ...
    if (selectedPeriodLocked) { setMessage({ tone: "error", text: "This payroll period is locked; leave requests cannot be changed here." }); return; }
    if (selectedPeriod && (leaveForm.startDate < selectedPeriod.periodStart || leaveForm.endDate > selectedPeriod.periodEnd)) { setMessage({ tone: "error", text: "Leave dates must stay inside the selected payroll period." }); return; }
  ...
  const saveOvertime = (event: React.FormEvent) => {
    ...
    if (selectedPeriodLocked) { setMessage({ tone: "error", text: "This payroll period is locked; overtime requests cannot be changed here." }); return; }
    if (!periodDatesValid || (selectedPeriod && !dateInPeriod(overtimeForm.date, selectedPeriod))) { setMessage({ tone: "error", text: "Overtime date must stay inside the selected payroll period." }); return; }
  ...
  return <section className="space-y-4" data-tour="attendance-roster">
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      {message && <div role={message.tone === "error" ? "alert" : "status"} className={`mt-3 flex items-start gap-2 rounded-xl border p-3 text-xs ${message.tone === "error" ? "border-rose-200 bg-rose-50 text-rose-900" : "border-indigo-100 bg-indigo-50 text-indigo-900"}`}><ShieldAlert aria-hidden="true" className="mt-0.5 h-3.5 w-3.5 shrink-0" />{message.text}</div>}
      <div className="mt-4 flex flex-wrap items-center gap-2"><button type="button" onClick={markPresent} disabled={!periodDatesValid || selectedPeriodLocked || !workers.length || !canManagePayrollSources} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 className="h-3.5 w-3.5" /> Mark scheduled workers present</button><span className="text-[10px] text-slate-500">Exceptions can be edited below before payroll calculation.</span></div>
    </div>
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase text-slate-500"><tr><th className="px-4 py-3">Worker</th><th className="px-4 py-3">Schedule</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Time in</th><th className="px-4 py-3">Time out</th><th className="px-4 py-3">Derived</th><th className="px-4 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{snapshots.map((snapshot) => {
        ...
        const effective = record ? record : undefined;
         const workerLabel = worker?.displayName || `Worker ${snapshot.workerId}`;
         const lockedHint = snapshot.status !== "SCHEDULED" ? label(snapshot.status) : "Scheduled";
         return <tr key={snapshot.workerId} className={!snapshot.expected ? "bg-slate-50/50" : undefined}><td className="px-4 py-3"><p className="font-bold">{workerLabel}</p><p className="text-[10px] text-slate-500">{worker?.employeeCode || "—"} · {lockedHint}</p></td><td className="px-4 py-3 text-slate-600">{snapshot.scheduledStart && snapshot.scheduledEnd ? `${snapshot.scheduledStart}–${snapshot.scheduledEnd}` : "No clock schedule"}<p className="text-[10px] text-slate-400">{snapshot.scheduledMinutes ? `${snapshot.scheduledMinutes} min` : "Schedule snapshot pending"}</p></td><td className="px-4 py-3"><select aria-label={`${workerLabel} attendance status`} disabled={!canManagePayrollSources || selectedPeriodLocked} value={draft.status} onChange={(event) => setDrafts((current) => ({ ...current, [snapshot.workerId]: { ...draft, status: event.target.value as AttendanceStatus } }))} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px] font-bold disabled:cursor-not-allowed disabled:opacity-60">{ATTENDANCE_STATUSES.map((status) => <option key={status} value={status}>{label(status)}</option>)}</select></td><td className="px-4 py-3"><input aria-label={`${workerLabel} time in`} disabled={!canManagePayrollSources || selectedPeriodLocked} type="time" value={draft.timeIn} onChange={(event) => setDrafts((current) => ({ ...current, [snapshot.workerId]: { ...draft, timeIn: event.target.value } }))} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px] disabled:cursor-not-allowed disabled:opacity-60" /></td><td className="px-4 py-3"><input aria-label={`${workerLabel} time out`} disabled={!canManagePayrollSources || selectedPeriodLocked} type="time" value={draft.timeOut} onChange={(event) => setDrafts((current) => ({ ...current, [snapshot.workerId]: { ...draft, timeOut: event.target.value } }))} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px] disabled:cursor-not-allowed disabled:opacity-60" /></td><td className="px-4 py-3 text-[10px] text-slate-600">{effective ? `${effective.regularMinutes}m regular · ${effective.lateMinutes}m late · ${effective.undertimeMinutes}m under` : "Missing attendance"}</td><td className="px-4 py-3 text-right"><div className="flex flex-wrap justify-end gap-1.5">{canManagePayrollSources && <button type="button" onClick={() => saveRow(snapshot)} disabled={!periodDatesValid || selectedPeriodLocked} className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[10px] font-bold text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40"><Save className="h-3 w-3" /> Save</button>}{canManagePayrollSources && record && record.recordStatus === "DRAFT" && !selectedPeriodLocked && <button type="button" onClick={() => runSourceLifecycle("ATTENDANCE", record.id, "DELETE_DRAFT")} className="text-[10px] font-bold text-rose-700">Delete draft</button>}{canManagePayrollSources && record && record.recordStatus === "CONFIRMED" && !selectedPeriodLocked && <button type="button" onClick={() => runSourceLifecycle("ATTENDANCE", record.id, "VOID")} className="text-[10px] font-bold text-amber-700">Void</button>}</div></td></tr>;
      })}</tbody></table></div>
    ...
    <div className="grid gap-4 xl:grid-cols-3">
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><h4 className="text-sm font-black">Leave / absences</h4><p className="mt-1 text-[10px] text-slate-500">Approve, reject, or cancel leave here. Approved leave appears in the roster automatically; pay treatment is never assumed.</p></div><CalendarDays aria-hidden="true" className="h-4 w-4 text-indigo-600" /></div>{onSaveLeave && <form onSubmit={saveLeave} className="mt-3 space-y-2"><select aria-label="Leave worker" disabled={!canManagePayrollSources || selectedPeriodLocked || !workers.length} value={leaveForm.workerId} onChange={(event) => setLeaveForm({ ...leaveForm, workerId: event.target.value })} className="field-input disabled:cursor-not-allowed disabled:opacity-60">{workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.displayName}</option>)}</select><div className="grid grid-cols-2 gap-2"><input aria-label="Leave start date" disabled={selectedPeriodLocked} type="date" value={leaveForm.startDate} onChange={(event) => setLeaveForm({ ...leaveForm, startDate: event.target.value })} className="field-input disabled:cursor-not-allowed disabled:opacity-60" /><input aria-label="Leave end date" disabled={selectedPeriodLocked} type="date" value={leaveForm.endDate} onChange={(event) => setLeaveForm({ ...leaveForm, endDate: event.target.value })} className="field-input disabled:cursor-not-allowed disabled:opacity-60" /></div><div className="grid grid-cols-2 gap-2"><input aria-label="Leave type" disabled={selectedPeriodLocked} value={leaveForm.leaveType} onChange={(event) => setLeaveForm({ ...leaveForm, leaveType: event.target.value })} placeholder="Leave type" className="field-input disabled:cursor-not-allowed disabled:opacity-60" /><select aria-label="Leave pay treatment" disabled={selectedPeriodLocked} value={leaveForm.paid} onChange={(event) => setLeaveForm({ ...leaveForm, paid: event.target.value })} className="field-input disabled:cursor-not-allowed disabled:opacity-60"><option value="">Pay treatment not configured</option><option value="true">Explicitly paid</option><option value="false">Explicitly unpaid</option></select></div><button type="submit" disabled={!canManagePayrollSources || selectedPeriodLocked || !workers.length} className="inline-flex items-center gap-1 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-3.5 w-3.5" /> Record leave request</button></form>}{leaveRequests.filter((request) => ["PENDING", "APPROVED"].includes(request.status)).slice(0, 6).map((request) => <div key={request.id} className="mt-3 rounded-xl bg-slate-50 p-3 text-[10px]"><div className="flex items-center justify-between gap-2"><span className="min-w-0 truncate font-bold">{workers.find((worker) => worker.id === request.workerId)?.displayName || request.workerId} · {request.startDate}–{request.endDate}</span><span className={`rounded-full px-2 py-1 font-black ${request.status === "APPROVED" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}`}>{request.status}</span></div>{onSaveLeave && <div className="mt-2 flex flex-wrap justify-end gap-1.5">{request.status === "PENDING" && <><button type="button" disabled={selectedPeriodLocked} onClick={() => updateLeaveStatus(request, "APPROVED")} className="rounded-lg bg-emerald-50 px-2 py-1.5 font-bold text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40">Approve</button><button type="button" disabled={selectedPeriodLocked} onClick={() => updateLeaveStatus(request, "REJECTED")} className="rounded-lg bg-rose-50 px-2 py-1.5 font-bold text-rose-800 disabled:cursor-not-allowed disabled:opacity-40">Reject</button></>}{request.status === "APPROVED" && <button type="button" disabled={selectedPeriodLocked} onClick={() => updateLeaveStatus(request, "CANCELLED")} className="rounded-lg bg-slate-200 px-2 py-1.5 font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40">Cancel</button>}</div>}</div>)}</section>
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><h4 className="text-sm font-black">Overtime approvals</h4><p className="mt-1 text-[10px] text-slate-500">Only approved explicit overtime affects payroll. Legacy time-entry OT remains a fallback when no approved request exists.</p></div><RotateCcw aria-hidden="true" className="h-4 w-4 text-violet-600" /></div>{onSaveOvertime && <form onSubmit={saveOvertime} className="mt-3 space-y-2"><select aria-label="Overtime worker" disabled={!canManagePayrollSources || selectedPeriodLocked || !workers.length} value={overtimeForm.workerId} onChange={(event) => setOvertimeForm({ ...overtimeForm, workerId: event.target.value })} className="field-input disabled:cursor-not-allowed disabled:opacity-60">{workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.displayName}</option>)}</select><div className="grid grid-cols-2 gap-2"><input aria-label="Overtime date" disabled={!canManagePayrollSources || selectedPeriodLocked} type="date" value={overtimeForm.date} onChange={(event) => setOvertimeForm({ ...overtimeForm, date: event.target.value })} className="field-input disabled:cursor-not-allowed disabled:opacity-60" /><input aria-label="Overtime hours" disabled={!canManagePayrollSources || selectedPeriodLocked} type="number" min="0" step="0.25" value={overtimeForm.hours} onChange={(event) => setOvertimeForm({ ...overtimeForm, hours: event.target.value })} placeholder="Hours" className="field-input disabled:cursor-not-allowed disabled:opacity-60" /></div><input aria-label="Overtime reason" disabled={!canManagePayrollSources || selectedPeriodLocked} value={overtimeForm.reason} onChange={(event) => setOvertimeForm({ ...overtimeForm, reason: event.target.value })} placeholder="Reason" className="field-input disabled:cursor-not-allowed disabled:opacity-60" /><button type="submit" disabled={!canManagePayrollSources || selectedPeriodLocked || !workers.length} className="inline-flex items-center gap-1 rounded-xl bg-violet-600 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-3.5 w-3.5" /> Request overtime</button></form>}{overtimeRequests.filter((request) => request.overtimeDate === date).slice(0, 5).map((request) => <div key={request.id} className="mt-3 flex items-center justify-between gap-2 rounded-xl bg-slate-50 p-3 text-[10px]"><div><p className="font-bold">{workers.find((worker) => worker.id === request.workerId)?.displayName || request.workerId}</p><p className="text-slate-500">{(request.requestedMinutes / 60).toFixed(2)}h requested · {request.reason || "No reason"}</p></div><div className="flex items-center gap-1.5">{request.status === "PENDING" && onSaveOvertime ? <button type="button" disabled={!canManagePayrollSources || selectedPeriodLocked} onClick={() => onSaveOvertime({ ...request, status: "APPROVED", approvedMinutes: request.requestedMinutes, approvedAt: now(), updatedAt: now() })} className="rounded-lg bg-emerald-50 px-2 py-1.5 font-bold text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40">Approve</button> : <span className="rounded-full bg-slate-100 px-2 py-1 font-black">{request.status}</span>}{request.status === "APPROVED" && canManagePayrollSources && <button type="button" disabled={selectedPeriodLocked} onClick={() => runSourceLifecycle("OVERTIME", request.id, "CANCEL")} className="text-[10px] font-bold text-amber-700 disabled:cursor-not-allowed disabled:opacity-40">Cancel</button>}</div></div>)}</section>
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><h4 className="text-sm font-black">Holiday context</h4><p className="mt-1 text-[10px] text-slate-500">Company-defined classification only; premium pay requires a configured policy.</p></div><CalendarDays aria-hidden="true" className="h-4 w-4 text-amber-600" /></div>{onSaveHoliday && <form onSubmit={saveHoliday} className="mt-3 space-y-2"><input aria-label="Holiday date" type="date" value={holidayForm.date} onChange={(event) => setHolidayForm({ ...holidayForm, date: event.target.value })} className="field-input" /><input aria-label="Holiday name" value={holidayForm.name} onChange={(event) => setHolidayForm({ ...holidayForm, name: event.target.value })} placeholder="Holiday name" className="field-input" /><button type="submit" className="inline-flex items-center gap-1 rounded-xl bg-amber-500 px-3 py-2 text-xs font-bold text-white"><Plus className="h-3.5 w-3.5" /> Add holiday</button></form>}{holidays.filter((holiday) => holiday.active && holiday.holidayDate >= date).slice(0, 4).map((holiday) => <div key={holiday.id} className="mt-3 flex items-center justify-between rounded-xl bg-amber-50 p-3 text-[10px]"><span className="font-bold">{holiday.holidayDate} · {holiday.name}</span><span className="text-amber-800">Context only</span></div>)}</section>
    </div>
  </section>;
};
