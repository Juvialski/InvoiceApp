...
export const AttendanceWorkspace: React.FC<AttendanceWorkspaceProps> = ({
  workers, periods, selectedPeriodId, attendanceRecords, leaveRequests, overtimeRequests, holidays,
  onSaveAttendance, onSaveAttendanceBatch, onSaveLeave, onSaveOvertime, onSaveHoliday,
}) => {
  ...
  return <section className="space-y-4">
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2"><button type="button" onClick={markPresent} disabled={!periodDatesValid || !workers.length} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 className="h-3.5 w-3.5" /> Mark scheduled workers present</button><span className="text-[10px] text-slate-500">Exceptions can be edited below before payroll calculation.</span></div>
    </div>
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase text-slate-500"><tr><th className="px-4 py-3">Worker</th><th className="px-4 py-3">Schedule</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Time in</th><th className="px-4 py-3">Time out</th><th className="px-4 py-3">Derived</th><th className="px-4 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{snapshots.map((snapshot) => {
        ...
        return <tr key={snapshot.workerId} className={!snapshot.expected ? "bg-slate-50/50" : undefined}><td className="px-4 py-3"><p className="font-bold">{worker?.displayName || snapshot.workerId}</p><p className="text-[10px] text-slate-500">{worker?.employeeCode || "—"} · {lockedHint}</p></td><td className="px-4 py-3 text-slate-600">{snapshot.scheduledStart && snapshot.scheduledEnd ? `${snapshot.scheduledStart}–${snapshot.scheduledEnd}` : "No clock schedule"}<p className="text-[10px] text-slate-400">{snapshot.scheduledMinutes ? `${snapshot.scheduledMinutes} min` : "Schedule snapshot pending"}</p></td><td className="px-4 py-3"><select value={draft.status} onChange={(event) => setDrafts((current) => ({ ...current, [snapshot.workerId]: { ...draft, status: event.target.value as AttendanceStatus } }))} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px] font-bold">{ATTENDANCE_STATUSES.map((status) => <option key={status} value={status}>{label(status)}</option>)}</select></td><td className="px-4 py-3"><input type="time" value={draft.timeIn} onChange={(event) => setDrafts((current) => ({ ...current, [snapshot.workerId]: { ...draft, timeIn: event.target.value } }))} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px]" /></td><td className="px-4 py-3"><input type="time" value={draft.timeOut} onChange={(event) => setDrafts((current) => ({ ...current, [snapshot.workerId]: { ...draft, timeOut: event.target.value } }))} className="rounded-lg border border-slate-200 px-2 py-1.5 text-[10px]" /></td><td className="px-4 py-3 text-[10px] text-slate-600">{effective ? `${effective.regularMinutes}m regular · ${effective.lateMinutes}m late · ${effective.undertimeMinutes}m under` : "Missing attendance"}</td><td className="px-4 py-3 text-right"><button type="button" onClick={() => saveRow(snapshot)} disabled={!periodDatesValid} className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[10px] font-bold text-emerald-800 disabled:opacity-40"><Save className="h-3 w-3" /> Save</button></td></tr>;
      })}</tbody></table></div>
