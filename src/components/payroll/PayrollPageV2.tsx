...
export interface PayrollPageV2Props {
  ...
  periodPreparationState?: "NO_SCHEDULE" | "PREPARING" | "READY" | "FAILED";
...
export const PayrollPageV2: React.FC<PayrollPageV2Props> = ({ workers, assignments, periods, runs, entries, allocations, adjustments = [], workEntries = [], attendanceRecords = [], leaveRequests = [], overtimeRequests = [], holidays = [], projects, schedules = [], compensationProfiles = [], recurringComponents = [], importBatches = [], importTemplates = [], periodPreparationState = periods.length ? "READY" : schedules.length ? "PREPARING" : "NO_SCHEDULE", onRetryPeriodPreparation, onSaveWorker, onSavePeriod, onSaveSchedule, onSaveCompensationProfile, onSaveRecurringComponent, onSaveAssignment, onSaveWorkEntry, onSaveAttendance, onSaveAttendanceBatch, onSaveLeave, onSaveOvertime, onSaveHoliday, onSavePayrollEntry, onUpdateRun, onCreateRun, onCalculateRun, onStagePayrollImport, onSavePayrollImportTemplate, onCommitPayrollImport, canManagePayrollSettings = true, canManagePayrollMaintenance = true, onPreviewPayrollMaintenance, onApplyPayrollMaintenance, selectedPeriodId: controlledSelectedPeriodId, onSelectedPeriodIdChange }) => {
  ...
  return <div className="space-y-5" data-tour="payroll-overview">
    ...
    {!selectedPeriod && periodPreparationState !== "READY" && <div role={periodPreparationState === "PREPARING" ? "status" : "alert"} className={`flex items-start gap-3 rounded-2xl p-4 text-sm ${periodPreparationState === "FAILED" ? "border border-rose-200 bg-rose-50 text-rose-900" : periodPreparationState === "NO_SCHEDULE" ? "border border-slate-200 bg-white text-slate-700" : "border border-amber-200 bg-amber-50 text-amber-900"}`}><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><div className="min-w-0"><p className="font-black">{periodPreparationState === "FAILED" ? "Payroll periods could not be prepared." : periodPreparationState === "NO_SCHEDULE" ? "Set up payroll schedule" : "Preparing your payroll calendar…"}</p><p className="mt-1 leading-5">{periodPreparationState === "FAILED" ? "The schedule is still available. Retry calendar preparation when the workspace connection is ready." : periodPreparationState === "NO_SCHEDULE" ? "An active schedule is required before InvoiceApp can create the payroll calendar." : "InvoiceApp is generating a bounded period horizon. This does not depend on having workers yet."}</p>{periodPreparationState === "FAILED" && onRetryPeriodPreparation && <button type="button" onClick={onRetryPeriodPreparation} className="mt-3 rounded-xl bg-rose-700 px-3 py-2 text-xs font-bold text-white hover:bg-rose-800">Retry</button>}</div></div>}
