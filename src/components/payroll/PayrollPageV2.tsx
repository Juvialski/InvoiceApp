...
export interface PayrollPageV2Props {
  ...
  periodPreparationState?: "NO_SCHEDULE" | "PREPARING" | "SYNCING" | "READY" | "FAILED";
...
export const PayrollPageV2: React.FC<PayrollPageV2Props> = ({ workers, assignments, periods, runs, entries, allocations, adjustments = [], workEntries = [], attendanceRecords = [], leaveRequests = [], overtimeRequests = [], holidays = [], projects, schedules = [], compensationProfiles = [], recurringComponents = [], importBatches = [], importTemplates = [], periodPreparationState = periods.length ? "READY" : schedules.length ? "PREPARING" : "NO_SCHEDULE", onRetryPeriodPreparation, onPreviewFactoryReset, onApplyFactoryReset, onSaveWorker, onSavePeriod, onSaveSchedule, onSaveCompensationProfile, onSaveRecurringComponent, onSaveAssignment, onSaveWorkEntry, onSaveAttendance, onSaveAttendanceBatch, onSaveLeave, onSaveOvertime, onSaveHoliday, onSavePayrollEntry, onUpdateRun, onCreateRun, onCalculateRun, onStagePayrollImport, onSavePayrollImportTemplate, onCommitPayrollImport, canManagePayrollSettings = true, canManagePayrollMaintenance = true, onPreviewPayrollMaintenance, onApplyPayrollMaintenance, selectedPeriodId: controlledSelectedPeriodId, onSelectedPeriodIdChange }) => {
  ...
  return <div className="space-y-5" data-tour="payroll-overview">
    <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-600">Project labor operations</p><h1 className="mt-1 text-2xl font-black sm:text-[1.75rem]">Payroll &amp; labor</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Configure recurring payroll once, then import or collect work, review exceptions, calculate, approve, and mark paid. Approved, paid, void, and historical records remain protected.</p></div><div className="flex flex-col gap-2 sm:min-w-[280px]"><label><span className="field-label">Jump to period</span><span className="relative block"><select value={selectedPeriodId} onChange={(event) => { setSelectedPeriodId(event.target.value); setTab("overview"); }} className="field-input appearance-none pr-8"><option value="">No period yet</option>{selectablePeriods.map((period) => <option key={period.id} value={period.id}>{period.periodStart} – {period.periodEnd} · {period.status}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /></span></label></div></div>{selectedPeriod && <div className="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-indigo-100 bg-indigo-50/60 px-3 py-2.5"><div><p className="text-sm font-black text-indigo-950">{selectedPeriod.periodStart} – {selectedPeriod.periodEnd}</p><p className="text-[10px] text-indigo-800">{payrollPeriodFrequencyLabel(selectedPeriod, schedules)} · Pay date {selectedPeriod.payDate || "manual"}</p></div><div className="flex items-center gap-1.5"><button type="button" onClick={() => navigatePeriod(previousPeriod)} disabled={!previousPeriod} className="inline-flex items-center gap-1 rounded-lg border border-indigo-200 bg-white px-2.5 py-1.5 text-xs font-bold text-indigo-800 disabled:opacity-40"><ArrowLeft className="h-3 w-3" /> Previous</button><button type="button" onClick={() => navigatePeriod(periods.find((period) => period.id === currentSuggestedId))} className="rounded-lg bg-indigo-600 px-2.5 py-1.5 text-xs font-bold text-white">Current period</button><button type="button" onClick={() => navigatePeriod(nextPeriod)} disabled={!nextPeriod} className="inline-flex items-center gap-1 rounded-lg border border-indigo-200 bg-white px-2.5 py-1.5 text-xs font-bold text-indigo-800 disabled:opacity-40">Next <ArrowRight className="h-3 w-3" /></button></div></div>}</section>
    {!selectedPeriod && periodPreparationState !== "READY" && <div role={periodPreparationState === "FAILED" ? "alert" : "status"} className={`flex items-start gap-3 rounded-2xl p-4 text-sm ${periodPreparationState === "FAILED" ? "border border-rose-200 bg-rose-50 text-rose-900" : periodPreparationState === "NO_SCHEDULE" ? "border border-slate-200 bg-white text-slate-700" : "border border-amber-200 bg-amber-50 text-amber-900"}`}><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><div className="min-w-0"><p className="font-black">{periodPreparationState === "FAILED" ? "Payroll periods could not be prepared." : periodPreparationState === "NO_SCHEDULE" ? "Set up payroll schedule" : periodPreparationState === "SYNCING" ? "Syncing your payroll calendar…" : "Preparing your payroll calendar…"}</p><p className="mt-1 leading-5">{periodPreparationState === "FAILED" ? "The schedule is still available. Retry calendar preparation when the workspace connection is ready." : periodPreparationState === "NO_SCHEDULE" ? "An active schedule is required before InvoiceApp can create the payroll calendar." : periodPreparationState === "SYNCING" ? "Generated periods are being persisted and reloaded from Supabase. The calendar becomes durable once syncing finishes." : "InvoiceApp is generating a bounded period horizon. This does not depend on having workers yet."}</p>{periodPreparationState === "FAILED" && onRetryPeriodPreparation && <button type="button" onClick={onRetryPeriodPreparation} className="mt-3 rounded-xl bg-rose-700 px-3 py-2 text-xs font-bold text-white hover:bg-rose-800">Retry</button>}</div></div>}
...
function Metric({ icon, value, label, small = false }: { icon: React.ReactNode; value: React.ReactNode; label: string; small?: boolean }) { return <div className="min-w-0 rounded-xl border border-slate-200 bg-white p-3.5"><div>{icon}</div><p className={`${small ? "text-xs" : "text-xl"} mt-3 truncate font-black tabular-nums`}>{value}</p><p className="text-[10px] font-semibold text-slate-500">{label}</p></div>; }
