...
import { formatPayrollPeriodLabel, selectStablePayrollPeriod } from "../../utils/payrollCalendar";
...
function Metric({ icon, value, secondary, label, small = false }: { icon: React.ReactNode; value: React.ReactNode; secondary?: React.ReactNode; label: string; small?: boolean }) { return <div className="min-w-0 rounded-xl border border-slate-200 bg-white p-3.5"><div>{icon}</div><p className={`${small ? "text-[11px] sm:text-xs leading-snug break-words" : "text-lg sm:text-xl truncate"} mt-3 font-black tabular-nums`}>{value}</p>{secondary && <p className="mt-1 break-words text-[10px] font-semibold leading-4 text-indigo-700">{secondary}</p>}<p className="text-[10px] font-semibold text-slate-500">{label}</p></div>; }
