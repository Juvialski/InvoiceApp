...
type PayrollTab = "overview" | "workers" | "time" | "runs" | "import";
...
function periodIdForToday(periods: PayrollPeriod[], today = new Date().toISOString().slice(0, 10)) { return selectCurrentPayrollPeriod(periods.filter((period) => period.status !== "VOID").map(scheduledPeriod), today)?.id || periods.find((period) => period.status !== "VOID")?.id || ""; }

export const PayrollPageV2: React.FC<PayrollPageV2Props> = ({ workers, assignments, periods, runs, entries, allocations, workEntries = [], projects, schedules = [], compensationProfiles = [], recurringComponents = [], importBatches = [], importTemplates = [], onSaveWorker, onSavePeriod, onSaveSchedule, onSaveCompensationProfile, onSaveRecurringComponent, onSaveAssignment, onSaveWorkEntry, onSavePayrollEntry, onUpdateRun, onCreateRun, onCalculateRun, onStagePayrollImport, onSavePayrollImportTemplate, onCommitPayrollImport }) => {
