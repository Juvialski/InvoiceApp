...
function recommendNextPeriodStart(periods: PayrollPeriod[], editing: PayrollSchedule, today: string): { recommendedStart?: string; diagnosticMessage?: string } {
  // Existing generated periods are an observation of the persisted calendar,
  // not the recurrence definition. In particular, a lone late row must not
  // advance the next schedule version past missing canonical periods.
  const current = periods.find((period) => period.status !== "VOID" && period.periodStart <= today && period.periodEnd >= today);
  const recommendationDate = current ? addDateDays(current.periodEnd, 1) : today;
  const knownProblem = configurationDiagnostic(editing);
  if (knownProblem) return { diagnosticMessage: knownProblem };
  try {
    const generated = findFirstGeneratablePayrollPeriod(recommendationCandidate(editing, recommendationDate), recommendationDate);
...
function buildCandidateSchedule(editing: PayrollSchedule, schedule: PayrollSchedule | undefined, effectiveMode: EffectiveMode, chosenDate: string, recommendedStart?: string, today?: string): PayrollSchedule {
  ...
  // A corrected next boundary can be earlier than an already-stored future
  // version (for example, repairing a stale Sep 14 row by starting Aug 31).
  // Bound the inserted version before that future version so the chain stays
  // continuous without changing the future version's identity or history.
  const followingVersion = versions
    .filter((version) => version.active && version.effectiveFrom > nextVersion.effectiveFrom)
    .sort((left, right) => left.effectiveFrom.localeCompare(right.effectiveFrom) || left.version - right.version)[0];
  const boundedNextVersion = followingVersion && (!nextVersion.effectiveTo || nextVersion.effectiveTo >= followingVersion.effectiveFrom)
    ? { ...nextVersion, effectiveTo: addDateDays(followingVersion.effectiveFrom, -1) }
    : nextVersion;
  const sameEffective = versions.findIndex((version) => version.effectiveFrom === boundedNextVersion.effectiveFrom);
  const closePriorVersion = (candidates: PayrollScheduleVersion[]) => {
    const prior = candidates
      .filter((version) => version.active && version.effectiveFrom < boundedNextVersion.effectiveFrom)
      .sort((left, right) => left.effectiveFrom.localeCompare(right.effectiveFrom) || left.version - right.version)
      .at(-1);
    return prior
      ? candidates.map((version) => version.id === prior.id ? { ...version, effectiveTo: addDateDays(boundedNextVersion.effectiveFrom, -1) } : version)
      : candidates;
  };
  if (sameEffective >= 0) {
    const closedVersions = closePriorVersion(versions);
    closedVersions[sameEffective] = { ...closedVersions[sameEffective]!, ...boundedNextVersion };
    versions.splice(0, versions.length, ...closedVersions);
  } else {
    const closedVersions = closePriorVersion(versions);
    closedVersions.push(boundedNextVersion);
    versions.splice(0, versions.length, ...closedVersions);
  }
...
export const PayrollScheduleSettings: React.FC<PayrollScheduleSettingsProps> = ({ schedule, periods = [], onSave, canManage = true }) => {
  ...
  return <>
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      {schedule && <div className="mt-4 grid grid-cols-2 gap-3 text-xs sm:gap-4"><div><p className="text-[10px] font-semibold text-slate-500">Frequency</p><p className="mt-1 font-black leading-snug">{schedule.frequency.replace("_", " ")}</p></div><div><p className="text-[10px] font-semibold text-slate-500">Pay date</p><p className="mt-1 font-black leading-snug">{schedule.payDateRule.type.replaceAll("_", " ")}{schedule.payDateRule.offsetDays !== undefined ? ` · ${schedule.payDateRule.offsetDays} days` : ""}</p></div><div><p className="text-[10px] font-semibold text-slate-500">Period generation</p><p className="mt-1 font-black text-emerald-700">{schedule.autoGeneratePeriods ? "Automatic" : "Manual"}</p></div><div><p className="text-[10px] font-semibold text-slate-500">Draft runs</p><p className="mt-1 font-black text-emerald-700">{schedule.autoCreateRuns ?? schedule.autoCalculate ? "Automatic" : "Manual"}</p></div></div>}
    </section>
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><form onSubmit={save} className="max-h-[92vh] w-full max-w-2xl space-y-4 overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-600">Payroll schedule</p><h3 className="mt-1 text-xl font-black">Recurring period configuration</h3><p className="mt-1 text-xs text-slate-500">Payroll workflow and project labor costing only; statutory Philippine payroll rules are not calculated here.</p></div><button type="button" onClick={() => setOpen(false)} disabled={saving} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 disabled:opacity-40"><X className="h-4 w-4" /></button></div>
      ...
      <fieldset className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3"><legend className="px-1 text-xs font-black text-indigo-900">Apply schedule change</legend><div className="mt-2 space-y-2 text-xs text-indigo-950"><label className="flex items-start gap-2"><input type="radio" name="effective-date" checked={effectiveMode === "NEXT_PERIOD"} onChange={() => setEffectiveMode("NEXT_PERIOD")} /><span><strong>Next payroll period</strong>{recommendationDiagnostic ? <span role="alert" className="block text-[10px] font-semibold text-rose-700">{recommendationDiagnostic}</span> : <span className="block text-[10px] text-indigo-800">Recommended · recurrence starts {recommendedStart ?? "…"}</span>}</span></label><label className="flex items-start gap-2"><input type="radio" name="effective-date" checked={effectiveMode === "IMMEDIATELY"} onChange={() => setEffectiveMode("IMMEDIATELY")} /><span>Immediately <span className="block text-[10px] text-indigo-800">Starts {today}; fixed periods wait for their next valid boundary.</span></span></label><label className="flex items-center gap-2"><input type="radio" name="effective-date" checked={effectiveMode === "CHOOSE_DATE"} onChange={() => setEffectiveMode("CHOOSE_DATE")} /><span>Choose date</span>{effectiveMode === "CHOOSE_DATE" && <input required type="date" value={chosenDate} min={today} onChange={(event) => setChosenDate(event.target.value)} className="field-input max-w-[10rem]" />}</label><p className="pl-6 text-[10px] text-indigo-800">The next boundary comes from this recurrence, not from an existing generated period.</p></div></fieldset>
