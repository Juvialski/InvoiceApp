...
export const TimeEntries: React.FC<TimeEntriesProps> = ({ entries, workers, projects, periods, assignments, runs, selectedPeriodId, onSave }) => {
  ...
  const save = (event: React.FormEvent) => {
    ...
    if (!editing || !editing.workerId || !editing.periodId || !editing.workDate || (editing.laborContext === "PROJECT" && !editing.projectId)) return;
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-black">Time / work entries</h3><p className="mt-1 text-xs text-slate-500">Attendance records presence; Time / Labor records project, office, overhead, or review allocation.</p></div><button onClick={openNew} disabled={!period || periodLocked || !workers.length} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-3.5 w-3.5" /> Add entry</button></div>
