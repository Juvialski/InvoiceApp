...
import { AlertTriangle, CheckCircle2, Clock3, Pencil, Plus } from "lucide-react";
import { Project, ProjectWorkerAssignment, PayrollPeriod, PayrollRun, Worker, WorkEntry } from "../../types";
...
export const TimeEntries: React.FC<TimeEntriesProps> = ({ entries, workers, projects, periods, assignments, runs, selectedPeriodId, onSave }) => {
  ...
  const openNew = () => {
    ...
    const seed = createLocalWorkEntry({ workerId: worker?.id || "", projectId: project?.id || "", periodId: period.id, workDate: period.periodStart, regularHours: 0, overtimeHours: 0, daysWorked: 0, rate: worker?.defaultRate || 0, status: "DRAFT" });
    setEditing(seed);
  };

  const openEdit = (entry: WorkEntry) => { if (!lockedPeriodIds.has(entry.periodId || "")) setEditing(entry); };
  ...
  const save = (event: React.FormEvent) => {
    ...
    if (!editing || !editing.workerId || !editing.projectId || !editing.periodId || !editing.workDate) return;
    ...
    onSave({ ...editing, rate: Number(editing.rate) || 0, regularHours: Number(editing.regularHours) || 0, overtimeHours: Number(editing.overtimeHours) || 0, daysWorked: Number(editing.daysWorked) || 0 });
    setEditing(null);
  };

  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-sm font-black">Time / work entries</h3><p className="mt-1 text-xs text-slate-500">Every entry must link to the selected period and fall within its date range.</p></div><button onClick={openNew} disabled={!period || periodLocked || !workers.length || !projects.length} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-3.5 w-3.5" /> Add entry</button></div>
    ...
    {visibleEntries.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase text-slate-500"><tr><th className="px-5 py-3">Date / worker</th><th className="px-5 py-3">Period</th><th className="px-5 py-3">Project</th><th className="px-5 py-3">Pay source</th><th className="px-5 py-3">Units</th><th className="px-5 py-3">Cost</th><th className="px-5 py-3">Status</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{visibleEntries.map((entry) => {
      ...
      return <tr key={entry.id} className={!valid ? "bg-amber-50/40" : undefined}><td className="px-5 py-3"><p className="font-bold">{entry.workDate}</p><p className="text-[10px] text-slate-500">{worker?.displayName || "Unknown worker"}</p></td><td className="px-5 py-3"><p className="font-bold">{entryPeriod ? `${entryPeriod.periodStart} – ${entryPeriod.periodEnd}` : "Not linked"}</p><p className="text-[10px] text-slate-500">{entryPeriod?.status || "Needs period"}</p></td><td className="px-5 py-3 text-slate-600">{project ? `${project.projectCode} · ${project.projectName}` : "Unknown project"}</td><td className="px-5 py-3"><p className="font-bold">{resolution ? `${resolution.payType} · ${money(resolution.rate)}` : "Unavailable"}</p><p className="text-[10px] text-slate-500">{resolution?.rateSource || "No worker rate"} rate</p></td><td className="px-5 py-3 text-slate-600">{resolution?.payType === "DAILY" ? `${entry.daysWorked || 0} days` : `${entry.regularHours || 0} regular h`}{(entry.overtimeHours || 0) > 0 ? ` · ${entry.overtimeHours} OT h` : ""}</td><td className="px-5 py-3 font-black tabular-nums">{money(cost)}</td><td className="px-5 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${valid ? "bg-emerald-50 text-emerald-700" : "bg-amber-100 text-amber-800"}`}>{valid ? entry.status : "INVALID LINK"}</span></td><td className="px-5 py-3 text-right"><button onClick={() => openEdit(entry)} disabled={locked} className="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-700 disabled:cursor-not-allowed disabled:text-slate-300"><Pencil className="h-3 w-3" /> Edit</button></td></tr>;
