...
export const PayrollAdvancedTools: React.FC<PayrollAdvancedToolsProps> = ({ schedules, periods, runs, entries, allocations, adjustments = [], workEntries, importBatches, canManageMaintenance = true, onPreview, onApply, onPreviewFactoryReset, onApplyFactoryReset }) => {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="payroll-advanced-title">
    <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-600">Advanced</p><h3 id="payroll-advanced-title" className="mt-1 text-sm font-black">Payroll data integrity</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">Repair derived calendar or unapproved payroll data. Approved, paid, and finalized history stays protected.</p></div><ShieldCheck className="h-5 w-5 shrink-0 text-emerald-600" aria-hidden="true" /></div>
    <div className="mt-4 grid gap-2 text-xs sm:grid-cols-4"><Summary label="Integrity issues" value={String(report.issues.length)} tone={report.issues.length ? "text-amber-800" : "text-emerald-700"} /><Summary label="Fixable records" value={String(fixablePeriodCount + fixableRunCount)} tone="text-indigo-700" /><Summary label="Protected payroll history" value={String(protectedHistoryCount)} tone="text-slate-700" hint="Protected includes finalized or data-bearing payroll records that maintenance will not delete automatically." /><Summary label="Retired / legacy" value={String(retiredLegacyCount)} tone="text-slate-600" /></div>
     <div className="mt-4 flex flex-wrap gap-2">{PAYROLL_MAINTENANCE_ACTIONS.map((action) => <button key={action} type="button" onClick={() => startAction(action)} disabled={busy || previewLoading} className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-50 ${action === "REPAIR" ? "bg-indigo-600 text-white" : action === "REBUILD_CALENDAR" ? "border border-indigo-200 bg-indigo-50 text-indigo-700" : "border border-rose-200 bg-rose-50 text-rose-800"}`}>{action === "REPAIR" ? <Wrench className="h-3.5 w-3.5" /> : action === "RESET_UNAPPROVED" ? <AlertTriangle className="h-3.5 w-3.5" /> : null}{ACTION_CONFIGURATION[action].title}</button>)}</div>
    <div className="mt-3 rounded-xl border border-rose-200 bg-rose-50/60 p-3">
      ...
      <p className="mt-1 text-xs text-rose-900">Deletes the entire payroll &amp; workforce domain for this company after an exact typed confirmation. A clean default schedule and canonical calendar are recreated afterwards.</p>
       <button type="button" onClick={() => startAction("FACTORY_RESET")} disabled={busy || previewLoading} className="mt-2 inline-flex items-center gap-1.5 rounded-xl border border-rose-300 bg-white px-3 py-2 text-xs font-black text-rose-800 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50"><AlertTriangle className="h-3.5 w-3.5" aria-hidden="true" />Factory reset payroll workspace…</button>
    </div>
    ...
    {toolMode !== null && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="payroll-action-title" aria-busy={previewLoading || busy}><div className="max-h-[calc(100vh-2rem)] w-full max-w-lg overflow-y-auto overscroll-contain rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-600">Preview before changes</p><h4 id="payroll-action-title" className="mt-1 text-lg font-black">{ACTION_CONFIGURATION[toolMode].title}</h4></div><button type="button" onClick={closeAction} disabled={busy} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 disabled:opacity-40" aria-label="Close payroll action preview"><X className="h-4 w-4" /></button></div><p className="mt-2 text-xs text-slate-600">{ACTION_CONFIGURATION[toolMode].description}</p>
