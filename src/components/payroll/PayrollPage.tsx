...
export const PayrollPage: React.FC<PayrollPageProps> = ({ workers, assignments, periods, runs, entries, allocations, workEntries = [], projects, costCodes = [], importBatches = [], importTemplates = [], onSaveWorker, onSavePeriod, onSaveAssignment, onSaveWorkEntry, onSavePayrollEntry, onUpdateRun, onCreateRun, onCalculateRun, onStagePayrollImport, onSavePayrollImportTemplate, onCommitPayrollImport }) => {
  ...
  return <div className="space-y-5">
    ...
    <nav aria-label="Payroll workspace sections" className="flex flex-wrap gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm">{([["workers", "Workers"], ["time", `Time entries${selectedPeriodWorkEntries.length ? ` (${selectedPeriodWorkEntries.length})` : ""}`], ["runs", `Payroll runs${selectedPeriodRuns.length ? ` (${selectedPeriodRuns.length})` : ""}`], ["import", "Import workbook"]] as Array<[PayrollTab, string]>).map(([value, label]) => <button key={value} type="button" onClick={() => setTab(value)} aria-current={tab === value ? "page" : undefined} className={`rounded-lg px-3 py-2 text-xs font-bold ${tab === value ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}>{label}</button>)}</nav>
