...
export const ProjectRfis: React.FC<{
  ...
  const refreshNotice = controller.hasLoaded && (controller.isLoading || controller.loadError) ? (
    <div role={controller.loadError ? "alert" : "status"} className={`flex items-start justify-between gap-3 rounded-xl border px-3 py-2 text-xs ${controller.loadError ? "border-rose-200 bg-rose-50 text-rose-800" : "border-indigo-100 bg-indigo-50 text-indigo-800"}`}>
      <span className="min-w-0 break-words leading-5">{controller.loadError ? `Could not refresh the RFI register. Showing the last successful records. ${controller.loadError}` : "Refreshing the RFI register… Existing records remain available."}</span>
      {controller.loadError && <button type="button" onClick={controller.retryLoad} className="min-h-10 shrink-0 rounded-lg bg-white px-2.5 py-1.5 text-[10px] font-black text-rose-800 shadow-sm">Retry</button>}
    </div>
  ) : null;
      ...
      <section className="space-y-4" data-phase1b="rfi-detail">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <h2 className="mt-2 break-words text-lg font-black text-slate-950">{selected.subject}</h2>
        ...
        {actionError && <div role="alert" className="break-words rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs font-semibold text-rose-800">{actionError}</div>}
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,.85fr)]">
          <div className="space-y-4">
            <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-3 break-words whitespace-pre-wrap text-sm leading-6 text-slate-800">{selected.question}</p>
  ...
  return (
    <section className="space-y-4" data-phase1b="rfi-register">
      ...
      <div className="grid min-w-0 gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:grid-cols-2 lg:grid-cols-[minmax(220px,1fr)_160px_180px_140px]">
        <label className="relative min-w-0"><Search aria-hidden="true" className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><input aria-label="Search RFIs" value={query} onChange={(e) => setQuery(e.target.value)} className={`${fieldClass} pl-9`} placeholder="Search number, subject, question…" /></label>
      ...
      {filtered.length ? <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="hidden grid-cols-[120px_minmax(240px,1fr)_140px_110px_130px_30px] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-[0.1em] text-slate-500 lg:grid"><span>RFI</span><span>Subject</span><span>Discipline</span><span>Status</span><span>Due</span><span /></div>{filtered.map((rfi) => { const due = dueLabel(rfi, today); return <button type="button" key={rfi.id} onClick={() => selectRfi(rfi.id)} className="grid w-full gap-2 border-b border-slate-100 px-4 py-4 text-left last:border-b-0 hover:bg-slate-50 lg:grid-cols-[120px_minmax(240px,1fr)_140px_110px_130px_30px] lg:items-center lg:gap-3"><span className="text-xs font-black text-slate-900">{rfi.rfiNumber}</span><span className="min-w-0"><span className="block truncate text-sm font-bold text-slate-800" title={rfi.subject}>{rfi.subject}</span><span className="mt-1 block truncate text-[10px] text-slate-500" title={rfi.question}>{rfi.question}</span></span><span className="text-[10px] font-bold text-slate-500">{statusLabel(rfi.discipline)}</span><span><span className={`inline-flex rounded-full border px-2 py-1 text-[10px] font-black ${statusClass[rfi.status]}`}>{statusLabel(rfi.status)}</span></span><span className={`text-[10px] font-bold ${due?.className || "text-slate-400"}`}>{due?.text || "No due date"}</span><ChevronRight className="hidden h-4 w-4 text-slate-300 lg:block" /></button>; })}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center"><MessageSquareText className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-black text-slate-700">No RFIs match this view.</p><p className="mt-1 text-xs text-slate-500">Create a draft RFI or clear the register filters.</p></div>}
