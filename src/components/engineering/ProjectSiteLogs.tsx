...
import { AlertTriangle, CalendarDays, CheckCircle2, ClipboardCheck, CloudRain, Cog, FileText, HardHat, History, Package, Plus, Save, ShieldAlert, Users, X } from "lucide-react";
import type { Project, ProjectCostCode, ProjectEquipment, ProjectMaterial, PurchaseOrder, PurchaseOrderReceipt } from "../../types.ts";
...
import {
  ...
  DAILY_SITE_LOG_ISSUE_SEVERITIES,
  DAILY_SITE_LOG_ISSUE_STATUSES,
  type DailySiteLogSafetySeverity,
  type DailySiteLogIssueSeverity,
  type DailySiteLogIssueStatus,
...
import { formatCostCodeOptionLabel } from "../../lib/projectCostCodes.ts";
...
function emptyForm(siteDate = today()): SiteLogForm {
  return {
    ...
    crew: [{ trade: "", crewLabel: "General site crew", contractorLabel: "", projectCostCodeId: "", headcount: 0, regularHours: "", overtimeHours: "", notes: "" }],
    equipment: [],
    work: [],
    materialDeliveries: [],
    issues: [],
...
function formFromAggregate(aggregate: EngineeringDailySiteLogAggregate): SiteLogForm {
  ...
  return {
    ...
    crew: aggregate.crew.map((row) => ({ id: row.id, trade: row.trade || "", crewLabel: row.crewLabel || "", contractorLabel: row.contractorLabel || "", projectCostCodeId: row.projectCostCodeId || "", headcount: row.headcount, regularHours: row.regularHours ?? "", overtimeHours: row.overtimeHours ?? "", notes: row.notes || "" })),
    equipment: aggregate.equipment.map((row) => ({ id: row.id, equipmentId: row.equipmentId || "", equipmentName: row.equipmentName, equipmentType: row.equipmentType || "", assetReference: row.assetReference || "", operatingHours: row.operatingHours ?? "", idleHours: row.idleHours ?? "", operatorCrewNote: row.operatorCrewNote || "", conditionStatus: row.conditionStatus || "", notes: row.notes || "" })),
    work: aggregate.work.map((row) => ({ id: row.id, description: row.description, projectCostCodeId: row.projectCostCodeId || "", quantity: row.quantity ?? "", unit: row.unit || "", workLocation: row.workLocation || "", notes: row.notes || "" })),
    materialDeliveries: aggregate.materialDeliveries.map((row) => ({ id: row.id, materialId: row.materialId || "", materialNameSnapshot: row.materialNameSnapshot, quantityObserved: row.quantityObserved, unitSnapshot: row.unitSnapshot, supplierDeliveryReference: row.supplierDeliveryReference || "", purchaseOrderId: row.purchaseOrderId || "", purchaseOrderLineId: row.purchaseOrderLineId || "", purchaseOrderReceiptId: row.purchaseOrderReceiptId || "", deliveryCondition: row.deliveryCondition || "", location: row.location || "", projectCostCodeId: row.projectCostCodeId || "", notes: row.notes || "" })),
    issues: aggregate.issues.map((row) => ({ id: row.id, category: row.category, description: row.description, severity: row.severity, status: row.status, mitigation: row.mitigation || "", responsibleParty: row.responsibleParty || "", projectCostCodeId: row.projectCostCodeId || "", resolvedAt: row.resolvedAt || "", notes: row.notes || "" })),
...
function formToInput(form: SiteLogForm) {
  return {
    ...
    crew: form.crew.map((row, index) => ({ ...row, projectCostCodeId: row.projectCostCodeId || null, headcount: Number(row.headcount || 0), regularHours: numberOrUndefined(row.regularHours), overtimeHours: numberOrUndefined(row.overtimeHours), sortOrder: index })),
    equipment: form.equipment.map((row, index) => ({ ...row, equipmentId: row.equipmentId || null, operatingHours: numberOrUndefined(row.operatingHours), idleHours: numberOrUndefined(row.idleHours), sortOrder: index })),
    work: form.work.map((row, index) => ({ ...row, projectCostCodeId: row.projectCostCodeId || null, quantity: numberOrUndefined(row.quantity), sortOrder: index })),
    materialDeliveries: form.materialDeliveries.map((row, index) => ({ ...row, materialId: row.materialId || null, quantityObserved: numberOrUndefined(row.quantityObserved), purchaseOrderId: row.purchaseOrderId || null, purchaseOrderLineId: row.purchaseOrderLineId || null, purchaseOrderReceiptId: row.purchaseOrderReceiptId || null, projectCostCodeId: row.projectCostCodeId || null, sortOrder: index })),
    issues: form.issues.map((row, index) => ({ ...row, projectCostCodeId: row.projectCostCodeId || null, resolvedAt: row.resolvedAt || undefined, sortOrder: index })),
...
function DetailItem({ label, children }: { label: string; children: React.ReactNode }) {
  return <div><p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">{label}</p><div className="mt-1 break-words text-sm font-semibold text-slate-800">{children}</div></div>;
}

function StructuredSiteLogDetails({ aggregate, canReadProcurement }: { aggregate: EngineeringDailySiteLogAggregate; canReadProcurement: boolean }) {
  return (
    <div className="grid gap-4 xl:grid-cols-3" data-phase3c="structured-site-log-details">
      <section className={sectionClass}><div className="flex items-center gap-2"><ClipboardCheck className="h-4 w-4 text-emerald-600" /><h3 className="text-sm font-black text-slate-900">Work accomplished</h3></div>{aggregate.work.length ? <div className="mt-4 space-y-2">{aggregate.work.map((row) => <div key={row.id} className="rounded-xl border border-emerald-100 bg-emerald-50/40 p-3"><p className="text-xs font-black text-slate-900">{row.description}</p><p className="mt-1 text-[10px] text-slate-600">{[row.quantity !== undefined ? `${row.quantity} ${row.unit || ""}`.trim() : "Quantity not recorded", row.workLocation, row.projectCostCodeId ? "Cost coded" : "No cost code"].filter(Boolean).join(" · ")}</p>{row.notes && <p className="mt-2 text-[10px] leading-4 text-slate-600">{row.notes}</p>}</div>)}</div> : <p className="mt-4 text-xs text-slate-500">No structured work rows recorded.</p>}</section>
      <section className={sectionClass}><div className="flex items-center gap-2"><Package className="h-4 w-4 text-cyan-600" /><h3 className="text-sm font-black text-slate-900">Materials / deliveries</h3></div>{aggregate.materialDeliveries.length ? <div className="mt-4 space-y-2">{aggregate.materialDeliveries.map((row) => <div key={row.id} className="rounded-xl border border-cyan-100 bg-cyan-50/40 p-3"><p className="text-xs font-black text-slate-900">{row.materialNameSnapshot}</p><p className="mt-1 text-[10px] text-slate-600">{row.quantityObserved} {row.unitSnapshot} · {row.location || "Location not recorded"}</p><p className="mt-2 text-[10px] font-semibold text-cyan-900">{row.supplierDeliveryReference || "No delivery reference"}{row.deliveryCondition ? ` · ${row.deliveryCondition}` : ""}</p><p className="mt-1 text-[10px] text-slate-500">{row.purchaseOrderReceiptId ? canReadProcurement ? "Formal receipt linked" : "Formal receipt reference restricted" : "No formal receipt linked; observation remains separate."}</p></div>)}</div> : <p className="mt-4 text-xs text-slate-500">No material delivery observations recorded.</p>}</section>
      <section className={sectionClass}><div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-amber-600" /><h3 className="text-sm font-black text-slate-900">Issues / delays</h3></div>{aggregate.issues.length ? <div className="mt-4 space-y-2">{aggregate.issues.map((row) => <div key={row.id} className="rounded-xl border border-amber-100 bg-amber-50/50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-black text-amber-950">{conditionLabel(row.category)} · {conditionLabel(row.severity)}</p><span className={`rounded-full border px-2 py-1 text-[9px] font-black ${row.status === "RESOLVED" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-100 text-amber-900"}`}>{conditionLabel(row.status)}</span></div><p className="mt-2 text-xs leading-5 text-slate-700">{row.description}</p>{row.mitigation && <p className="mt-2 text-[10px] leading-4 text-slate-600">Action: {row.mitigation}</p>}{row.responsibleParty && <p className="mt-1 text-[10px] text-slate-500">Responsible: {row.responsibleParty}</p>}</div>)}</div> : <p className="mt-4 text-xs text-slate-500">No structured issues or delays recorded.</p>}</section>
    </div>
  );
}

function Editor({ form, setForm, onSubmit, onCancel, saving, error, title, materials = [], registeredEquipment = [], purchaseOrders = [], receipts = [], costCodes = [], canReadProcurement = false }: { form: SiteLogForm; setForm: React.Dispatch<React.SetStateAction<SiteLogForm>>; onSubmit: () => void; onCancel: () => void; saving: boolean; error: string | null; title: string; materials?: readonly ProjectMaterial[]; registeredEquipment?: readonly ProjectEquipment[]; purchaseOrders?: readonly PurchaseOrder[]; receipts?: readonly PurchaseOrderReceipt[]; costCodes?: readonly ProjectCostCode[]; canReadProcurement?: boolean }) {
  const update = <K extends keyof SiteLogForm>(key: K, value: SiteLogForm[K]) => setForm((current) => ({ ...current, [key]: value }));
  const projectCostCodes = costCodes;
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/45 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label={title}>
      <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }} className="max-h-[min(94vh,60rem)] min-h-0 w-full max-w-4xl overflow-y-auto overscroll-contain rounded-t-3xl bg-white p-4 shadow-2xl sm:rounded-3xl sm:p-6">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(280px,.6fr)]">
          <div className="space-y-4">
            ...
            <section className={sectionClass}>
              <div className="flex items-start gap-3"><ClipboardCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-600" /><div><h3 className="text-sm font-black text-slate-900">Work accomplished</h3><p className="mt-1 text-xs text-slate-500">Structured field quantities and locations; this does not calculate earned value or schedule progress.</p></div></div>
              <div className="mt-4 space-y-3">{form.work.map((row, index) => <div key={row.id || index} className="rounded-xl border border-emerald-100 bg-emerald-50/40 p-3"><div className="grid gap-3 sm:grid-cols-2"><label className="block sm:col-span-2"><span className={labelClass}>Work description</span><input required className={inputClass} value={row.description} onChange={(event) => update("work", form.work.map((item, itemIndex) => itemIndex === index ? { ...item, description: event.target.value } : item))} placeholder="Install pipe supports at grid B4" /></label><label className="block sm:col-span-2"><span className={labelClass}>Project cost code (optional)</span><select className={inputClass} value={row.projectCostCodeId} onChange={(event) => update("work", form.work.map((item, itemIndex) => itemIndex === index ? { ...item, projectCostCodeId: event.target.value } : item))}><option value="">No cost code</option>{projectCostCodes.map((costCode) => <option key={costCode.id} value={costCode.id}>{formatCostCodeOptionLabel(costCode)}</option>)}</select></label><label className="block"><span className={labelClass}>Quantity</span><input type="number" min="0" step="0.0001" className={inputClass} value={row.quantity} onChange={(event) => update("work", form.work.map((item, itemIndex) => itemIndex === index ? { ...item, quantity: formNumber(event.target.value) } : item))} /></label><label className="block"><span className={labelClass}>Unit</span><input className={inputClass} value={row.unit} onChange={(event) => update("work", form.work.map((item, itemIndex) => itemIndex === index ? { ...item, unit: event.target.value } : item))} placeholder="m, sq.m, cu.m" /></label><label className="block sm:col-span-2"><span className={labelClass}>Work location / area</span><input className={inputClass} value={row.workLocation} onChange={(event) => update("work", form.work.map((item, itemIndex) => itemIndex === index ? { ...item, workLocation: event.target.value } : item))} placeholder="North elevation" /></label><label className="block sm:col-span-2"><span className={labelClass}>Notes</span><input className={inputClass} value={row.notes} onChange={(event) => update("work", form.work.map((item, itemIndex) => itemIndex === index ? { ...item, notes: event.target.value } : item))} /></label></div><button type="button" className="mt-2 text-xs font-black text-rose-700" onClick={() => update("work", form.work.filter((_, itemIndex) => itemIndex !== index))}>Remove work row</button></div>)}<button type="button" className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-800" onClick={() => update("work", [...form.work, { description: "", projectCostCodeId: "", quantity: "" as FormNumber, unit: "", workLocation: "", notes: "" }])}><Plus className="h-3.5 w-3.5" />Add work row</button></div>
            </section>

            <section className={sectionClass}>
              <div className="flex items-start gap-3"><Package className="mt-0.5 h-5 w-5 shrink-0 text-cyan-600" /><div><h3 className="text-sm font-black text-slate-900">Materials / deliveries</h3><p className="mt-1 text-xs text-slate-500">Record what was observed arriving. This never creates or changes a formal Procurement receipt.</p></div></div>
              <div className="mt-4 space-y-3">{form.materialDeliveries.map((row, index) => { const selectedMaterial = materials.find((material) => material.id === row.materialId); const availableReceipts = canReadProcurement ? receipts.filter((receipt) => receipt.status === "RECEIVED" && receipt.purchaseOrderId === row.purchaseOrderId && (receipt.lines || []).some((receiptLine) => receiptLine.purchaseOrderLineId === row.purchaseOrderLineId)) : []; return <div key={row.id || index} className="rounded-xl border border-cyan-100 bg-cyan-50/40 p-3"><div className="grid gap-3 sm:grid-cols-2"><label className="block sm:col-span-2"><span className={labelClass}>Registered material</span><select className={inputClass} value={row.materialId} onChange={(event) => { const material = materials.find((item) => item.id === event.target.value); update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, materialId: material?.id || "", materialNameSnapshot: material?.materialName || item.materialNameSnapshot, unitSnapshot: material?.unit || item.unitSnapshot, purchaseOrderId: canReadProcurement ? material?.purchaseOrderId || "" : item.purchaseOrderId, purchaseOrderLineId: canReadProcurement ? material?.purchaseOrderLineId || "" : item.purchaseOrderLineId, projectCostCodeId: material?.projectCostCodeId || item.projectCostCodeId } : item)); }}>{materials.length ? <option value="">Select a material or use the snapshot fields</option> : <option value="">No material register rows available</option>}{materials.map((material) => <option key={material.id} value={material.id}>{material.materialName}{material.referenceCode ? ` · ${material.referenceCode}` : ""}</option>)}</select></label><label className="block sm:col-span-2"><span className={labelClass}>Material description snapshot</span><input required className={inputClass} value={row.materialNameSnapshot} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, materialNameSnapshot: event.target.value } : item))} placeholder="Material as observed on site" /></label><label className="block"><span className={labelClass}>Quantity observed</span><input required type="number" min="0.0001" step="0.0001" className={inputClass} value={row.quantityObserved} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, quantityObserved: formNumber(event.target.value) } : item))} /></label><label className="block"><span className={labelClass}>Unit snapshot</span><input required className={inputClass} value={row.unitSnapshot} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, unitSnapshot: event.target.value } : item))} placeholder="pcs / cu.m" /></label><label className="block"><span className={labelClass}>Supplier / delivery reference</span><input className={inputClass} value={row.supplierDeliveryReference} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, supplierDeliveryReference: event.target.value } : item))} placeholder="Delivery ticket" /></label><label className="block"><span className={labelClass}>Delivery condition</span><input className={inputClass} value={row.deliveryCondition} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, deliveryCondition: event.target.value } : item))} placeholder="Accepted / quarantined" /></label><label className="block"><span className={labelClass}>Location</span><input className={inputClass} value={row.location} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, location: event.target.value } : item))} placeholder="Laydown area" /></label><label className="block"><span className={labelClass}>Project cost code (optional)</span><select className={inputClass} value={row.projectCostCodeId} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, projectCostCodeId: event.target.value } : item))}><option value="">No cost code</option>{projectCostCodes.map((costCode) => <option key={costCode.id} value={costCode.id}>{formatCostCodeOptionLabel(costCode)}</option>)}</select></label>{canReadProcurement && <label className="block sm:col-span-2"><span className={labelClass}>Formal receipt reference (optional)</span><select className={inputClass} value={row.purchaseOrderReceiptId} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, purchaseOrderReceiptId: event.target.value } : item))}><option value="">No formal receipt linked</option>{availableReceipts.map((receipt) => <option key={receipt.id} value={receipt.id}>{receipt.receiptNumber} · {receipt.receiptDate}</option>)}</select></label>}<label className="block sm:col-span-2"><span className={labelClass}>Notes</span><textarea rows={2} className={`${inputClass} resize-y`} value={row.notes} onChange={(event) => update("materialDeliveries", form.materialDeliveries.map((item, itemIndex) => itemIndex === index ? { ...item, notes: event.target.value } : item))} /></label></div>{selectedMaterial && <p className="mt-2 text-[10px] leading-4 text-cyan-900">Snapshot source: {selectedMaterial.materialName} · current register edits will not rewrite this historical observation.</p>}<button type="button" className="mt-2 text-xs font-black text-rose-700" onClick={() => update("materialDeliveries", form.materialDeliveries.filter((_, itemIndex) => itemIndex !== index))}>Remove delivery row</button></div>; })}<button type="button" className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-black text-cyan-800" onClick={() => update("materialDeliveries", [...form.materialDeliveries, { materialId: "", materialNameSnapshot: "", quantityObserved: "" as FormNumber, unitSnapshot: "pcs", supplierDeliveryReference: "", purchaseOrderId: "", purchaseOrderLineId: "", purchaseOrderReceiptId: "", deliveryCondition: "", location: "", projectCostCodeId: "", notes: "" }])}><Plus className="h-3.5 w-3.5" />Add delivery row</button></div>
            </section>
        ...
        <section className={`${sectionClass} mt-4`}><div className="flex items-start gap-3"><Cog className="mt-0.5 h-5 w-5 shrink-0 text-orange-600" /><div><h3 className="text-sm font-black text-slate-900">Registered equipment links</h3><p className="mt-1 text-xs text-slate-500">Link a stable project equipment record while preserving the name, type, and reference snapshot below.</p></div></div><div className="mt-4 space-y-2">{form.equipment.length ? form.equipment.map((row, index) => <div key={row.id || index} className="rounded-xl border border-orange-100 bg-orange-50/40 p-3"><label className="block"><span className={labelClass}>Equipment register record</span><select className={inputClass} value={row.equipmentId || ""} onChange={(event) => { const registered = registeredEquipment.find((item) => item.id === event.target.value); update("equipment", form.equipment.map((item, itemIndex) => itemIndex === index ? { ...item, equipmentId: registered?.id || "", equipmentName: registered?.equipmentName || item.equipmentName, equipmentType: registered?.equipmentType || item.equipmentType, assetReference: registered?.assetReference || item.assetReference } : item)); }}><option value="">No stable register link</option>{registeredEquipment.map((item) => <option key={item.id} value={item.id}>{item.equipmentName}{item.assetReference ? ` · ${item.assetReference}` : ""}</option>)}</select></label><p className="mt-2 text-[10px] text-orange-900">Observation snapshot: {row.equipmentName || "Name not yet entered"}{row.assetReference ? ` · ${row.assetReference}` : ""}</p></div>) : <p className="text-xs text-slate-500">Add an equipment usage row above to link it to the project register.</p>}</div></section>
        <section className={`${sectionClass} mt-4`}><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" /><div><h3 className="text-sm font-black text-slate-900">Delays / operational issues</h3><p className="mt-1 text-xs text-slate-500">Record explicit field issues without inferring schedule variance or critical-path impact.</p></div></div><div className="mt-4 space-y-3">{form.issues.map((row, index) => <div key={row.id || index} className="rounded-xl border border-amber-100 bg-amber-50/50 p-3"><div className="grid gap-3 sm:grid-cols-2"><label className="block"><span className={labelClass}>Category</span><select className={inputClass} value={row.category} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, category: event.target.value } : item))}><option value="">Select category</option>{["MATERIAL", "EQUIPMENT", "MANPOWER", "WEATHER", "ACCESS", "DESIGN", "SAFETY", "CLIENT", "SUBCONTRACTOR", "OTHER"].map((value) => <option key={value} value={value}>{conditionLabel(value)}</option>)}</select></label><label className="block"><span className={labelClass}>Severity</span><select className={inputClass} value={row.severity} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, severity: event.target.value as DailySiteLogIssueSeverity } : item))}>{DAILY_SITE_LOG_ISSUE_SEVERITIES.map((value) => <option key={value} value={value}>{conditionLabel(value)}</option>)}</select></label><label className="block sm:col-span-2"><span className={labelClass}>Issue / delay description</span><textarea required rows={2} className={`${inputClass} resize-y`} value={row.description} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, description: event.target.value } : item))} placeholder="Describe the observed constraint" /></label><label className="block"><span className={labelClass}>Status</span><select className={inputClass} value={row.status} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, status: event.target.value as DailySiteLogIssueStatus, resolvedAt: event.target.value === "RESOLVED" ? item.resolvedAt || today() : "" } : item))}>{DAILY_SITE_LOG_ISSUE_STATUSES.map((value) => <option key={value} value={value}>{conditionLabel(value)}</option>)}</select></label><label className="block"><span className={labelClass}>Resolved date</span><input type="date" disabled={row.status !== "RESOLVED"} className={inputClass} value={row.resolvedAt} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, resolvedAt: event.target.value } : item))} /></label><label className="block"><span className={labelClass}>Responsible party</span><input className={inputClass} value={row.responsibleParty} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, responsibleParty: event.target.value } : item))} placeholder="Site logistics" /></label><label className="block"><span className={labelClass}>Project cost code</span><select className={inputClass} value={row.projectCostCodeId} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, projectCostCodeId: event.target.value } : item))}><option value="">No cost code</option>{projectCostCodes.map((costCode) => <option key={costCode.id} value={costCode.id}>{formatCostCodeOptionLabel(costCode)}</option>)}</select></label><label className="block sm:col-span-2"><span className={labelClass}>Mitigation / action</span><input className={inputClass} value={row.mitigation} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, mitigation: event.target.value } : item))} /></label></div><label className="mt-3 block"><span className={labelClass}>Notes</span><textarea rows={2} className={`${inputClass} resize-y`} value={row.notes} onChange={(event) => update("issues", form.issues.map((item, itemIndex) => itemIndex === index ? { ...item, notes: event.target.value } : item))} /></label><button type="button" className="mt-2 text-xs font-black text-rose-700" onClick={() => update("issues", form.issues.filter((_, itemIndex) => itemIndex !== index))}>Remove issue row</button></div>)}<button type="button" className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-800" onClick={() => update("issues", [...form.issues, { category: "", description: "", severity: "MEDIUM", status: "OPEN", mitigation: "", responsibleParty: "", projectCostCodeId: "", resolvedAt: "", notes: "" }])}><Plus className="h-3.5 w-3.5" />Add issue / delay</button></div></section>
        <div className="mt-5 flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end"><button type="button" onClick={onCancel} disabled={saving} className="min-h-11 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-black text-slate-700 hover:bg-slate-50 disabled:opacity-50 sm:w-auto">Cancel</button><button type="submit" disabled={saving} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-black text-white shadow-sm hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"><Save className="h-4 w-4" />{saving ? "Saving draft…" : "Save draft"}</button></div>
      </form>
    </div>
  );
}
...
export const ProjectSiteLogs: React.FC<ProjectSiteLogsProps> = ({ project, companyId, canRead = false, canCreate = false, canUpdate = false, canSubmit = false, canManage = false, guestMode = false, initialSiteLogId, pathForSiteLog, onNavigatePath, controlledData, onControlledDataChange, controlledPersistence, materials = [], registeredEquipment = [], purchaseOrders = [], receipts = [], costCodes = [], canReadProcurement = false }) => {
  ...
  const selectLog = (id?: string, replace = false) => {
    setSelectedId(id);
    setActionError(null);
    navigate(pathForSiteLog ? pathForSiteLog(id) : id ? appPathForProject(project.id, "site-logs", { siteLogId: id }) : appPathForProject(project.id, "site-logs"), replace);
  ...
  const openLifecycleReview = async () => {
    ...
    setBusy(true); setActionError(null);
    try { setLifecyclePreview(await controller.previewLifecycle(selected.log.id)); } catch (error) { setActionError(error instanceof Error ? error.message : "The Site Log lifecycle preview could not be loaded."); } finally { setBusy(false); }
  };

  const applyLifecycleAction = (action: EngineeringLifecycleAction, reason?: string, correctionText?: string) => {
    if (!selected || (action !== "DELETE_UNUSED" && action !== "VOID" && action !== "ADDENDUM")) return;
    void runAction(async () => {
      if (action === "ADDENDUM") await controller.addAddendum(selected.log.id, reason || "Correction addendum", correctionText || "");
      else {
        const result = await controller.applyLifecycle(selected.log.id, action, reason);
        if (result.deleted) selectLog(undefined, true);
      }
      setLifecyclePreview(null);
    }, "The Site Log lifecycle action could not be completed.");
  };
  ...
  if (controller.isLoading && !controller.hasLoaded) return <div role="status" className={`${sectionClass} p-10 text-center text-sm font-semibold text-slate-600`}>Loading Site Logs…</div>;
  if (controller.loadError && !controller.hasLoaded) return <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm text-rose-900"><p className="font-black">Site Log register unavailable</p><p className="mt-1 text-xs leading-5">{controller.loadError}</p><button type="button" onClick={controller.retryLoad} className="mt-3 min-h-10 rounded-xl bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm">Retry</button></div>;
  if (selectedId && !selected) return <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950"><p className="font-black">Site Log not available</p><p className="mt-1 text-xs leading-5">The requested field record is not available in this project or company.</p><button type="button" onClick={() => selectLog(undefined, true)} className="mt-3 min-h-10 rounded-xl bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm">Return to register</button></div>;
  ...
  const refreshNotice = controller.hasLoaded && (controller.isLoading || controller.loadError) ? (
    <div role={controller.loadError ? "alert" : "status"} className={`flex items-center justify-between gap-3 rounded-xl border px-3 py-2 text-xs ${controller.loadError ? "border-rose-200 bg-rose-50 text-rose-800" : "border-indigo-100 bg-indigo-50 text-indigo-800"}`}>
      <span className="min-w-0 break-words leading-5">{controller.loadError ? `Could not refresh Site Logs. Showing the last successful records. ${controller.loadError}` : "Refreshing Site Logs… Existing records remain available."}</span>
  ...
  if (selected) {
    ...
    return <section className="space-y-4" data-phase1c="site-log-detail">
      ...
      <StructuredSiteLogDetails aggregate={selected} canReadProcurement={canReadProcurement} />
