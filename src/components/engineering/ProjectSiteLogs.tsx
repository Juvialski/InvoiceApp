...
export const ProjectSiteLogs: React.FC<ProjectSiteLogsProps> = ({ project, companyId, canRead = true, canCreate = true, canUpdate = true, canSubmit = true, canManage = true, guestMode = false, initialSiteLogId, pathForSiteLog, onNavigatePath, controlledData, onControlledDataChange }) => {
  ...
  const refreshNotice = controller.hasLoaded && (controller.isLoading || controller.loadError) ? (
    <div role={controller.loadError ? "alert" : "status"} className={`flex items-center justify-between gap-3 rounded-xl border px-3 py-2 text-xs ${controller.loadError ? "border-rose-200 bg-rose-50 text-rose-800" : "border-indigo-100 bg-indigo-50 text-indigo-800"}`}>
      <span>{controller.loadError ? `Could not refresh Site Logs. Showing the last successful records. ${controller.loadError}` : "Refreshing Site Logs… Existing records remain available."}</span>
