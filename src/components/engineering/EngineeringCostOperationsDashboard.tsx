...
export const EngineeringCostOperationsDashboard: React.FC<EngineeringCostOperationsDashboardProps> = ({ data, projects, selectedProjectId, onProjectChange, onActivityPeriodChange, onCustomRangeChange, onCurrencyChange, onNavigate, onOpenProject, onOpenInvoice }) => {
  ...
  return <div className="space-y-5">
    ...
    <ChartCard title="Project budget position" description="Lifetime position, sorted by financial pressure. Remaining never goes below zero; excess shows as a separate extension.">
      {!hasBudgetRows ? <ChartEmpty message="No active projects yet." /> : <><div className="h-[300px] w-full sm:h-[360px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.projectRows} layout="vertical" margin={{ top: 4, right: 18, bottom: 4, left: 4 }} barCategoryGap="20%"><CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e2e8f0" /><XAxis type="number" tickFormatter={(value) => compactMoney(Number(value), data.selectedCurrency)} tick={{ fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis type="category" dataKey="projectCode" width={78} tick={{ fontSize: 10, fontWeight: 700 }} axisLine={false} tickLine={false} /><Tooltip content={renderTooltip(data.selectedCurrency, { confirmed: "Confirmed", committed: "Committed", pending: "Pending exposure", remaining: "Remaining", excess: "Over commitment" })} /><Legend wrapperStyle={{ fontSize: 10 }} /><Bar dataKey="confirmed" stackId="position" fill="#4f46e5" name="Confirmed" /><Bar dataKey="committed" stackId="position" fill="#7c3aed" name="Committed" /><Bar dataKey="pending" stackId="position" fill="#f59e0b" name="Pending exposure" /><Bar dataKey="remaining" stackId="position" fill="#cbd5e1" name="Remaining" /><Bar dataKey="excess" stackId="position" fill="#e11d48" name="Over commitment" /></BarChart></ResponsiveContainer></div><div className="mt-3 grid gap-2 text-[10px] text-slate-500 sm:grid-cols-4"><p><strong className="text-slate-700">Confirmed</strong> is lifetime project cost.</p><p><strong className="text-slate-700">Committed</strong> is approved PO and subcontract liability.</p><p><strong className="text-slate-700">Pending</strong> is unconfirmed exposure.</p><p><strong className="text-slate-700">Available</strong> is shown in each project row below.</p></div></>}
    </ChartCard>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><h2 className="text-sm font-black">Supplier document operations</h2><p className="mt-1 text-xs text-slate-500">Intake, review, source-currency amounts, and PHP tax reporting stay connected to the supplier-document workflow.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => onNavigate("extractor")} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white"><Receipt className="h-3 w-3" /> Upload</button><button type="button" onClick={() => onNavigate("review")} className="inline-flex items-center gap-1.5 rounded-xl bg-amber-400 px-3 py-2 text-xs font-bold text-amber-950"><AlertTriangle className="h-3 w-3" /> Review {data.invoiceOperations.needsReviewCount}</button><button type="button" onClick={() => onNavigate("inbox")} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700"><Mail className="h-3 w-3" /> Process email</button></div></div><div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6"><SummaryValue label="Source document value" value={money(invoiceCurrencyTotal, data.selectedCurrency)} /><SummaryValue label="Outstanding" value={money(invoiceCurrencyOutstanding, data.selectedCurrency)} /><SummaryValue label="Overdue" value={String(data.invoiceOperations.overdueCount)} /><SummaryValue label="VAT amount" value={money(data.invoiceOperations.phpVat, "PHP")} /><SummaryValue label="Needs review" value={String(data.invoiceOperations.needsReviewCount)} /><SummaryValue label="PHP FX required" value={String(data.invoiceOperations.phpFxRequired)} /></div><div className="mt-4 grid gap-4 lg:grid-cols-2"><div className="rounded-xl bg-slate-50 p-3"><h3 className="text-xs font-black">Currency breakdown</h3>{Object.keys(data.invoiceOperations.totalsByCurrency).length ? <div className="mt-3 space-y-2">{Object.entries(data.invoiceOperations.totalsByCurrency).map(([currency, value]) => <div key={currency} className="flex items-center justify-between gap-3 text-xs"><span className="font-bold text-slate-700">{currency}</span><span className="font-black tabular-nums">{money(Number(value) || 0, currency)} <span className="font-semibold text-slate-400">outstanding {money(data.invoiceOperations.outstandingByCurrency[currency] || 0, currency)}</span></span></div>)}</div> : <p className="mt-3 text-xs text-slate-500">No supplier-document activity yet.</p>}</div><div className="rounded-xl bg-slate-50 p-3"><h3 className="text-xs font-black">Philippine VAT summary</h3><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><SummaryValue label="VATable purchases" value={money(data.invoiceOperations.phpVatable, "PHP")} /><SummaryValue label="VAT amount" value={money(data.invoiceOperations.phpVat, "PHP")} /><SummaryValue label="Zero-rated" value={money(data.invoiceOperations.phpZeroRated, "PHP")} /><SummaryValue label="VAT-exempt" value={money(data.invoiceOperations.phpExempt, "PHP")} /><SummaryValue label="Missing details" value={String(data.invoiceOperations.phpMissingVatDetails)} /><SummaryValue label="FX required" value={String(data.invoiceOperations.phpFxRequired)} /></div></div></div>{data.invoiceOperations.recent.length > 0 && <div className="mt-4 border-t border-slate-200 pt-4"><h3 className="text-xs font-black">Recent supplier-document activity</h3><div className="mt-2 space-y-1.5">{data.invoiceOperations.recent.slice(0, 5).map((invoice) => <button type="button" key={invoice.id} onClick={() => onOpenInvoice(invoice)} className="flex w-full items-center justify-between gap-3 rounded-xl p-2.5 text-left hover:bg-slate-50"><span className="min-w-0"><strong className="block truncate text-xs text-slate-800">{invoice.invoiceNumber || "Supplier document"}</strong><span className="block truncate text-xs text-slate-500">{invoice.invoiceDate || "Date not set"} · {invoice.reviewStatus === "VERIFIED" ? "Verified" : "Needs review"}</span></span><span className="shrink-0 text-xs font-black tabular-nums">{money(invoice.grandTotal, invoice.currency || data.selectedCurrency)}</span></button>)}</div></div>}</section>
  </div>;
};
