...
export const EngineeringCostOperationsDashboard: React.FC<EngineeringCostOperationsDashboardProps> = ({ data, projects, selectedProjectId, onProjectChange, onActivityPeriodChange, onCustomRangeChange, onCurrencyChange, onNavigate, onOpenProject, onOpenInvoice }) => {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-5" aria-label="Supporting dashboard metrics">
      ...
      <OperationsMetricCard label="Committed cost" value={money(data.committedProjectCost, data.selectedCurrency)} detail="Approved PO and subcontract obligations" icon={ShoppingCart} tone="info" />
      <OperationsMetricCard label="Pending cost exposure" value={money(data.pendingProjectCost, data.selectedCurrency)} detail="Review, draft, calculated" icon={Clock3} tone="warning" />
      <OperationsMetricCard label="Available after commitments" value={money(data.availableAfterCommitments, data.selectedCurrency)} detail="Budget less actual, committed, and pending" icon={BarChart3} tone={data.availableAfterCommitments < 0 ? "danger" : "info"} />
    </div>
