...
export const ProjectSubmittals: React.FC<{
  ...
  guestMode?: boolean;
}> = ({ project, companyId, initialSubmittalId, initialRoundId, canRead = true, canCreate = true, canReview = true, canManage = true, canReadDocuments = true, guestMode = false }) => {
  ...
  const selectSubmittal = (id: string, roundId?: string) => {
    ...
    if (typeof window !== "undefined") window.history.replaceState({}, "", appPathForProject(project.id, "submittals", { submittalId: id, roundId }));
  };
  ...
  if (controller.isLoading) return <div role="status" className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm font-semibold text-slate-600">Loading submittal register…</div>;
  if (controller.loadError) return <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm text-rose-800"><p className="font-bold">Submittal register unavailable</p><p className="mt-1 text-xs">{controller.loadError}</p><button type="button" className="mt-3 rounded-lg bg-white px-3 py-2 text-xs font-bold shadow-sm" onClick={controller.retryLoad}>Retry</button></div>;
      ...
      <section className="space-y-4" data-phase1b="submittal-detail">
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(330px,.85fr)]">
          ...
          <aside className="space-y-4">
            ...
            {!['CLOSED','VOID'].includes(selected.status) && canManage && <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-bold text-slate-600">Preserve this submittal as void history?</p><button type="button" disabled={busy} onClick={() => { const reason = typeof window === "undefined" ? "Voided by manager" : window.prompt("Reason for voiding this formal submittal?") || ""; if (reason.trim()) void run(() => controller.voidSubmittal(selected, reason)); }} className="mt-3 text-xs font-black text-rose-700">Void submittal</button></article>}
          </aside>
        </div>
      </section>
    );
