...
export const ProjectSubmittals: React.FC<{
  ...
  const refreshNotice = controller.hasLoaded && (controller.isLoading || controller.loadError) ? (
    <div role={controller.loadError ? "alert" : "status"} className={`flex items-start justify-between gap-3 rounded-xl border px-3 py-2 text-xs ${controller.loadError ? "border-rose-200 bg-rose-50 text-rose-800" : "border-indigo-100 bg-indigo-50 text-indigo-800"}`}>
      <span className="min-w-0 break-words leading-5">{controller.loadError ? `Could not refresh the submittal register. Showing the last successful records. ${controller.loadError}` : "Refreshing the submittal register… Existing records remain available."}</span>
  ...
  return (
    <section className="space-y-4" data-phase1b="submittal-register">
      ...
      <div className="grid min-w-0 gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:grid-cols-2 lg:grid-cols-[minmax(240px,1fr)_190px_190px]"><label className="relative min-w-0"><Search aria-hidden="true" className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><input aria-label="Search submittals" className={`${fieldClass} pl-9`} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search number, title, category…" /></label><select className={fieldClass} value={status} onChange={(e) => setStatus(e.target.value as SubmittalStatus | "ALL")}><option value="ALL">All statuses</option>{STATUSES.map((value) => <option key={value} value={value}>{label(value)}</option>)}</select><select className={fieldClass} value={discipline} onChange={(e) => setDiscipline(e.target.value as DisciplineType | "ALL")}><option value="ALL">All disciplines</option>{DISCIPLINES.map((value) => <option key={value} value={value}>{label(value)}</option>)}</select></div>
