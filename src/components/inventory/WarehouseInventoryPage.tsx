...
interface ReceiptOption {
  ...
  line: NonNullable<PurchaseOrder["lines"]>[number] & { inventoryItemId?: string | null };
}
...
function ModalShell({ title, eyebrow = "Warehouse Inventory", children, onClose, busy = false, wide = false }: { title: string; eyebrow?: string; children: React.ReactNode; onClose: () => void; busy?: boolean; wide?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/45 p-0 sm:items-center sm:p-4" role="presentation">
      <section role="dialog" aria-modal="true" aria-label={title} className={`hqs-surface-raised hqs-border max-h-[min(94vh,58rem)] w-full overflow-y-auto rounded-t-3xl border p-4 shadow-2xl sm:rounded-3xl sm:p-6 ${wide ? "max-w-5xl" : "max-w-2xl"}`}>
        <div className="hqs-border flex items-start justify-between gap-4 border-b pb-4">
          <div className="min-w-0"><p className="hqs-accent-text text-[10px] font-black uppercase tracking-[0.16em]">{eyebrow}</p><h2 className="hqs-primary-text mt-1 break-words text-xl font-black">{title}</h2></div>
          <button type="button" disabled={busy} onClick={close} className="hqs-control hqs-focus-ring shrink-0 rounded-xl p-2 disabled:cursor-not-allowed disabled:opacity-50" aria-label={busy ? "Close dialog unavailable while saving" : "Close dialog"}><X className="h-5 w-5" /></button>
        </div>
...
function MovementFormModal({
  ...
  const eligibleReceiptItems = selectedReceipt
    ? items.filter((item) => item.status === "ACTIVE"
      && normalizeUnit(item.stockUnit) === normalizeUnit(selectedReceipt.line.unit)
      && (!selectedReceipt.line.inventoryItemId || item.id === selectedReceipt.line.inventoryItemId))
    : items.filter((item) => item.status === "ACTIVE");
    ...
    <ModalShell title={actionLabel(action)} onClose={onClose} busy={busy}>
      <form onSubmit={(event) => void submit(event)} className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {isProcurementReceipt && <label className="sm:col-span-2"><span className={labelClass}>Eligible procurement receipt line</span><select required className={inputClass} value={form.receiptSourceKey} onChange={(event) => { const selected = receiptOptions.find((option) => option.key === event.target.value); update("receiptSourceKey", event.target.value); if (selected) { update("quantity", String(selected.line.quantity)); update("inventoryItemId", selected.line.inventoryItemId || ""); } }}><option value="">Choose receipt line</option>{receiptOptions.map((option) => <option key={option.key} value={option.key}>{option.receipt.receiptNumber} · {option.purchaseOrder.poNumber} · line {option.line.lineNumber} · {option.line.description} · {option.line.quantity} {option.line.unit}{option.line.inventoryItemId ? " · canonical item matched" : " · item match required"}</option>)}</select>{!receiptOptions.length && <span className="mt-1 block text-[10px] text-amber-700">No eligible non-void receipt lines are available, or all have already been posted into warehouse stock.</span>}<span className="mt-1 block text-[10px] text-slate-500">The receipt quantity is fixed by Procurement. A persisted canonical match is preselected; otherwise choose an exact-unit item separately.</span></label>}
...
export const WarehouseInventoryPage: React.FC<WarehouseInventoryPageProps> = ({
  ...
  const inventoryItemIds = useMemo(() => items.map((item) => item.id), [items]);
  const { byEntityId: mediaByInventoryItemId } = useEntityMediaThumbnails("MATERIAL", inventoryItemIds, canRead);
  ...
  const receiptOptions = useMemo<ReceiptOption[]>(() => {
    ...
    return receipts.flatMap((receipt) => {
      ...
      return (receipt.lines || []).flatMap((line) => {
        ...
        const purchaseOrderLine = purchaseOrder.lines?.find((candidate) => candidate.id === line.purchaseOrderLineId);
        const resolvedLine = purchaseOrderLine
          ? { ...purchaseOrderLine, inventoryItemId: line.inventoryItemId || null }
          : { ...line, description: "Receipt line", quantity: line.receivedQuantity, unit: "", unitPrice: 0, amount: 0, purchaseOrderId: purchaseOrder.id, lineNumber: line.lineNumber, id: line.purchaseOrderLineId, inventoryItemId: line.inventoryItemId || null };
        return alreadyPosted ? [] : [{ key: `${receipt.id}::${line.purchaseOrderLineId}`, receipt, purchaseOrder, line: resolvedLine }];
      });
    });
  ...
  return (
    <section className="space-y-5" data-domain="warehouse-inventory">
      ...
      <PageHeader eyebrow="Company operations" title="Warehouse Inventory" description="Stock and custody from recorded movements; no valuation is posted here." actions={canManage ? <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setItemModal({})} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-black text-white"><Plus className="h-3.5 w-3.5" />Add item</button><button type="button" onClick={() => openMovement("OPENING")} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700"><ArrowDownToLine className="h-3.5 w-3.5" />Opening stock</button></div> : undefined} />
