...
export const SourceComparison: React.FC<SourceComparisonProps> = ({ invoice, onRevertField, mode = "full" }) => {
  ...
  return <div className={`${sourcePane ? "h-full flex flex-col" : "bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden"}`}>
    ...
    {tab === "document" && <div className={`${sourcePane ? "flex-1 min-h-0 p-3" : "p-4"}`}>
      {!invoice.previewUrl ? <div data-testid="supplier-invoice-source-document" data-source-state="unavailable" className={`${sourcePane ? "h-full" : "p-10"} rounded-xl border border-dashed border-slate-300 text-center text-xs text-slate-500 flex items-center justify-center`}>Original source is unavailable for this older invoice. The extracted editor remains usable.</div> : imageSource ? <div data-testid="supplier-invoice-source-document" data-source-state="available" className="h-full max-h-[min(78vh,52rem)] min-h-0 overflow-auto rounded-xl border border-slate-200 bg-slate-100 p-3"><div className="flex min-h-full items-start justify-center" style={{ width: zoom === 1 ? "100%" : `${zoom * 100}%` }}><img src={invoice.previewUrl} alt={invoice.fileName || "Original invoice"} className="max-w-none rounded-lg border border-slate-200 bg-white object-contain shadow-sm" style={{ width: zoom === 1 ? "100%" : "100%" }} /></div></div> : <div data-testid="supplier-invoice-source-document" data-source-state="available" className="h-full min-h-0 flex max-h-[min(78vh,52rem)] flex-col gap-2"><div className="flex items-center justify-end"><a href={invoice.previewUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-bold text-indigo-700 hover:text-indigo-900"><ExternalLink className="w-3.5 h-3.5" />Open original</a></div><iframe title="Original invoice document" src={invoice.previewUrl} className={`w-full flex-1 min-h-[440px] ${sourcePane ? "h-full" : "h-[min(78vh,52rem)]"} rounded-xl border border-slate-200 bg-slate-50`} /></div>}
    </div>}
