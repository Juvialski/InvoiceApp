...
import { normalizedInvoiceAllocationAmount, validateInvoiceAllocations } from "../utils/projectCosting";
...
const ProjectAssignmentPanel: React.FC<ProjectAssignmentPanelProps> = ({ invoice, projects, savedAllocations, readOnly, preferredProject, onSave }) => {
  const [allocations, setAllocations] = useState<InvoiceProjectAllocation[]>(savedAllocations);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  useEffect(() => { setAllocations(savedAllocations); setError(null); }, [invoice.id]);
  ...
  const addAllocation = (projectId?: string) => setAllocations((current) => [...current, { id: `local-${Date.now()}-${current.length}`, invoiceId: invoice.id, projectId: projectId || projects.find((project) => project.status !== "ARCHIVED")?.id || "", allocationType: "AMOUNT", allocationAmount: current.length ? 0 : invoice.grandTotal }]);
  const updateAllocation = (id: string, patch: Partial<InvoiceProjectAllocation>) => setAllocations((current) => current.map((allocation) => allocation.id === id ? { ...allocation, ...patch } : allocation));
  const save = async () => { const validation = validateInvoiceAllocations(invoice.grandTotal, allocations); if (!allocations.every((allocation) => allocation.projectId)) { setError("Choose a project for each allocation."); return; } if (!validation.valid) { setError(validation.message || "Allocation exceeds invoice total."); return; } setSaving(true); setError(null); try { await onSave(allocations); } catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Could not save allocation."); } finally { setSaving(false); } };
  ...
  return <section className="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-700">Project allocation</p><p className="mt-1 text-[10px] text-indigo-900">Project links are human-confirmed. The extracted project reference remains text only until assigned.</p></div>{!readOnly && <button type="button" onClick={() => addAllocation()} disabled={!projects.some((project) => project.status !== "ARCHIVED")} className="inline-flex items-center gap-1 rounded-lg bg-indigo-600 px-2.5 py-1.5 text-[10px] font-bold text-white disabled:opacity-50"><Plus className="h-3 w-3" /> Add allocation</button>}</div>{suggestions.length > 0 && allocations.length === 0 && <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2"><div><p className="text-[10px] font-bold text-slate-600">Suggested project</p><p className="text-xs font-black text-slate-900">{suggestions[0].project.projectCode} — {suggestions[0].project.projectName}</p><p className="text-[9px] text-slate-500">Matched by {suggestions[0].reasons.join(", ")}</p></div>{!readOnly && <button type="button" onClick={() => addAllocation(suggestions[0].project.id)} className="rounded-lg bg-indigo-100 px-2.5 py-1.5 text-[10px] font-black text-indigo-800">Use suggestion</button>}</div>}{allocations.length > 0 ? <div className="mt-3 space-y-2">{allocations.map((allocation) => <div key={allocation.id} className="grid gap-2 rounded-xl border border-indigo-100 bg-white p-2 sm:grid-cols-[minmax(0,1fr)_110px_34px]"><select disabled={readOnly} value={allocation.projectId} onChange={(event) => updateAllocation(allocation.id, { projectId: event.target.value })} className="min-w-0 rounded-lg border border-slate-200 px-2 py-1.5 text-[10px] font-semibold"><option value="">Select project</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.projectCode} — {project.projectName}{project.status === "ARCHIVED" ? " (archived)" : ""}</option>)}</select><div className="flex items-center gap-1"><input disabled={readOnly} type="number" min="0" step="0.01" value={allocation.allocationType === "PERCENTAGE" ? allocation.allocationPercentage || 0 : allocation.allocationAmount} onChange={(event) => updateAllocation(allocation.id, allocation.allocationType === "PERCENTAGE" ? { allocationPercentage: Number(event.target.value) } : { allocationAmount: Number(event.target.value) })} className="min-w-0 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-right text-[10px] tabular-nums" /><select disabled={readOnly} value={allocation.allocationType} onChange={(event) => updateAllocation(allocation.id, { allocationType: event.target.value as InvoiceProjectAllocation["allocationType"], allocationAmount: event.target.value === "PERCENTAGE" ? normalizedInvoiceAllocationAmount(invoice.grandTotal, { ...allocation, allocationType: "PERCENTAGE" }) : allocation.allocationAmount })} className="w-16 rounded-lg border border-slate-200 px-1 py-1.5 text-[10px]"><option value="AMOUNT">Amount</option><option value="PERCENTAGE">%</option></select></div>{!readOnly && <button type="button" onClick={() => setAllocations((current) => current.filter((item) => item.id !== allocation.id))} className="rounded-lg p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-700" aria-label="Remove allocation"><Trash2 className="h-3.5 w-3.5" /></button>}</div>)}</div> : <p className="mt-3 rounded-xl border border-dashed border-indigo-200 bg-white/70 px-3 py-3 text-[10px] text-slate-600">Unallocated — this invoice does not affect project actual cost until a project is confirmed.</p>}<div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-[10px]"><span className="font-semibold text-slate-600">Allocated {invoice.currency || ""} {allocatedTotal.toFixed(2)} / {invoice.grandTotal.toFixed(2)}</span>{!readOnly && <button type="button" onClick={() => void save()} disabled={saving} className="rounded-lg bg-emerald-700 px-3 py-1.5 font-black text-white disabled:opacity-50">{saving ? "Saving…" : "Save allocation"}</button>}</div>{error && <p className="mt-2 text-[10px] font-bold text-rose-700">{error}</p>}</section>;
};
