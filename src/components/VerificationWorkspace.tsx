...
const ProjectAssignmentPanel: React.FC<ProjectAssignmentPanelProps> = ({ invoice, projects, savedAllocations, readOnly, preferredProject, onSave }) => {
  ...
  const addAllocation = (projectId?: string) => setAllocations((current) => { const allocated = current.reduce((sum, allocation) => sum + normalizedInvoiceAllocationAmount(invoice.grandTotal, allocation), 0); const remaining = Math.max(0, Math.round((invoice.grandTotal - allocated) * 100) / 100); return [...current, { id: `local-${Date.now()}-${current.length}`, invoiceId: invoice.id, projectId: projectId || projects.find((project) => project.status !== "ARCHIVED")?.id || "", allocationType: "AMOUNT", allocationAmount: remaining }]; });
