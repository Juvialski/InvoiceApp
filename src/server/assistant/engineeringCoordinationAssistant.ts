...
export const ENGINEERING_COORDINATION_TOOL_DEFINITIONS: readonly EngineeringCoordinationToolDefinition[] = Object.freeze([
  ...
  prepare("prepare_open_rfi", "Prepare formal opening of an existing RFI draft. Confirmation is required.", ["engineering.rfis.create"], { rfiId: uuid }, ["rfiId"]),
  ...
  prepare("prepare_start_submittal_review", "Prepare starting review of the current submitted submittal round. Confirmation is required.", ["engineering.submittals.review"], { submittalId: uuid }, ["submittalId"]),
  ...
  prepare("prepare_close_submittal", "Prepare closure of a completed technical submittal. Confirmation is required and prior rounds remain intact.", ["engineering.submittals.manage"], { submittalId: uuid, reason: { type: "string" } }, ["submittalId"]),
]);
...
export function validateEngineeringCoordinationToolArguments(toolName: string, input: unknown): Record<string, unknown> {
  ...
  switch (toolName) {
    ...
    case "prepare_open_rfi": return { rfiId: requireUuid(args.rfiId, "rfiId") };
    ...
    case "prepare_submit_submittal":
    case "prepare_start_submittal_review": return { submittalId: requireUuid(args.submittalId, "submittalId") };
    ...
    case "prepare_close_submittal": return { submittalId: requireUuid(args.submittalId, "submittalId"), reason: boundedText(args.reason, "reason", 1000, false) };
...
export async function executePreparedEngineeringCoordinationAction(context: AssistantToolContext, toolName: string, args: Record<string, unknown>): Promise<Record<string, unknown>> {
  switch (toolName) {
    ...
    case "prepare_open_rfi": return { rfi: await rpc(context, "open_engineering_rfi", { p_rfi_id: args.rfiId }) };
    ...
    case "prepare_start_submittal_review": return { submittal: await rpc(context, "start_engineering_submittal_review", { p_submittal_id: args.submittalId }) };
    ...
    case "prepare_close_submittal": return { submittal: await rpc(context, "close_engineering_submittal", { p_submittal_id: args.submittalId, p_reason: args.reason || null }) };
