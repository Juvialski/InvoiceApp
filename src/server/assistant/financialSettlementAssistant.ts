...
export const FINANCIAL_SETTLEMENT_TOOL_DEFINITIONS: readonly FinancialSettlementToolDefinition[] = Object.freeze([
  read("get_invoice_settlement", "Return the authoritative supplier-invoice payment summary. When a verified invoice has an active linked Expense, settlement truth is read from that Expense instead of stale invoice payment status.", ["invoices.read", "expenses.read", "cash.transactions.read"], { invoiceId: uuid }, ["invoiceId"]),
  ...
  read("list_open_invoice_settlements", "List verified supplier invoices with an operational settlement balance still outstanding. Linked supplier invoices use their authoritative Expense settlement rather than invoice status alone.", ["invoices.read", "expenses.read", "cash.transactions.read"], { query: { type: "string" }, limit }),
