...
export const ASSISTANT_TOOL_DEFINITIONS: readonly AssistantToolDefinition[] = Object.freeze([
  ...
  navigation("start_tour", "Start an allowlisted in-app tour.", ["dashboard.read"], { tourId: { type: "string", enum: ["engoryx-overview", "cash-banking", "first-invoice", "communications", "projects-costing", "engineering-documents", "project-lifecycle", "payroll-basics", "attendance-overtime", "payroll-run", "workforce-lifecycle", "cash-corrections", "company-access", "reports", "assistant-basics"] } }, ["tourId"]),

  prepare("prepare_process_attached_invoice", "Prepare extraction and draft creation from an attached invoice PDF or image file. Confirmation is required before the invoice is created.", ["invoices.extract", "invoices.read", "invoices.manage", "invoices.verify"], { fileName: { type: "string" }, notes: { type: "string" } }),
