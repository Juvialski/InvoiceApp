...
export const ASSISTANT_TOOL_DEFINITIONS: readonly AssistantToolDefinition[] = Object.freeze([
  ...
  navigation("navigate_to", "Navigate to an allowlisted InvoiceApp route.", (args) => [routePermission(args.routeId)], { routeId: { type: "string", enum: ["dashboard", "projects", "extract", "invoices", "payroll", "expenses", "vendors", "reports", "inbox", "review", "settings"] } }, ["routeId"]),
  ...
  navigation("start_tour", "Start an allowlisted in-app tour.", ["dashboard.read"], { tourId: { type: "string", enum: ["invoiceapp-overview", "first-invoice", "gmail-import", "projects-costing", "payroll-basics", "attendance-overtime", "payroll-run", "reports", "assistant-basics"] } }, ["tourId"]),
