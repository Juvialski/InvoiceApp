...
export function setActiveCompanyId(value: string | null | undefined): string | null {
  ...
  if (activeCompanyId && next && next !== activeCompanyId) {
    throw new Error(`${currentWorkspacePresentation().productName} deployment company context cannot be switched at runtime.`);
  }
...
export function requireActiveCompanyId(): string {
  ...
  if (!companyId) throw new Error(`The ${currentWorkspacePresentation().productName} deployment company is not available for this session.`);
  return companyId;
}
