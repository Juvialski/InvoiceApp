...
export async function saveFinancialTransactionMatchToSupabase(match: FinancialTransactionMatch): Promise<FinancialTransactionMatch> {
  ...
  if (!match.targetId || !["INVOICE", "PAYROLL", "EXPENSE", "CLIENT_COLLECTION", "SUBCONTRACT_CLAIM"].includes(match.targetType)) {
    throw new Error("Invoice, payroll, expense, client collection, and subcontract claim settlement confirmations must use the guarded settlement operation. Transfers use their dedicated operation.");
  }
