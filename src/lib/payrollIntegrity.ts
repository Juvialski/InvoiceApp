...
export function isPayrollPeriodDataBearing(period: PayrollPeriod, context: PayrollPeriodSourceContext = {}) {
  ...
  if (!isSystemMaintenanceNote(period.notes)) return true;
  if (runs.some((run) => runIds.has(run.id) && !isSystemMaintenanceNote(run.notes))) return true;
  if (runs.some((run) => runIds.has(run.id) && ["CALCULATED", "APPROVED", "PAID"].includes(run.status))) return true;
  return false;
}

/** Disposable scheduling infrastructure is physically removed. */
export function isSafeToDeletePayrollPeriod(period: PayrollPeriod, context: PayrollPeriodSourceContext = {}) {
  return period.autoGenerated === true
    && (period.status === "DRAFT" || period.status === "OPEN" || period.status === "VOID")
    && (!isPayrollPeriodLocked(period) || (period.status === "VOID" && !period.lockedAt && isSystemMaintenanceNote(period.notes)))
    && !isPayrollPeriodDataBearing(period, context);
}

export function isSafeToDeletePayrollRun(run: PayrollRun, context: PayrollPeriodSourceContext = {}) {
  if (run.status !== "DRAFT" && run.status !== "VOID") return false;
  ...
  return isSystemMaintenanceNote(run.notes);
}

/** Backwards-compatible names for callers that still describe deletion as retirement. */
export const isSafeToRetirePayrollPeriod = isSafeToDeletePayrollPeriod;
export const isSafeToRetirePayrollRun = isSafeToDeletePayrollRun;
...
export function reconcileObsoleteGeneratedPayrollPeriods(
  ...
  for (const rows of byBoundary.values()) {
    ...
    const keeper = rows.slice().sort((left, right) => Number(!isSafeToDeletePayrollPeriod(left, context)) - Number(!isSafeToDeletePayrollPeriod(right, context)) || statusRank(right.status) - statusRank(left.status) || (left.createdAt || "").localeCompare(right.createdAt || "") || left.id.localeCompare(right.id))[0]!;
    for (const duplicate of rows) {
      if (duplicate.id === keeper.id || !isSafeToDeletePayrollPeriod(duplicate, context)) continue;
      retiredPeriodIds.push(duplicate.id);
      issues.push({ code: "DUPLICATE_PERIOD_BOUNDARY", message: "An empty duplicate payroll period can be removed safely.", periodIds: [keeper.id, duplicate.id], safeToRepair: true });
    }
  }
  const nextPeriods = periods.filter((period) => {
    if (retiredPeriodIds.includes(period.id)) return false;
    if (!isSafeToDeletePayrollPeriod(period, context)) return true;
    const systemTombstone = period.status === "VOID";
    ...
    const obsolete = desiredPeriods.length > 0 && prospective && period.autoGenerated === true && !desiredBoundaries.has(boundaryKey(period));
    if (!systemTombstone && !obsolete) return true;
    retiredPeriodIds.push(period.id);
    issues.push({ code: "STALE_AUTO_GENERATED_PERIOD", message: systemTombstone ? "An empty generated tombstone can be removed safely." : "An empty generated payroll period no longer matches the active schedule and can be removed safely.", periodIds: [period.id], safeToRepair: true });
    return false;
  });
...
export function retireEmptyGeneratedPayrollRuns(runs: readonly PayrollRun[], retiredPeriodIds: readonly string[], entries: readonly PayrollEntry[] = []) {
  ...
  const context: PayrollPeriodSourceContext = { entries };
  return runs.filter((run) => !(retired.has(run.periodId) && isSafeToDeletePayrollRun(run, context)));
}
...
export function planPayrollRepair(input: PayrollRepairInput): PayrollRepairPlan {
  ...
  const reconciliation = reconcileObsoleteGeneratedPayrollPeriods(input.periods, desiredPeriods, { ...context, referenceDate, horizonEnd });
  ...
  for (const rows of runsByPeriod.values()) {
    ...
    for (const duplicate of rows) if (duplicate.id !== keeper.id && isSafeToDeletePayrollRun(duplicate, context)) retiredRuns.add(duplicate.id);
  }
  const retiredPeriodSet = new Set(retiredPeriodIds);
  for (const run of input.runs) if (retiredPeriodSet.has(run.periodId) && isSafeToDeletePayrollRun(run, context)) retiredRuns.add(run.id);
  const runs = input.runs.filter((run) => !retiredRuns.has(run.id));
  const protectedPeriodCount = periods.filter((period) => !isSafeToDeletePayrollPeriod(period, context)).length;
  const protectedRunCount = input.runs.filter((run) => !isSafeToDeletePayrollRun(run, context)).length;
  const manualReviewCount = report.issues.filter((issue) => !issue.safeToRepair).length;
  const messages = [
    `${retiredPeriodIds.length} ${retiredPeriodIds.length === 1 ? "empty period" : "empty periods"} can be removed safely.`,
    `${retiredRuns.size} ${retiredRuns.size === 1 ? "empty run" : "empty runs"} can be removed safely.`,
    ...
    `${manualReviewCount} ${manualReviewCount === 1 ? "schedule conflict needs" : "schedule conflicts need"} manual review.`,
  ];
...
export function previewSafePayrollReset(input: PayrollRepairInput): PayrollResetPreview {
  ...
  const safeGeneratedPeriods = input.periods.filter((period) => isSafeToDeletePayrollPeriod(period, context)).length;
  const emptyDraftRuns = input.runs.filter((run) => isSafeToDeletePayrollRun(run, context)).length;
  const protectedPeriods = input.periods.length - safeGeneratedPeriods;
  const protectedRuns = input.runs.length - emptyDraftRuns;
  const finalizedPeriods = input.periods.filter((period) => !isSafeToDeletePayrollPeriod(period, context) && (["APPROVED", "PAID"].includes(period.status) || Boolean(period.lockedAt) || isPayrollPeriodDataBearing(period, context))).length;
  const dataBearingPeriods = input.periods.filter((period) => isPayrollPeriodDataBearing(period, context)).length;
  return { safeGeneratedPeriods, emptyDraftRuns, protectedPeriods, protectedRuns, finalizedPeriods, dataBearingPeriods, messages: [`${safeGeneratedPeriods} empty generated periods can be removed.`, `${emptyDraftRuns} empty draft runs can be removed.`, `${finalizedPeriods} finalized or locked periods are protected.`, `${dataBearingPeriods} data-bearing periods are protected.`] };
}
