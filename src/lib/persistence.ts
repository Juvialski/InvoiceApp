import type { Expense, InvoiceData, OriginalSourcePayload, StoredSourceDocument, ReviewEvent, Vendor } from "../types.ts";
import { supabase } from "./supabase.ts";
import { companyStoragePath, requireActiveCompanyId } from "./companyContext.ts";
import { companyApiRequest } from "./companyApi.ts";
import { validateInvoiceDocumentBytes } from "./fileSecurity.ts";
...
export async function markSourceDocumentStatus(sourceDocumentId: string, status: string, documentType?: string) {
  ...
  const { error } = await client
    .from("source_documents")
    .update({ processing_status: status, ...(documentType ? { document_type: documentType } : {}) })
