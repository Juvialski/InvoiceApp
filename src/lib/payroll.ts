...
export async function savePayrollPeriodToSupabase(period: PayrollPeriod) {
  ...
  const companyId = requireActiveCompanyId();
  const periodId = persistedId(period.id, "period");
  const row = {
    id: periodId,
    user_id: userId,
    ...
    pay_date: period.payDate || null,
    schedule_id: period.scheduleId || null,
    schedule_version_id: period.scheduleVersionId || null,
    auto_generated: period.autoGenerated ?? false,
    locked_at: period.lockedAt || null,
    source_revision: period.sourceRevision ?? 0,
    source_revision_updated_at: period.sourceRevisionUpdatedAt || null,
    status: period.status,
    notes: period.notes || null,
    updated_at: new Date().toISOString(),
  };

  // Reconciliation can be performed by another authorized actor in the same
  // company. Read the authoritative row first so an existing period update
  // never attempts to rewrite its immutable actor or company metadata.
  ...
  if (existing) {
    const { id: _id, user_id: _userId, company_id: _companyId, ...updateRow } = row;
  ...
  const { data, error } = await supabase.from("payroll_periods").insert(row).select("*").single();
  if (error) throw error;
  return periodFromRow(data as Record<string, unknown>);
}
