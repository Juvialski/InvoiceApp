...
export async function loadProjectsFromSupabase(): Promise<Project[]> { const userId = await currentUserId(); if (!supabase || !userId) return []; const { data, error } = await supabase.from("projects").select("*").eq("user_id", userId).order("updated_at", { ascending: false }); if (error) throw error; return (data || []).map((row) => projectFromRow(row as Row)); }
