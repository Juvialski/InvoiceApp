...
export const PROJECT_LIFECYCLE_DEPENDENCY_KEYS: readonly ProjectLifecycleDependencyKey[] = [
  "invoiceProjectAllocations",
  "expenses",
  "projectWorkerAssignments",
  "workEntries",
...
export function projectFromRow(row: Row): Project { return { id: String(row.id), userId: text(row.user_id), projectCode: String(row.project_code || ""), projectName: String(row.project_name || ""), description: text(row.description), clientName: text(row.client_name), clientReference: text(row.client_reference), billingContactName: text(row.billing_contact_name), billingEmail: text(row.billing_email), billingAddress: text(row.billing_address), location: text(row.location), siteAddress: text(row.site_address), projectManager: text(row.project_manager), status: String(row.status || "PLANNING") as ProjectStatus, startDate: text(row.start_date), targetEndDate: text(row.target_end_date), actualEndDate: text(row.actual_end_date), contractValue: row.contract_value === null ? undefined : numberValue(row.contract_value), projectBudget: numberValue(row.project_budget), currency: String(row.currency || "PHP").toUpperCase(), taxTreatment: normalizeProjectTaxTreatment(row.tax_treatment), notes: text(row.notes), createdAt: String(row.created_at || new Date().toISOString()), updatedAt: String(row.updated_at || new Date().toISOString()), archivedAt: text(row.archived_at), archivedFromStatus: text(row.archived_from_status) as Project["archivedFromStatus"] }; }
...
function projectPayload(project: Project, companyId: string) {
  return {
    id: project.id,
    companyId,
    ...
    currency: (project.currency || "PHP").toUpperCase(),
    taxTreatment: normalizeProjectTaxTreatment(project.taxTreatment),
...
export async function saveProjectToSupabase(project: Project, expectedUpdatedAt?: string): Promise<Project> {
  const userId = await currentUserId();
  if (!supabase || !userId) throw new Error("Sign in before saving projects.");
  const companyId = requireActiveCompanyId();
  const { data, error } = await supabase.rpc("save_project", {
    p_project: projectPayload(project, companyId),
    p_expected_updated_at: expectedUpdatedAt || null,
  });
  if (error) throw error;
  const result = data as { project: Row };
  return projectFromRow(result.project);
}
