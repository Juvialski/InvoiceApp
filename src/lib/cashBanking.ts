...
function readStatementRows(input: ArrayBuffer | Uint8Array | string, format: "CSV" | "XLSX"): { sheetName: string; rows: StatementCell[][] } {
  ...
  const workbook = typeof input === "string"
    ? XLSX.read(input, { type: "string", cellDates: false, cellFormula: false, cellText: true })
    : XLSX.read(input, { type: "array", cellDates: false, cellFormula: false, cellText: true });
...
export function parseStatementFile(input: ArrayBuffer | Uint8Array | string, fileName = "statement.csv"): ParsedStatementDocument {
  ...
  const structure = detectStatementStructure(workbook.rows);
  return { format, fileName, fileFingerprint: statementFileFingerprint(workbook.rows), sheetName: workbook.sheetName, rawRows: workbook.rows, structure };
}
