...
export function buildDeploymentTransferPreflight(input: TransferPreflightInput): TransferPreflightReport {
  ...
  checks.push(
    manualCheck("supabase-transfer-eligibility", "Supabase project-transfer eligibility", "Confirm the source organization owner, target-organization membership, absence of an active GitHub integration, absence of project-scoped roles pointing to the project, and absence of log drains. Also confirm the target organization's plan/project capacity: a transfer onto a Free plan must not exceed Supabase's two-free-project limit. If the plan changes, review possible feature loss, brief transfer downtime, and the operator's effective role after transfer. This local preflight does not call the Supabase Management API."),
