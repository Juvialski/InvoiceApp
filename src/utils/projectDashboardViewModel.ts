...
export function buildProjectDashboardViewData(input: ProjectDashboardInput): ProjectDashboardViewData {
  ...
  if (pendingExpenses) attention.push({ id: "project-pending-expenses", label: "Pending project expenses", detail: "Draft direct expenses remain commitments until approved or paid.", tab: "expenses" });
  if (availableAfterCommitments < 0) attention.push({ id: "project-budget-pressure", label: "Commitments exceed budget", detail: "Confirmed cost plus pending commitments is above the project budget.", tab: "overview" });
