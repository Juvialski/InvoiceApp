...
export function buildDashboardViewData(input: DashboardViewModelInput): DashboardViewData {
  ...
  const currencies = [...new Set([...input.projects.map((project) => currencyOf(project.currency)), ...input.invoices.map((invoice) => currencyOf(invoice.currency)), ...input.expenses.map((expense) => currencyOf(expense.currency)), ...preparedPayroll.map((run) => currencyOf(run.currency)), ...(input.cash?.accounts || []).map((account) => currencyOf(account.currency))])].sort();
