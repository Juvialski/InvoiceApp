...
export function buildDashboardViewData(input: DashboardViewModelInput): DashboardViewData {
  ...
  const currencies = [...new Set([...input.projects.map((project) => currencyOf(project.currency)), ...input.invoices.map((invoice) => currencyOf(invoice.currency)), ...input.expenses.map((expense) => currencyOf(expense.currency)), ...preparedPayroll.map((run) => currencyOf(run.currency))])].sort();
  ...
  const projectRows = input.projects.filter((project) => (!input.projectId || project.id === input.projectId) && project.status !== "ARCHIVED" && currencyOf(project.currency) === selectedCurrency).map((project) => {
    const summary = calculateProjectCost(project, { invoices: accountingIndex.invoicesByProjectId.get(project.id) || [], expenses: accountingIndex.expensesByProjectId.get(project.id) || [], payroll: accountingIndex.payrollByProjectId.get(project.id) || [] });
  ...
  const currentPeriod = input.periods.find((period) => period.status !== "VOID" && period.periodStart <= today && period.periodEnd >= today) || input.periods.find((period) => period.status !== "VOID");
