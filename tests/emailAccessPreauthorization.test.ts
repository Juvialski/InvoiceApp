...
test("browser access management is SMTP-independent and keeps legacy delivery state out of the primary UI", () => {
  ...
  assert.match(accessManagement, /Authorize an email to access this \{currentWorkspacePresentation\(\)\.productName\} deployment/);
  ...
  assert.match(accessStates, /This email has not been authorized for this \{currentWorkspacePresentation\(\)\.productName\} deployment/);
