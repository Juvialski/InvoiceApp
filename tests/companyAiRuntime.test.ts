...
test("runtime credentials remain company-bound and cache invalidation follows versions", async () => {
  ...
  const firstA = await resolveCompanyAiRuntime({ supabase: fake.client, companyId: COMPANY_A, now: 100, environment: ENVIRONMENT });
  const cachedA = await resolveCompanyAiRuntime({ supabase: fake.client, companyId: COMPANY_A, now: 101, environment: ENVIRONMENT });
  const firstB = await resolveCompanyAiRuntime({ supabase: fake.client, companyId: COMPANY_B, now: 101, environment: ENVIRONMENT });
  ...
  const rotatedA = await resolveCompanyAiRuntime({ supabase: fake.client, companyId: COMPANY_A, now: 102, forceRefresh: true, environment: ENVIRONMENT });
  ...
  const invalidatedA = await resolveCompanyAiRuntime({ supabase: fake.client, companyId: COMPANY_A, now: 103, environment: ENVIRONMENT });
...
test("a mismatched resolver row fails closed instead of being relabeled to the requested company", async () => {
  ...
  await assert.rejects(
    () => resolveCompanyAiRuntime({ supabase: fake.client, companyId: COMPANY_A, environment: ENVIRONMENT }),
    (error: any) => {
...
test("missing or disabled company credentials do not consume the global fallback", async () => {
  ...
  const fallback = await resolveCompanyAiRuntime({ supabase: noSettings.client, companyId: COMPANY_A, environment: localEnvironment });
  ...
  for (const [companyId, status] of [[COMPANY_A, "NOT_CONFIGURED"], [COMPANY_B, "DISABLED"]] as const) {
    ...
    const fake = fakeSupabase(rows);
    await assert.rejects(
      () => resolveCompanyAiRuntime({ supabase: fake.client, companyId, environment: localEnvironment }),
      (error: any) => {
...
test("authentication retry re-resolves the same company runtime", async () => {
  ...
  let calls = 0;
  const result = await withCompanyAiRuntime({ supabase: fake.client, companyId: COMPANY_A, environment: ENVIRONMENT }, async (runtime) => {
...
test("runtime cache evicts old company entries at its hard bound", async () => {
  ...
  const fake = fakeSupabase(rows);
  for (const companyId of companyIds) await resolveCompanyAiRuntime({ supabase: fake.client, companyId, now: 500, environment: ENVIRONMENT });
  const callsAfterFill = fake.calls();
  await resolveCompanyAiRuntime({ supabase: fake.client, companyId: companyIds[0]!, now: 501, environment: ENVIRONMENT });
