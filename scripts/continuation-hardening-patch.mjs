...
function replaceBlock(text, start, end, replacement, label) {
  ...
  if (startIndex < 0 || endIndex < 0) throw new Error(`Patch block missing: ${label}`);
...
`, "persistence cleanup helpers");
  ...
  const manual = `export async function saveManualSourceDocument(input: { fileData: string; mimeType: string; fileName: string; emailMessageId?: string; sourceType?: "UPLOAD" | "EMAIL" }): Promise<StoredSourceDocument> {
  const client = requireSupabase();
  ...
  const { data, error } = await client
    ...
    .select("id")
    .single();
  if (error) return cleanupUploadedObject(INVOICE_BUCKET, path, error);
  ...
  text = replaceBlock(text, "export async function saveManualSourceDocument", "export async function loadSourcePayloadForRetry", manual, "manual source persistence");

  const gmail = `export async function saveGmailMessageSource(message: GmailImportedMessage): Promise<{ email: StoredEmailRecord; documents: StoredSourceDocument[] }> {
  const client = requireSupabase();
  const userId = await requireUserId();
  const companyId = requireActiveCompanyId();
  ...
  text = replaceBlock(text, "export async function saveGmailMessageSource", "function normalizedVendorName", gmail, "gmail persistence");
...
`    .from("source_documents")
    ...
    .eq("email_message_id", emailId)
    .order("attachment_index", { ascending: true });`, "email attachment company filter");
  write(path, text);
}

{
  const path = "src/lib/engineeringDocumentsPersistence.ts";
  let text = read(path);
  text = replaceOnce(text,
...
`  const normalizedCompanyId = safeStorageSegment(companyId, "Company ID");
  ...
  const normalizedRevisionId = safeStorageSegment(revisionId, "Engineering revision ID");`, "engineering storage path validation");
  text = replaceOnce(text,
...
`  const parts = filePath.trim().split("/");
  ...
  return parts.length === 7`, "engineering path predicate validation");
  write(path, text);
}

{
  const path = "src/lib/payrollImportPersistence.ts";
  let text = read(path);
  text = replaceOnce(text,
...
`export async function uploadPayrollImportSourceToSupabase(input: { batchId: string; fileName: string; mimeType?: string; bytes: Uint8Array }) {
  ...
  const { error } = await supabase.storage.from(PAYROLL_IMPORT_BUCKET).upload(storagePath, input.bytes, { contentType: input.mimeType, upsert: false });
  if (error) throw error;`, "payroll source validation");
  write(path, text);
}

{
  const path = "src/lib/payrollImport.ts";
  let text = read(path);
  text = text.replaceAll("cellFormula: true", "cellFormula: false");
  write(path, text);
}
...
`// Keep the global parser intentionally small. Large binary content belongs in
// validated Storage upload paths; individual JSON endpoints may opt into a
// narrower documented limit before this middleware only when required.
app.use(express.json({ limit: "8mb" }));
app.use(express.urlencoded({ extended: true, limit: "1mb" }));`, "global request body limits");
  write(path, text);
}

{
  const path = "scripts/workflow-map/graph.ts";
  let text = read(path);
  text = replaceOnce(text,
`  node({
    id: "production-mode",
...
`    description: "The deployment-resolved company plus the authenticated user's membership and permission snapshot; no tenant picker or unrelated company selection is authoritative.",`, "workflow company description");
  text = replaceOnce(text,
`    id: "company-membership",
    label: "Company membership and access snapshot",`,
`    id: "company-membership",
...
{
  const path = ".github/workflows/database-tests.yml";
  let text = read(path);
  text = text.replace(/version:\s*2\.76\.8/g, "version: 2.115.0");
  write(path, text);
}

write("tests/fileSecurity.test.ts", `import assert from "node:assert/strict";
...
console.log("Continuation hardening patch applied.");
