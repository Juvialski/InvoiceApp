...
- `superpowers/specs/2026-09-20-worksheet-density-clarity-correction.md` — blocking UX-W4.5 usability correction before UX-W5. It now begins with an app-wide screenshot investigation/visual triage across major authenticated/demo routes and four viewport classes, then applies evidence-backed hierarchy, density, protected/read-only/provenance, progressive-disclosure, and chrome corrections.

The selective-workbook rollout has progressed through the shared worksheet foundation, Projects, Supplier Invoice review, and bounded UX-W4 transaction editors. UX-W4 is complete. The next phase is **UX-W4.5A app-wide screenshot investigation and visual triage**, followed by evidence-backed UX-W4.5 correction slices before UX-W5 so the current visual problems are not copied into new domains. Real `.xlsx` round trips remain required where supported: export -> external edit -> upload -> validate/compare -> human review -> authoritative apply.
