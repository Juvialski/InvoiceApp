...
| **Dashboard route**<br/><small>`route-dashboard`</small> | `dashboard` | `/dashboard` | `view` | `company` |
...
| **Project Client Billing route**<br/><small>`route-project-billing`</small> | `projects` | `/projects/:projectId/billing` | `billingId` | `project` |
...
| **Email / SMS route**<br/><small>`route-inbox`</small> | `inbox` | `/email-sms` | `view`, `channel`, `documentType`, `documentId`, `from` | `company` |
| **Documents route**<br/><small>`route-documents`</small> | `documents` | `/documents` | `view`, `managedId` | `company` |
...
| **Expenses route**<br/><small>`route-expenses`</small> | `expenses` | `/expenses` | `expenseId`, `from` | `company` |
...
| **Warehouse Inventory route**<br/><small>`route-warehouse-inventory`</small> | `warehouse` | `/warehouse` | `movementId`, `receiptId`, `from` | `global` |
...
flowchart LR
  subgraph g_workforce["Workforce"]
    ...
    n_payroll_run_action("Calculate or approve payroll run<br/><small>ACTION</small>")
...
flowchart LR
  subgraph g_assistant["Assistant"]
    n_assistant_screen["HydroQualiSense Assistant panel<br/><small>SCREEN</small>"]
...
| **Base reporting uses authoritative FX evidence**<br/><small>`base-reporting-uses-authoritative-fx`</small> | When an immutable conversion snapshot exists, base-currency reporting uses its persisted base amount and removes the converted source from unresolved foreign residual buckets; without that evidence the source remains foreign and unconverted. | `src/utils/financialCurrency.ts`<br/>`src/utils/projectCosting.ts`<br/>`src/utils/dashboardViewModel.ts`<br/>`src/components/expenses/ExpensesPage.tsx`<br/>`src/components/VerificationWorkspace.tsx` | `tests/financialReportingCurrency.test.ts`<br/>`tests/r4CurrencyAndTax.test.ts` |
| **Only issued client billing contributes to billed-to-date**<br/><small>`commercial-client-billing-issued-only`</small> | Client progress billing is project revenue-side history. Header totals derive from billing lines, and only ISSUED records contribute to Billed to Date; drafts, submitted, cancelled, and voided records remain excluded. Issuance is bounded by the authoritative project contract value. | `src/lib/clientBilling.ts`<br/>`src/components/projects/ClientBillingPanel.tsx`<br/>`src/components/projects/ProjectOverview.tsx`<br/>`supabase/migrations/20260903224406_client_progress_billing_foundation.sql` | `tests/subcontractsDomain.test.ts`<br/>`tests/subcontractClaimsDomain.test.ts`<br/>`tests/subcontractClaimsReviewHardening.test.ts`<br/>`tests/subcontractVariationsDomain.test.ts`<br/>`tests/subcontractVariationsFinancialInvariants.test.ts`<br/>`tests/subcontractVariationsClaimIntegration.test.ts`<br/>`tests/clientProgressBilling.test.ts`<br/>`tests/clientProgressBillingMigration.test.ts`<br/>`tests/clientCollectionsDomain.test.ts`<br/>`tests/wave1bClientReceivableUx.test.ts` |
...
GitDiagram was used only to accelerate repository discovery. The following record preserves what was useful and what required domain-specific correction before becoming HydroQualiSense context.
...
| **Dashboard derived metrics**<br/><small>`dashboard-derived-metrics`</small> | `derived-data` | `company`<br/>— | — | — | `mixed` | `src/utils/dashboardStats.ts`<br/>`src/utils/dashboardViewModel.ts`<br/>`src/App.tsx` | `tests/accountingStatistics.test.ts`<br/>`tests/financialSettlement.test.ts`<br/>`tests/financialReportingCurrency.test.ts` | — |
...
| Node | Type | Scope / route | Status values | Permissions | Source / confirmation | Source files | Tests | QA-1 scenarios |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **Project Client Billing route**<br/><small>`route-project-billing`</small> | `route` | `project`<br/>`projects`<br/>`/projects/:projectId/billing`<br/>query: `billingId` | — | — | `code-derived` | `src/utils/appRouting.ts`<br/>`src/utils/appRouteContracts.ts`<br/>`src/app/routes/AppRouter.tsx`<br/>`src/app/routes/ProjectsRoute.tsx`<br/>`src/components/projects/ProjectWorkspace.tsx`<br/>`src/components/projects/ClientBillingPanel.tsx` | `tests/appRouting.test.ts`<br/>`tests/wave1bClientReceivableUx.test.ts` | `client-receivables--project-billing--client-invoice-collection-lifecycle-verified--desktop-1440`<br/>`client-receivables--project-billing--client-invoice-collection-lifecycle-verified--mobile-390` |
...
| **Client Collections Register**<br/><small>`client-collection-workspace`</small> | `screen` | `project`<br/>— | — | `projects.read`<br/>`projects.manage` | `mixed` | `src/lib/clientCollections.ts`<br/>`src/lib/financialSettlement.ts`<br/>`src/lib/financialSettlementPersistence.ts`<br/>`src/lib/cashBanking.ts`<br/>`src/lib/cashBankingPersistence.ts`<br/>`src/components/CashSettlementAllocationWorkspace.tsx`<br/>`src/components/projects/ClientCollectionSettlementPanel.tsx`<br/>`src/components/projects/ClientBillingPanel.tsx`<br/>`src/components/projects/ProjectWorkspace.tsx`<br/>`src/components/projects/ProjectOverview.tsx`<br/>`src/app/routes/ProjectsRoute.tsx`<br/>`src/App.tsx`<br/>`src/utils/appRouteContracts.ts`<br/>`src/utils/appRouting.ts`<br/>`src/utils/projectFinancialSummary.ts`<br/>`supabase/migrations/20260904090000_client_collections_foundation.sql`<br/>`supabase/migrations/20260904100000_client_collection_cash_settlement_linkage.sql` | `tests/subcontractsDomain.test.ts`<br/>`tests/subcontractClaimsDomain.test.ts`<br/>`tests/subcontractClaimsReviewHardening.test.ts`<br/>`tests/subcontractVariationsDomain.test.ts`<br/>`tests/subcontractVariationsFinancialInvariants.test.ts`<br/>`tests/subcontractVariationsClaimIntegration.test.ts`<br/>`tests/clientCollectionsDomain.test.ts`<br/>`tests/clientCollectionsMigration.test.ts`<br/>`tests/clientCollectionSettlement.test.ts`<br/>`tests/wave1bClientReceivableUx.test.ts`<br/>`supabase/tests/database/16_client_collection_settlement.test.sql` | `client-receivables--project-billing--client-invoice-collection-lifecycle-verified--desktop-1440`<br/>`client-receivables--project-billing--client-invoice-collection-lifecycle-verified--mobile-390` |
...
| **Email / SMS route**<br/><small>`route-inbox`</small> | `route` | `company`<br/>`inbox`<br/>`/email-sms`<br/>query: `view`, `channel`, `documentType`, `documentId`, `from` | — | — | `mixed` | `src/utils/routes.ts`<br/>`src/utils/appRouting.ts`<br/>`src/app/routes/AppRouter.tsx`<br/>`src/app/routes/EmailSmsRoute.tsx`<br/>`src/components/EmailComposePanel.tsx`<br/>`src/components/EmailProviderStatusPanel.tsx` | `tests/appRouting.test.ts`<br/>`tests/assistantBackend.test.ts`<br/>`tests/emailSmsDocumentsWorkspace.test.ts` | — |
| **Documents route**<br/><small>`route-documents`</small> | `route` | `company`<br/>`documents`<br/>`/documents`<br/>query: `view`, `managedId` | — | — | `mixed` | `src/utils/routes.ts`<br/>`src/utils/appRouteContracts.ts`<br/>`src/utils/appRouting.ts`<br/>`src/app/routes/AppRouter.tsx`<br/>`src/app/routes/DocumentsRoute.tsx`<br/>`src/components/documents/DocumentCreateView.tsx`<br/>`src/components/documents/ManagedDocumentDetailView.tsx`<br/>`src/components/documents/ManagedDocumentUploadView.tsx`<br/>`src/lib/documentRegister.ts`<br/>`src/lib/managedDocuments.ts`<br/>`src/server/managedDocuments/managedDocumentRouter.ts`<br/>`supabase/migrations/20260921095140_wide_documents_managed_artifacts_foundation.sql` | `tests/navigationRoutes.test.ts`<br/>`tests/appRouting.test.ts`<br/>`tests/documentCenter.test.ts`<br/>`tests/managedDocumentsMigration.test.ts`<br/>`tests/managedDocumentsRuntime.test.ts`<br/>`tests/managedDocumentsWorkspace.test.tsx`<br/>`tests/emailSmsDocumentsWorkspace.test.ts` | — |
| **Email / SMS communications workspace**<br/><small>`email-sms-workspace`</small> | `screen` | `company`<br/>— | — | `documents.send` | `mixed` | `src/app/routes/EmailSmsRoute.tsx`<br/>`src/components/EmailComposePanel.tsx`<br/>`src/components/EmailProviderStatusPanel.tsx`<br/>`src/components/SmsComposePanel.tsx`<br/>`src/components/CommunicationHistoryPanel.tsx`<br/>`src/components/SmsProviderStatusPanel.tsx`<br/>`src/server/messaging/messagingRouter.ts`<br/>`src/server/documentDelivery/documentDeliveryRouter.ts`<br/>`src/server/documentDelivery/issuedDocumentRouter.ts` | `tests/documentDelivery.test.ts`<br/>`tests/emailSmsDocumentsWorkspace.test.ts`<br/>`tests/smsProvider.test.ts`<br/>`tests/smsDeliveryContract.test.ts` | — |
| **Unified Documents workspace**<br/><small>`documents-workspace`</small> | `screen` | `company`<br/>— | — | `invoices.read`<br/>`projects.read`<br/>`procurement.read`<br/>`expenses.read`<br/>`cash.summary.read`<br/>`engineering.documents.read`<br/>`documents.read`<br/>`documents.manage`<br/>`documents.send` | `mixed` | `src/app/routes/DocumentsRoute.tsx`<br/>`src/components/documents/DocumentCreateView.tsx`<br/>`src/components/documents/ManagedDocumentDetailView.tsx`<br/>`src/components/documents/ManagedDocumentUploadView.tsx`<br/>`src/lib/documentRegister.ts`<br/>`src/lib/managedDocuments.ts`<br/>`src/components/DocumentPreviewModal.tsx`<br/>`src/server/managedDocuments/managedDocumentRouter.ts` | `tests/documentCenter.test.ts`<br/>`tests/managedDocumentsWorkspace.test.tsx`<br/>`tests/managedDocumentRouter.test.ts`<br/>`tests/emailSmsDocumentsWorkspace.test.ts`<br/>`tests/documentDelivery.test.ts` | — |
| **SMS provider adapter boundary**<br/><small>`sms-provider-boundary`</small> | `external-boundary` | `company`<br/>— | — | `documents.send` | `mixed` | `src/server/messaging/smsProvider.ts`<br/>`src/lib/messaging.ts`<br/>`src/server/messaging/messagingRouter.ts`<br/>`server.ts` | `tests/emailSmsDocumentsWorkspace.test.ts`<br/>`tests/smsProvider.test.ts`<br/>`tests/smsDeliveryContract.test.ts` | — |
...
| **Expenses route**<br/><small>`route-expenses`</small> | `route` | `company`<br/>`expenses`<br/>`/expenses`<br/>query: `expenseId`, `from` | — | — | `code-derived` | `src/utils/routes.ts`<br/>`src/utils/appRouting.ts`<br/>`src/app/routes/ExpensesRoute.tsx`<br/>`src/components/expenses/ExpensesPage.tsx`<br/>`src/components/FinancialSettlementCard.tsx` | `tests/accountingStatistics.test.ts`<br/>`tests/financialSettlement.test.ts`<br/>`tests/wave1aSupplierPayableUx.test.ts` | `expenses--expenses--base-route-loaded--desktop-1440`<br/>`supplier-payables--expenses--authoritative-expense-payment-surface-opened--desktop-1440`<br/>`supplier-payables--expenses--authoritative-expense-payment-surface-opened--mobile-390` |
...
| **Invoice source document or email**<br/><small>`invoice-source`</small> | `data` | `company`<br/>— | — | `invoices.extract` | `mixed` | `src/components/UploadZone.tsx`<br/>`src/types.ts`<br/>`src/lib/persistence.ts` | `tests/invoiceLogic.test.ts`<br/>`tests/assistantAttachedInvoice.test.ts` | — |
| **Invoice extraction workflow**<br/><small>`invoice-extraction-workflow`</small> | `workflow` | `company`<br/>— | `NEW` → `EXTRACTING` → `EXTRACTED` → `FAILED` | `invoices.extract` | `mixed` | `src/utils/invoiceWorkspace.ts`<br/>`src/utils/invoiceLogic.ts`<br/>`src/App.tsx`<br/>`src/server/ai/companyAiRuntime.ts`<br/>`src/server/invoiceExtraction/invoiceExtractionRouter.ts`<br/>`src/server/invoiceExtraction/invoiceExtractionService.ts`<br/>`src/server/invoiceExtraction/invoiceExtractionSchema.ts` | `tests/invoiceLogic.test.ts`<br/>`tests/extractionQuality.test.ts`<br/>`tests/assistantAttachedInvoice.test.ts` | — |
| **Gemini-compatible extraction boundary**<br/><small>`invoice-extraction-ai-boundary`</small> | `external-boundary` | `company`<br/>— | — | `invoices.extract` | `mixed` | `src/server/ai/companyAiRuntime.ts`<br/>`src/server/ai/companyAiCredentials.ts`<br/>`src/server/ai/companyAiRouter.ts`<br/>`src/server/invoiceExtraction/invoiceExtractionRouter.ts`<br/>`src/server/invoiceExtraction/invoiceExtractionService.ts`<br/>`src/server/invoiceExtraction/invoiceExtractionSchema.ts`<br/>`src/utils/invoiceLogic.ts` | `tests/companyAiRuntime.test.ts`<br/>`tests/companyAiServer.test.ts`<br/>`tests/extractionQuality.test.ts` | — |
...
| **Cash reconciliation workspace**<br/><small>`cash-reconciliation-workspace`</small> | `screen` | `company`<br/>— | — | `cash.summary.read`<br/>`cash.transactions.read`<br/>`cash.reconcile` | `mixed` | `src/components/CashBankingPage.tsx`<br/>`src/components/CashSettlementAllocationWorkspace.tsx`<br/>`src/components/InvoiceSettlementDirectoryPanel.tsx`<br/>`src/components/financial/FinancialReasonDialog.tsx` | `tests/cashBanking.test.ts`<br/>`tests/financialSettlement.test.ts`<br/>`tests/structuredBrowserEvidence.test.ts`<br/>`tests/coreHardeningWave2B3CashCorrections.test.ts`<br/>`tests/wave1aSupplierPayableUx.test.ts`<br/>`tests/wave1bClientReceivableUx.test.ts` | `cash-banking--cash-settlement--cash-settlement-workspace-opened--desktop-1440`<br/>`supplier-payables--cash--cash-expense-target-and-return-context-opened--desktop-1440`<br/>`supplier-payables--cash--cash-expense-target-and-return-context-opened--mobile-390`<br/>`client-receivables--project-billing--client-invoice-collection-lifecycle-verified--desktop-1440`<br/>`client-receivables--project-billing--client-invoice-collection-lifecycle-verified--mobile-390` |
...
| **Settlement eligibility and concurrency guard**<br/><small>`cash-settlement-guard`</small> | `guard` | `company`<br/>— | — | `cash.reconcile`<br/>`invoices.manage`<br/>`payroll.approve`<br/>`expenses.manage`<br/>`procurement.approve`<br/>`projects.manage` | `mixed` | `src/lib/financialSettlement.ts`<br/>`src/lib/financialSettlementPersistence.ts`<br/>`supabase/migrations/20260827210000_financial_settlement_integration.sql`<br/>`supabase/migrations/20260827215000_financial_reconciliation_status_guard.sql`<br/>`supabase/migrations/20260829234056_core_hardening_wave2b3_cash_corrections.sql` | `tests/financialSettlement.test.ts`<br/>`tests/cashBankingMigration.test.ts`<br/>`tests/coreHardeningWave2B3CashCorrections.test.ts`<br/>`supabase/tests/database/09_core_hardening_wave2b3_settlement_corrections.test.sql` | — |
