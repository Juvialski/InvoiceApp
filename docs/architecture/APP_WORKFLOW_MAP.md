...
| **Email / SMS communications workspace**<br/><small>`email-sms-workspace`</small> | `screen` | `company`<br/>— | — | `gmail.read`<br/>`gmail.manage`<br/>`documents.send` | `mixed` | `src/app/routes/EmailSmsRoute.tsx`<br/>`src/components/EmailInbox.tsx`<br/>`src/components/EmailComposePanel.tsx`<br/>`src/components/SmsComposePanel.tsx`<br/>`src/components/CommunicationHistoryPanel.tsx`<br/>`src/components/SmsProviderStatusPanel.tsx` | `tests/emailIntakePhase1.test.ts`<br/>`tests/documentDelivery.test.ts`<br/>`tests/emailSmsDocumentsWorkspace.test.ts`<br/>`tests/smsProvider.test.ts`<br/>`tests/smsDeliveryContract.test.ts` | — |
