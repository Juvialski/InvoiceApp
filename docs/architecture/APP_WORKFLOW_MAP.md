...
| **SMS provider adapter boundary**<br/><small>`sms-provider-boundary`</small> | `external-boundary` | `company`<br/>— | — | `documents.send` | `mixed` | `src/server/messaging/smsProvider.ts`<br/>`src/lib/messaging.ts`<br/>`server.ts` | `tests/emailSmsDocumentsWorkspace.test.ts`<br/>`tests/smsProvider.test.ts`<br/>`tests/smsDeliveryContract.test.ts` | — |
