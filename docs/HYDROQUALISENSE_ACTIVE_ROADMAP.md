...
The exact next implementation phase is **UI-R4B — shared visual foundation**. R4B must implement System/Light/Dark using the existing paired Astryx tokens, shared button hierarchy, and compact filter/action primitives, and must stop before the R4C Dashboard/Project Portfolio redesign. Each Round 4 implementation slice must include actual screenshot inspection by the lead at the affected standard viewports; automated overflow/accessibility checks are not sufficient evidence of professional visual quality.
