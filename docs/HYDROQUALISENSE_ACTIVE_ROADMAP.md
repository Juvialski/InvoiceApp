...
**Public-site brand separation plan:** `docs/superpowers/specs/2026-09-23-public-site-brand-separation.md` — WEB-BRAND-1 next; production company site and QA software showcase remain separate audiences
...
The implementation originated on branch `codex/ci-eff-1-proportional-browser-qa`, based on synchronized `main` SHA `784b998a574ded11c67bcde3c717f5bb920deecc`. PR #247 was reviewed on exact head `fc3f446fc541d457e940dc184b6ae6edb3a81126` and squash-merged as `2c499979c1c7a6370fb3fce175c6a7deb0e1fcaa`. Demo Visual QA now uses a bounded ordered worker pool and a repository-owned changed-file selector. The selector maps known source/test areas to catalog route IDs, then derives exact `feature@route` filters from `scripts/qa/demoScenarios.ts`; uncertain, shared, incomplete, unknown, or browser-infrastructure changes use the full catalog. The existing `DEMO_QA_FEATURES` whole-feature form remains supported. PR file selection is accepted only when the changed-file list is complete and its head/base match the triggering event.
...
Exhaustive Demo Visual QA remains on relevant `main` pushes and full-fallback PRs. The existing scheduled full-regression workflow remains unchanged and continues to cover application/database regression rather than adding a duplicate browser schedule. The protected job/check name `chromium-demo-qa` and exact PR-head checkout are unchanged. On exact head `fc3f446fc541d457e940dc184b6ae6edb3a81126`, Application Validation, Database Migration & Invariant Tests, Workflow Map Consistency, and Demo Visual QA all passed. The browser run completed the full 365-scenario catalog with four workers; the complete job fell from the recorded 11m50s baseline to 4m39s, while the capture portion completed in about 3m02s versus the prior 9m23s. Review also tightened fail-closed selection so Markdown outside known documentation locations is never silently skipped and relevant `index.html` / `tsconfig*.json` main-push changes trigger browser QA.
...
5. **REL-AUTH-1 — Idle Session & Deployment Access Recovery: MERGED.** PR #243 merged as `9da3ada95934ab0f14d906ab766d5a8b71bb9dcf`. Same-user workspaces remain available through temporary verification failures; stale-session recovery is bounded; real access loss remains fail-closed; terminal session expiry routes to sign-in with clear copy. The final integrated PR head passed Application Validation, Database Migration & Invariant Tests, Workflow Map Consistency, and Demo Visual QA. The exact deployed trigger remains unproven, so this merge does not claim hosted runtime certification.
6. **UI-R4E — app-wide rollout + professional certification: MERGED.** PR #245 merged as `30a42e926cb0f82948a6d7a217b809f22efc77e8` after exact review head `67ba197782b802d51d456bd941b7ecce297dc71c` passed Application Validation, Database Migration & Invariant Tests, Workflow Map Consistency, and Demo Visual QA. Review corrected one stale Payroll browser-copy assertion, restored the intended >=3:1 dark control-boundary contrast, added concise failed-assertion CI logging, and added cancel-in-progress concurrency to Demo Visual QA. The app-wide shell/theme/action/density rollout and lead-inspected System/Light/Dark responsive evidence remain recorded in `artifacts/ui-ux-audit/UI-R4E-APP-WIDE-CERTIFICATION.md`. Hosted, provider, production, and deployed-session certification remain unverified.
7. **CI-EFF-1 — Protected CI proportional browser execution: MERGED.** PR #247 merged as `2c499979c1c7a6370fb3fce175c6a7deb0e1fcaa` after exact head `fc3f446fc541d457e940dc184b6ae6edb3a81126` passed all four protected workflows. Demo Visual QA now uses measured four-worker bounded concurrency, conservative changed-file → route/feature selection with full-catalog fallback for shared/global/ambiguous inputs, deterministic pinned Playwright, and no duplicate Demo lint/typecheck. Database risk validation and protected check names remain unchanged.
8. **WEB-BRAND-1 — Production Company Landing + QA Software Showcase Separation: NEXT, NOT YET IMPLEMENTED.** The governing spec is `docs/superpowers/specs/2026-09-23-public-site-brand-separation.md`. Production/canonical public branding represents Hydroqualisense Solutions Corp. as the engineering company using only verified services and approved real project experience; QA remains an explicit synthetic software/workspace showcase; no permanent creator/software-vendor brand has been chosen or may be invented. CI-EFF-1 is now merged, so WEB-BRAND-1 is the next owner-directed public-site phase; keep it separate from unrelated authenticated-product or deferred feature work.
