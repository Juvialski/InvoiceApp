...
6. **UI-R4E — app-wide rollout + professional certification: IMPLEMENTED FOR RECORDED LOCAL/DEMO SCOPE; PR #245 OPEN.** The app-wide shell/theme/action/density rollout and lead-inspected System/Light/Dark responsive evidence are recorded in `artifacts/ui-ux-audit/UI-R4E-APP-WIDE-CERTIFICATION.md`. The desktop shell has no redundant permanent upper row, duplicate page/product/account identity, permanent successful sync label, or global Export; invoice export remains contextual. Account identity + Log out live in the lower-left sidebar, with a minimal tablet/phone navigation header. Hosted, provider, production, and deployed-session certification remain unverified. WEB-BRAND-1 remains separate and unstarted.
7. **WEB-BRAND-1 — Production Company Landing + QA Software Showcase Separation: PLANNED, NOT IMPLEMENTED.** The governing spec is `docs/superpowers/specs/2026-09-23-public-site-brand-separation.md`. Production/canonical public branding represents Hydroqualisense Solutions Corp. as the engineering company using only verified services and approved real project experience; QA remains an explicit synthetic software/workspace showcase; no permanent creator/software-vendor brand has been chosen or may be invented. Keep this as a separate owner-directed public-site phase after UI-R4E; do not broaden the authenticated workspace rollout into public-site implementation.
