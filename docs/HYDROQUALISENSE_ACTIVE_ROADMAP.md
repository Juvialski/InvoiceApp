...
5. **REL-AUTH-1 — Idle Session & Deployment Access Recovery: IMPLEMENTED FOR THE RECORDED LOCAL STATE CONTRACT.** Same-user workspaces remain available through temporary verification failures; stale-session recovery is bounded; real access loss remains fail-closed; terminal session expiry routes to sign-in with clear copy. The exact deployed trigger remains unproven, and hosted runtime recovery has not been certified. Keep the broad R4E visual migration separate.
