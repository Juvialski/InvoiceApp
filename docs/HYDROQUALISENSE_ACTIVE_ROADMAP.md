...
8. **WEB-BRAND-1 — Production Company Landing + QA Software Showcase Separation: MERGED FOR REPOSITORY SCOPE.** The governing spec is `docs/superpowers/specs/2026-09-23-public-site-brand-separation.md`. The canonical root now presents Hydroqualisense Solutions Corp. as an engineering company using the confirmed water treatment, water management, and related engineering-project themes. QA has a neutral, explicitly synthetic software showcase. Real company project references, project photography, and public inquiry contact details remain pending company confirmation, so full corporate-content certification is not claimed. The authenticated client workspace identity remains unchanged.
