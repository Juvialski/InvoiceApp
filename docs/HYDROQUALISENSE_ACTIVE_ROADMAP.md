...
7. **CI-EFF-1 — Protected CI proportional browser execution: NEXT.** Optimize Demo Visual QA execution strategy without deleting coverage: evaluate bounded browser parallelism, conservative changed-file → feature selection with automatic full-suite fallback for global/ambiguous changes, preserve exhaustive main/scheduled/release coverage, reduce duplicate setup/lint/build where exact-head guarantees remain sound, and evaluate reproducible pinned/cached Playwright. Required protected check names stay stable, and database fast-pass/full DB-risk validation must remain unchanged.
8. **WEB-BRAND-1 — Production Company Landing + QA Software Showcase Separation: PLANNED, NOT IMPLEMENTED.** The governing spec is `docs/superpowers/specs/2026-09-23-public-site-brand-separation.md`. Production/canonical public branding represents Hydroqualisense Solutions Corp. as the engineering company using only verified services and approved real project experience; QA remains an explicit synthetic software/workspace showcase; no permanent creator/software-vendor brand has been chosen or may be invented. Keep this as a separate owner-directed public-site phase after the explicitly prioritized CI-EFF-1 work; do not broaden CI work into public-site implementation.
...
UI-R4D is implemented for its recorded local/demo and local-Supabase scope; its durable storage/security/database and lead-inspected visual evidence is recorded in `artifacts/ui-ux-audit/UI-R4D-ENTITY-MEDIA.md`. Each Round 4 visual slice must include lead-inspected screenshots at affected standard viewports; automated overflow/accessibility checks alone are insufficient. REL-AUTH-1 is merged; hosted session recovery and the precise deployed trigger remain unverified. UI-R4E is implemented for local synthetic/demo scope on its focused branch; see the certification report for the exact matrix, checks, screenshots, and limits. WEB-BRAND-1 remains a separate planned public-site phase and was not implemented in R4E.
