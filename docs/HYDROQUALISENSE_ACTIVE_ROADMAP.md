...
The exact next implementation phase is **UI-R4C — Home Dashboard + Project Portfolio redesign**. R4C must consume the R4B grammar, simplify Home into orientation/attention/launch, and refine Project-card composition without changing financial truth or starting entity media. Each Round 4 implementation slice must include actual screenshot inspection by the lead at the affected standard viewports; automated overflow/accessibility checks are not sufficient evidence of professional visual quality.
