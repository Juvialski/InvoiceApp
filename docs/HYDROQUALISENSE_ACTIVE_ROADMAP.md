...
4. **UI-R4D — relevant entity media foundation: NEXT APPROVED ROUND 4 PHASE.** Keep project/equipment/material media work within its bounded design and storage/security validation requirements. Any storage/schema/RLS work requires explicit design and real local Supabase/Docker validation.
5. **UI-R4E — app-wide rollout + professional certification: AFTER R4D.** Apply the approved shared grammar to remaining existing routes and certify both light and dark themes across the standard four viewports, including keyboard/accessibility and visual inspection.
...
The next approved Round 4 phase is **UI-R4D — relevant entity media foundation**. Preserve its explicit storage, security, and database validation boundaries. R4C did not begin media work. Each Round 4 implementation slice must include actual screenshot inspection by the lead at affected standard viewports; automated overflow/accessibility checks are not sufficient evidence of professional visual quality.
