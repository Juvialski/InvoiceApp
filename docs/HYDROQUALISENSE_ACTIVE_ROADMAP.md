...
6. **UI-R4E — app-wide rollout + professional certification: NEXT.** Apply the approved shared grammar to remaining existing routes and certify Light, Dark, and System behavior across desktop, constrained laptop, tablet, and phone, including keyboard/focus/touch and lead visual inspection. Desktop shell cleanup removes the redundant permanent upper row and duplicate page/product identity; normal successful sync is silent; global Export is removed in favor of contextual exports in Documents/Reports/relevant workflows; duplicate account/email chrome is removed; account identity + Log out live in the lower-left sidebar; mobile/tablet may retain only a minimal navigation-trigger header where necessary.
7. **WEB-BRAND-1 — Production Company Landing + QA Software Showcase Separation: PLANNED, NOT IMPLEMENTED.** The governing spec is `docs/superpowers/specs/2026-09-23-public-site-brand-separation.md`. Production/canonical public branding represents Hydroqualisense Solutions Corp. as the engineering company using only verified services and approved real project experience; QA remains an explicit synthetic software/workspace showcase; no permanent creator/software-vendor brand has been chosen or may be invented. This bounded public-site phase may be scheduled before or after UI-R4E only by explicit owner reprioritization; the current sequence keeps UI-R4E next.
