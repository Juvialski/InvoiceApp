...
1. **Zero in-database binary blobs**: Verified that 0 `bytea` or BLOB columns exist in PostgreSQL. All durable physical files (PDFs, images, workbooks, `.eml` files) are stored in private Supabase Storage.
...
4. **Engineering document revisions maintain independent immutable provenance**: `engineering_document_revisions` retains its own append-only revision records, physical files, and `file_fingerprint` hashes. It is not forced into `source_documents`.
...
| Bucket Name | Visibility | Storage RLS Read Policy | Storage RLS Insert Policy | Update / Delete Policy | Stored Content |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `invoice-originals` | Private | `invoices.read` or `expenses.read` via `private.storage_company_id(name)` | `invoices.manage` or `expenses.manage` via `private.storage_company_id(name)` | Revoked / No policy (failed-upload compensation via client before DB commit) | Manual invoice scans, receipts, Gmail invoice/statement attachments |
...
| `engineering-documents` | Private | `engineering.documents.read` via `private.storage_company_id(name)` | `engineering.documents.create` on immutable revision PDF paths | **Revoked / No policy** (hardened via migration `20260826140000`; Storage RLS denies updates/deletes even to managers; unprovenanced upload compensation only) | Blueprint drawings, CAD sheet exports, and revision PDFs |
...
- **Validation**: Magic byte signature verification (`validateInvoiceDocumentBytes`) and extension matching.
...
- **Validation**: `validateGmailRawMessage`, `validateGmailAttachmentEnvelope` (max 20 attachments, max 25 MB aggregate), `validateGmailAttachmentBytes`.
...
- **Metadata Table**: `public.payroll_import_batches` (`storage_path`, `file_sha256`, `file_size`)
- **SHA-256 Behavior**: `file_sha256` recorded on batch; duplicate batch check warns user on re-imports. Staging rows (`payroll_import_rows`) reference the batch ID.
- **Validation**: `validatePayrollImportBytes` (ZIP/OLE magic bytes and non-binary CSV check).
...
- **SHA-256 Behavior**: `file_fingerprint` format `sha256:<hex64>`; indexed by `(company_id, file_fingerprint)`.
- **Immutability Invariant**: Committed revisions are strictly append-only. Storage update and delete policies are permanently revoked. Revision records and storage paths cannot be altered or deleted by normal users or managers.
- **Compensation**: Unprovenanced uploads compensated via `compensateUnprovenancedEngineeringDocumentUpload` only if atomic DB RPC fails before commit.
...
| **AI Assistant** | `assistant_messages.content` | JSONB | Chat turn history |
| **AI Assistant** | `assistant_action_events.normalized_args / preview` | JSONB | Action proposal payloads |
...
| :--- | :--- | :--- | :--- |
| **Authoritative / Must Retain** | `invoices.current_data`, `invoice_line_items.item_data`, `payroll_entries.calculation_snapshot`, `payroll_entries.cost_context`, `financial_transactions`, `financial_transaction_matches`, `engineering_documents`, `engineering_document_revisions`, `drawing_annotations`, master entities | Permanent business and engineering truth. Protected by core financial integrity and revision immutability policies. | Retain permanently; ensure optimized indexing. |
...
| **Reconstructable** | `invoice_extractions` (`raw_result`, `structured_result`, `validation_result`), `financial_balance_snapshots` | Extraction outputs are derived from immutable `source_documents` via AI/OCR; balance snapshots can be recalculated. | Eligible for compression or truncation in S4. |
| **Temporary / Staging** | `payroll_import_rows` (`STAGED`, `READY`, `SKIPPED`, `ERROR`), uncommitted `payroll_import_batches`, pending `assistant_action_events` (`PREPARED`) | Active operational staging prior to commit or confirmation. | Retain during active workflow; safe cleanup on failure. |
| **Potentially Safely Prunable Later** | 1. Failed/voided import batches (`payroll_import_batches` with status `FAILED`/`VOIDED`).<br>2. Expired assistant actions (`assistant_action_events` where `expires_at < now() - interval '30 days'`).<br>3. `payroll_import_rows.raw_row` JSONB dumps after batch is committed.<br>4. Discarded draft extractions without linked invoices. | High-safety pruning candidates for future retention/cleanup automation once measurement baseline is established. | Implement safe cleanup jobs in S4/S5. |
...
### 3.3 Index Access Paths & Potential Redundancy Candidates

#### A. Potentially Redundant Index Candidates (Legacy User-ID vs Company-ID)
During the transition from single-user to single-company deployment architecture, `company_id`-prefixed composite indexes were added. Several legacy `user_id`-prefixed indexes remain in the schema as **potentially redundant / index-removal candidates pending production measurements**:
1. **`payroll_import_rows`**:
   ...
   - *Analysis*: 5 separate index writes per row on high-volume spreadsheet ingestion. Exact scan frequency and query plan equivalence will be measured with `scripts/database-storage-audit.sql` before dropping legacy indexes in Wave S4.
2. **`payroll_entries`**:
...
#### B. Index Opportunity
- **`work_entries`**: Has `company_id` column added in tenancy migrations, but only has `work_entries_project_date_idx (user_id, project_id, work_date desc)` and `work_entries_worker_date_idx (user_id, worker_id, work_date desc)`. In Wave S4, a composite index on `(company_id, project_id, work_date)` should be evaluated.
...
- **Safety Test**: `tests/databaseStorageAudit.test.ts` (100% passing)

### Diagnostic Capabilities:
1. **Global Database Overview**: Server version, total database bytes, table/index counts, live tuple estimates.
2. **Relation Sizing Breakdown**: Dissects relation size using PostgreSQL catalog functions (`pg_relation_size` for heap, `coalesce(pg_total_relation_size(c.reltoastrelid), 0)` for TOAST, `pg_indexes_size` for indexes, `pg_total_relation_size` for total).
...
7. **Storage Object Tracking**: Inventories tracked file counts and aggregate byte sizes recorded across `source_documents` (`file_size`), `engineering_document_revisions` (`file_size_bytes`), and `payroll_import_batches` (`file_size`).
...
4. **Engineering revisions preserve per-revision provenance**: Even if a PDF matches an earlier revision or existing record, `evaluateDedupStrategy` always returns `CREATE_PROVENANCE_RECORD` and forbids binary sharing (`allowBinarySharing: false`).
...
1. Proof that no active or auditable record (`invoices`, `expenses`, `financial_import_batches`, `payroll_import_batches`, `engineering_document_revisions`, `email_messages`) references the object.
...
2. **Proven SHA-256 Foundation**: `source_documents` already stores `storage_path`, `sha256`, `mime_type`, and `file_size`.
...
7. `tests/databaseStorageAudit.test.ts` (safety and structural test for diagnostic SQL).
8. `tests/documentStorageFoundation.test.ts` (unit tests for storage keys, sanitization, and dedup contracts).

### Validation Run:
- `npm.cmd test`: **1016 passed, 0 failed, 1 skipped** (100% pass rate).
- `npm.cmd run lint`: **0 errors**.
- `npm.cmd run build`: **Success (24.36s)**.
