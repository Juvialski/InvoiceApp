...
The application currently spans project controls, supplier invoices and verification, expenses, procurement, cash/banking, client billing/collections, workforce/payroll, engineering documents and field records, reporting, document templates/exports, guarded AI assistance, and outbound Email/SMS workflows.

These domains may share navigation and reporting surfaces, but their source-of-truth rules must remain explicit. UI consolidation must not collapse distinct economic or historical events into one mutable record.
...
- Source invoice, recognized cost/expense, payment/settlement evidence, client billing, and client collection are distinct concepts unless an approved domain contract says otherwise.
...
Application roles and permissions are enforced at multiple layers. UI visibility is not security. Protected server routes and database operations must independently validate the authenticated user, deployment company membership, required permission, and company-bound target rows.
